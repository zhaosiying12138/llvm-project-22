# RUN: llvm-mc %s -triple=ysx64 | FileCheck -check-prefixes=ASM %s
# RUN: llvm-mc %s -triple=ysx64 -mattr=+relax | FileCheck -check-prefix=ASM %s
# RUN: llvm-mc -filetype=obj -triple ysx64 < %s \
# RUN:   | llvm-objdump --triple=ysx64 -d -M no-aliases - | FileCheck --check-prefixes=DISASM,DISASM-NORELAX %s
# RUN: llvm-mc -filetype=obj -triple ysx64 -mattr=+relax < %s \
# RUN:   | llvm-objdump --triple=ysx64 -d -M no-aliases - | FileCheck --check-prefixes=DISASM,DISASM-RELAX %s
