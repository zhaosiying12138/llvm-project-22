...
#
#
# RUN: llvm-mc %s -triple=ysx64 -mattr=+zaamo -M no-aliases -show-encoding \
# RUN:     | FileCheck -check-prefixes=CHECK-ASM,CHECK-ASM-AND-OBJ %s
# RUN: llvm-mc -filetype=obj -triple=ysx64 -mattr=+zaamo < %s \
# RUN:     | llvm-objdump --triple=ysx64 --mattr=+zaamo -M no-aliases -d -r - \
# RUN:     | FileCheck --check-prefix=CHECK-ASM-AND-OBJ %s
#
