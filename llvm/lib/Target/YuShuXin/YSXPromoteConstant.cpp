//==- YSXPromoteConstant.cpp - Promote constant fp to global for RISC-V --==//
//
...
#define DEBUG_TYPE "ysx-promote-const"
#define YSX_PROMOTE_CONSTANT_NAME "RISC-V Promote Constants"
...
class YSXPromoteConstant : public ModulePass {
public:
  static char ID;
  YSXPromoteConstant() : ModulePass(ID) {}

  StringRef getPassName() const override { return YSX_PROMOTE_CONSTANT_NAME; }
  ...
  bool runOnModule(Module &M) override {
    ...
    bool Changed = false;
    for (Function &F : M) {
      const YSXSubtarget &ST = TM.getSubtarget<YSXSubtarget>(F);
      const YSXTargetLowering *TLI = ST.getTargetLowering();
...
private:
  bool runOnFunction(Function &F, const YSXTargetLowering *TLI);
};
} // end anonymous namespace

char YSXPromoteConstant::ID = 0;

INITIALIZE_PASS(YSXPromoteConstant, DEBUG_TYPE, YSX_PROMOTE_CONSTANT_NAME,
                false, false)

ModulePass *llvm::createYSXPromoteConstantPass() {
  return new YSXPromoteConstant();
}

bool YSXPromoteConstant::runOnFunction(Function &F,
