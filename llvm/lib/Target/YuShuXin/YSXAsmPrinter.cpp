//===-- YSXAsmPrinter.cpp - RISC-V LLVM assembly writer -----------------===//
//
...
#include "MCTargetDesc/YSXELFStreamer.h"
#include "MCTargetDesc/YSXInstPrinter.h"
#include "MCTargetDesc/YSXMCAsmInfo.h"
#include "MCTargetDesc/YSXMatInt.h"
#include "MCTargetDesc/YSXTargetStreamer.h"
#include "YSX.h"
...
namespace {
class YSXAsmPrinter : public AsmPrinter {
public:
  static char ID;
...
public:
  explicit YSXAsmPrinter(TargetMachine &TM,
...
void YSXAsmPrinter::LowerPATCHPOINT(MCStreamer &OutStreamer, StackMaps &SM,
  ...
  if (CalleeMO.isImm()) {
    ...
    EmitToStreamer(OutStreamer,
                   MCInstBuilder(YSX::PseudoCALL).addOperand(CallTargetMCOp));
    EncodedBytes += 8;
  }
...
void YSXAsmPrinter::LowerSTATEPOINT(MCStreamer &OutStreamer, StackMaps &SM,
  ...
  if (unsigned PatchBytes = SOpers.getNumPatchBytes()) {
    ...
    case MachineOperand::MO_Immediate:
      ...
      EmitToStreamer(OutStreamer, MCInstBuilder(YSX::JAL)
                                      .addReg(YSX::X1)
...
void YSXAsmPrinter::emitInstruction(const MachineInstr *MI) {
  YSX_MC::verifyInstructionPredicates(MI->getOpcode(), STI->getFeatureBits());
  ...
  switch (MI->getOpcode()) {
  case YSX::HWASAN_CHECK_MEMACCESS_SHORTGRANULES:
...
bool YSXAsmPrinter::PrintAsmOperand(const MachineInstr *MI, unsigned OpNo,
  ...
  if (ExtraCode && ExtraCode[0]) {
    ...
    case 'z':      // Print zero register if zero, regular printing otherwise.
      if (MO.isImm() && MO.getImm() == 0) {
        OS << YSXInstPrinter::getRegisterName(YSX::X0);
        return false;
      }
      break;
  ...
  case MachineOperand::MO_Immediate:
    ...
    return false;
  case MachineOperand::MO_Register:
    OS << YSXInstPrinter::getRegisterName(MO.getReg());
    return false;
  case MachineOperand::MO_GlobalAddress:
...
bool YSXAsmPrinter::PrintAsmMemoryOperand(const MachineInstr *MI,
                                            unsigned OpNo,
  ...
  // YSXDAGToDAGISel::SelectInlineAsmMemoryOperand).
  if (!AddrReg.isReg())
    return true;
  ...
  OS << "(" << YSXInstPrinter::getRegisterName(AddrReg.getReg()) << ")";
  return false;
}

bool YSXAsmPrinter::emitDirectiveOptionArch() {
  YSXTargetStreamer &RTS =
      static_cast<YSXTargetStreamer &>(*OutStreamer->getTargetStreamer());
  SmallVector<YSXOptionArchArg> NeedEmitStdOptionArgs;
  const MCSubtargetInfo &MCSTI = *TM.getMCSubtargetInfo();
  for (const auto &Feature : YSXFeatureKV) {
    ...
    auto Delta = STI->hasFeature(Feature.Value) ? YSXOptionArchArgType::Plus
                                                : YSXOptionArchArgType::Minus;
...
bool YSXAsmPrinter::runOnMachineFunction(MachineFunction &MF) {
...
void YSXAsmPrinter::LowerPATCHABLE_FUNCTION_ENTER(const MachineInstr *MI) {
  emitSled(MI, SledKind::FUNCTION_ENTER);
}

void YSXAsmPrinter::LowerPATCHABLE_FUNCTION_EXIT(const MachineInstr *MI) {
  emitSled(MI, SledKind::FUNCTION_EXIT);
}

void YSXAsmPrinter::LowerPATCHABLE_TAIL_CALL(const MachineInstr *MI) {
...
void YSXAsmPrinter::emitSled(const MachineInstr *MI, SledKind Kind) {
  ...
  EmitToStreamer(
      *OutStreamer,
      MCInstBuilder(YSX::JAL).addReg(YSX::X0).addExpr(TargetExpr));

  // Emit NOP instructions
  for (int8_t I = 0; I < NoopsInSledCount; ++I)
    EmitToStreamer(*OutStreamer, MCInstBuilder(YSX::ADDI)
                                     .addReg(YSX::X0)
                                     .addReg(YSX::X0)
                                     .addImm(0));
...
void YSXAsmPrinter::emitStartOfAsmFile(Module &M) {
  ...
  if (const MDString *ModuleTargetABI =
    ...
    RTS.setTargetABI(YSXABI::getTargetABI(ModuleTargetABI->getString()));

  MCSubtargetInfo SubtargetInfo = *TM.getMCSubtargetInfo();

  // Use module flag to update feature bits.
  if (auto *MD = dyn_cast_or_null<MDNode>(M.getModuleFlag("ysx-isa"))) {
    for (auto &ISA : MD->operands()) {
      if (auto *ISAString = dyn_cast_or_null<MDString>(ISA)) {
        auto ParseResult = llvm::YSXISAInfo::parseArchString(
...
void YSXAsmPrinter::emitEndOfAsmFile(Module &M) {
  YSXTargetStreamer &RTS =
      static_cast<YSXTargetStreamer &>(*OutStreamer->getTargetStreamer());
...
void YSXAsmPrinter::emitAttributes(const MCSubtargetInfo &SubtargetInfo) {
  YSXTargetStreamer &RTS =
      static_cast<YSXTargetStreamer &>(*OutStreamer->getTargetStreamer());
...
LLVMInitializeYSXAsmPrinter() {
  RegisterAsmPrinter<YSXAsmPrinter> Y(getTheYSX64Target());
}

extern "C" LLVM_ABI LLVM_EXTERNAL_VISIBILITY void
LLVMInitializeYuShuXinAsmPrinter() {
  LLVMInitializeYSXAsmPrinter();
}

void YSXAsmPrinter::LowerHWASAN_CHECK_MEMACCESS(const MachineInstr &MI) {
  ...
  if (!Sym) {
    ...
    std::string SymName = "__hwasan_check_x" + utostr(Reg - YSX::X0) + "_" +
  ...
  EmitToStreamer(*OutStreamer, MCInstBuilder(YSX::PseudoCALL).addExpr(Expr));
}

void YSXAsmPrinter::LowerKCFI_CHECK(const MachineInstr &MI) {
  ...
  // needed.
  unsigned ScratchRegs[] = {YSX::X6, YSX::X7};
  unsigned NextReg = YSX::X28;
  ...
  if (AddrReg == YSX::X0) {
    ...
    EmitToStreamer(*OutStreamer, MCInstBuilder(YSX::LW)
                                     .addReg(ScratchRegs[0])
                                     .addReg(AddrReg)
  ...
  if (Hi20) {
    EmitToStreamer(
        *OutStreamer,
        MCInstBuilder(YSX::LUI).addReg(ScratchRegs[1]).addImm(Hi20));
  }
  if (Lo12 || Hi20 == 0) {
    EmitToStreamer(*OutStreamer,
                   MCInstBuilder((STI->hasFeature(YSX::Feature64Bit) && Hi20)
                                     ? YSX::ADDIW
                                     : YSX::ADDI)
  ...
  EmitToStreamer(*OutStreamer, MCInstBuilder(YSX::EBREAK));
  emitKCFITrapEntry(*MI.getMF(), Trap);
  OutStreamer->emitLabel(Pass);
}

void YSXAsmPrinter::EmitHwasanMemaccessSymbols(Module &M) {
  ...
  for (auto &P : HwasanMemaccessSymbols) {
    ...
    EmitToStreamer(
        *OutStreamer,
        MCInstBuilder(YSX::SLLI).addReg(YSX::X6).addReg(Reg).addImm(8),
        MCSTI);
    EmitToStreamer(*OutStreamer,
                   MCInstBuilder(YSX::SRLI)
                       .addReg(YSX::X6)
                       .addReg(YSX::X6)
                       .addImm(12),
                   MCSTI);
    ...
    EmitToStreamer(
        *OutStreamer,
        MCInstBuilder(YSX::ANDI).addReg(YSX::X28).addReg(Reg).addImm(0xF),
        MCSTI);
    ...
    EmitToStreamer(
        *OutStreamer,
        MCInstBuilder(YSX::ORI).addReg(YSX::X6).addReg(Reg).addImm(0xF),
        MCSTI);
    EmitToStreamer(
        *OutStreamer,
        MCInstBuilder(YSX::LBU).addReg(YSX::X6).addReg(YSX::X6).addImm(0),
    ...
    EmitToStreamer(
        *OutStreamer,
        MCInstBuilder(YSX::SD).addReg(YSX::X8).addReg(YSX::X2).addImm(8 *
                                                                            8),
        MCSTI);
    // store x1(ra) by new sp
    EmitToStreamer(
        *OutStreamer,
        MCInstBuilder(YSX::SD).addReg(YSX::X1).addReg(YSX::X2).addImm(1 *
                                                                            8),
        MCSTI);
    if (Reg != YSX::X10)
      EmitToStreamer(
          *OutStreamer,
          MCInstBuilder(YSX::ADDI).addReg(YSX::X10).addReg(Reg).addImm(0),
          MCSTI);
    EmitToStreamer(*OutStreamer,
                   MCInstBuilder(YSX::ADDI)
                       .addReg(YSX::X11)
...
void YSXAsmPrinter::emitNoteGnuProperty(const Module &M) {
  ...
  if (const Metadata *const Flag = M.getModuleFlag("cf-protection-return");
    ...
    YSXTargetELFStreamer &RTS = static_cast<YSXTargetELFStreamer &>(
...
static MCOperand lowerSymbolOperand(const MachineOperand &MO, MCSymbol *Sym,
  ...
  case YSXII::MO_HI:
    ...
    break;
  case YSXII::MO_PCREL_LO:
    Kind = YSX::S_PCREL_LO;
    break;
  case YSXII::MO_PCREL_HI:
    Kind = ELF::R_RISCV_PCREL_HI20;
    break;
  case YSXII::MO_GOT_HI:
    Kind = ELF::R_RISCV_GOT_HI20;
    break;
  case YSXII::MO_TPREL_LO:
    Kind = YSX::S_TPREL_LO;
    break;
  case YSXII::MO_TPREL_HI:
    Kind = ELF::R_RISCV_TPREL_HI20;
    break;
  case YSXII::MO_TPREL_ADD:
    Kind = ELF::R_RISCV_TPREL_ADD;
    break;
  case YSXII::MO_TLS_GOT_HI:
    Kind = ELF::R_RISCV_TLS_GOT_HI20;
    break;
  case YSXII::MO_TLS_GD_HI:
    Kind = ELF::R_RISCV_TLS_GD_HI20;
    break;
  case YSXII::MO_TLSDESC_HI:
    Kind = ELF::R_RISCV_TLSDESC_HI20;
    break;
  case YSXII::MO_TLSDESC_LOAD_LO:
    Kind = ELF::R_RISCV_TLSDESC_LOAD_LO12;
    break;
  case YSXII::MO_TLSDESC_ADD_LO:
    Kind = ELF::R_RISCV_TLSDESC_ADD_LO12;
    break;
  case YSXII::MO_TLSDESC_CALL:
...
bool YSXAsmPrinter::lowerOperand(const MachineOperand &MO,
...
void YSXAsmPrinter::emitMachineConstantPoolValue(
    MachineConstantPoolValue *MCPV) {
  auto *RCPV = static_cast<YSXConstantPoolValue *>(MCPV);
  MCSymbol *MCSym;
...
char YSXAsmPrinter::ID = 0;

INITIALIZE_PASS(YSXAsmPrinter, "ysx-asm-printer", "RISC-V Assembly Printer",
                false, false)
