//===------ YSXIndirectBranchTracking.cpp - Enables lpad mechanism ------===//
//
...
#define DEBUG_TYPE "ysx-indirect-branch-tracking"
#define PASS_NAME "RISC-V Indirect Branch Tracking"

using namespace llvm;

cl::opt<uint32_t> PreferredLandingPadLabel(
    "ysx-landing-pad-label", cl::ReallyHidden,
    cl::desc("Use preferred fixed label for all labels"));

namespace {
class YSXIndirectBranchTracking : public MachineFunctionPass {
public:
  static char ID;
  YSXIndirectBranchTracking() : MachineFunctionPass(ID) {}
...
INITIALIZE_PASS(YSXIndirectBranchTracking, DEBUG_TYPE, PASS_NAME, false,
                false)

char YSXIndirectBranchTracking::ID = 0;

FunctionPass *llvm::createYSXIndirectBranchTrackingPass() {
  return new YSXIndirectBranchTracking();
}

static void
emitLpad(MachineBasicBlock &MBB, const YSXInstrInfo *TII, uint32_t Label,
         MachineBasicBlock::iterator I = MachineBasicBlock::iterator{}) {
  if (!I.isValid())
    I = MBB.begin();
  BuildMI(MBB, I, MBB.findDebugLoc(I), TII->get(YSX::AUIPC), YSX::X0)
      .addImm(Label);
}
...
bool YSXIndirectBranchTracking::runOnMachineFunction(MachineFunction &MF) {
  ...
  if (PreferredLandingPadLabel.getNumOccurrences() > 0) {
    if (!isUInt<20>(PreferredLandingPadLabel))
      report_fatal_error("ysx-landing-pad-label=<val>, <val> needs to fit in "
                         "unsigned 20-bits");
