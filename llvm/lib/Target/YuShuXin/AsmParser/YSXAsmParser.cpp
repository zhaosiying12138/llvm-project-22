//===-- YSXAsmParser.cpp - Parse RISC-V assembly to MCInst instructions -===//
//
...
class YSXAsmParser : public MCTargetAsmParser {
  ...
  YSXTargetStreamer &getTargetStreamer() {
    assert(getParser().getStreamer().getTargetStreamer() &&
           "do not have a target streamer");
    MCTargetStreamer &TS = *getParser().getStreamer().getTargetStreamer();
    return static_cast<YSXTargetStreamer &>(TS);
  }
...
#undef GET_OPERAND_DIAGNOSTIC_TYPES
  ...
  static bool classifySymbolRef(const MCExpr *Expr, YSX::Specifier &Kind);
...
/// YSXOperand - Instances of this class represent a parsed machine
/// instruction
struct YSXOperand final : public MCParsedAsmOperand {
  ...
  YSXOperand(KindTy K) : Kind(K) {}

public:
  YSXOperand(const YSXOperand &o) : MCParsedAsmOperand() {
    Kind = o.Kind;
    StartLoc = o.StartLoc;
    EndLoc = o.EndLoc;
    switch (Kind) {
  ...
  bool isGPRPair() const {
    return Kind == KindTy::Register &&
           YSXMCRegisterClasses[YSX::GPRPairRegClassID].contains(Reg.Reg);
  }

  bool isGPRPairC() const {
    return Kind == KindTy::Register &&
           YSXMCRegisterClasses[YSX::GPRPairCRegClassID].contains(Reg.Reg);
  }
  ...
  void print(raw_ostream &OS, const MCAsmInfo &MAI) const override {
    auto RegName = [](MCRegister Reg) {
      if (Reg)
        return YSXInstPrinter::getRegisterName(Reg);
      else
        return "noreg";
    };
  ...
  static std::unique_ptr<YSXOperand> createToken(StringRef Str, SMLoc S) {
    auto Op = std::make_unique<YSXOperand>(KindTy::Token);
    Op->Tok = Str;
    Op->StartLoc = S;
    Op->EndLoc = S;
    return Op;
...
bool YSXAsmParser::generateImmOutOfRangeError(
...
bool YSXAsmParser::matchAndEmitInstruction(SMLoc IDLoc, unsigned &Opcode,
  ...
  case Match_InvalidOperand: {
    ...
    if (ErrorInfo != ~0ULL) {
      ...
      ErrorLoc = ((YSXOperand &)*Operands[ErrorInfo]).getStartLoc();
      if (ErrorLoc == SMLoc())
        ErrorLoc = IDLoc;
    }
...
ParseStatus YSXAsmParser::parseRegister(OperandVector &Operands,
  ...
  case AsmToken::Identifier:
    ...
    if (HadParens)
      Operands.push_back(YSXOperand::createToken("(", FirstS));
    SMLoc S = getLoc();
  ...
  if (HadParens) {
    getParser().Lex(); // Eat ')'
    Operands.push_back(YSXOperand::createToken(")", getLoc()));
  }
...
ParseStatus YSXAsmParser::parseOperandWithSpecifier(OperandVector &Operands) {
  SMLoc S = getLoc();
  SMLoc E;
  ...
  if (!Failed)
    Operands.push_back(YSXOperand::createExpr(Expr, S, E, isRV64()));
  return Failed;
}

bool YSXAsmParser::parseExprWithSpecifier(const MCExpr *&Res, SMLoc &E) {
  SMLoc Loc = getLoc();
  if (getLexer().getKind() != AsmToken::Identifier)
    return TokError("expected '%' relocation specifier");
  StringRef Identifier = getParser().getTok().getIdentifier();
  auto Spec = YSX::parseSpecifierName(Identifier);
  if (!Spec)
...
bool YSXAsmParser::parseDataExpr(const MCExpr *&Res) {
  SMLoc E;
...
ParseStatus YSXAsmParser::parseBareSymbol(OperandVector &Operands) {
  SMLoc S = getLoc();
  const MCExpr *Res;
...
ParseStatus YSXAsmParser::parseCallSymbol(OperandVector &Operands) {
  SMLoc S = getLoc();
  const MCExpr *Res;
  ...
  SMLoc E = SMLoc::getFromPointer(S.getPointer() + Identifier.size());
  YSX::Specifier Kind = ELF::R_RISCV_CALL_PLT;
...
ParseStatus YSXAsmParser::parsePseudoJumpSymbol(OperandVector &Operands) {
  SMLoc S = getLoc();
  SMLoc E;
  const MCExpr *Res;
...
ParseStatus YSXAsmParser::parseJALOffset(OperandVector &Operands) {
...
ParseStatus YSXAsmParser::parseFenceArg(OperandVector &Operands) {
  ...
  if (Tok.is(AsmToken::Integer)) {
    ...
    Operands.push_back(YSXOperand::createFenceArg(0, getLoc()));
    Lex();
    return ParseStatus::Success;
  }
  ...
  if (Tok.is(AsmToken::Identifier)) {
    ...
    Operands.push_back(YSXOperand::createFenceArg(Imm, getLoc()));
    Lex();
    return ParseStatus::Success;
  }
...
ParseStatus YSXAsmParser::parseMemOpBaseReg(OperandVector &Operands) {
  if (parseToken(AsmToken::LParen, "expected '('"))
    return ParseStatus::Failure;
  Operands.push_back(YSXOperand::createToken("(", getLoc()));
...
ParseStatus YSXAsmParser::parseZeroOffsetMemOp(OperandVector &Operands) {
  ...
  //
  // These operands are printed with YSXInstPrinter::printZeroOffsetMemOp,
  ...
  std::unique_ptr<YSXOperand> OptionalImmOp;
  ...
  if (getLexer().isNot(AsmToken::LParen)) {
    ...
    // Create a YSXOperand for checking later (so the error messages are
...
ParseStatus YSXAsmParser::parseRegReg(OperandVector &Operands) {
  ...
  if (!OffsetReg ||
      !YSXMCRegisterClasses[YSX::GPRRegClassID].contains(OffsetReg))
  ...
  Operands.push_back(YSXOperand::createRegReg(BaseReg, OffsetReg, S));
...
/// true.
bool YSXAsmParser::parseOperand(OperandVector &Operands, StringRef Mnemonic) {
...
bool YSXAsmParser::parseInstruction(ParseInstructionInfo &Info,
  ...
  Operands.push_back(YSXOperand::createToken(Name, NameLoc));
...
bool YSXAsmParser::classifySymbolRef(const MCExpr *Expr,
  ...
  if (Expr->evaluateAsRelocatable(Res, nullptr))
    return Res.getSpecifier() == YSX::S_None;
  return false;
}

bool YSXAsmParser::isSymbolDiff(const MCExpr *Expr) {
  MCValue Res;
  if (Expr->evaluateAsRelocatable(Res, nullptr)) {
    return Res.getSpecifier() == YSX::S_None && Res.getAddSym() &&
           Res.getSubSym();
  }
  return false;
}
...
bool YSXAsmParser::resetToArch(StringRef Arch, SMLoc Loc, std::string &Result,
  ...
  if (getSTI().getTargetTriple().isYSX64() &&
      !YSXFeatures::isValidYSXISAInfo(*ISAInfo))
...
bool YSXAsmParser::parseDirectiveOption() {
  ...
  if (Option == "arch") {
    ...
    do {
      ...
      if (parseOptionalToken(AsmToken::Plus))
        Type = YSXOptionArchArgType::Plus;
      else if (parseOptionalToken(AsmToken::Minus))
        Type = YSXOptionArchArgType::Minus;
      else if (!Args.empty())
        return Error(Parser.getTok().getLoc(),
                     "unexpected token, expected + or -");
      else
        Type = YSXOptionArchArgType::Full;
      ...
      std::string Feature = YSXISAInfo::getTargetFeatureForExtension(Arch);
      ...
      if (Type == YSXOptionArchArgType::Plus) {
        ...
        // TODO: Make use of YSXISAInfo to handle this
        for (auto &Feature : YSXFeatureKV) {
  ...
  if (Option == "exact") {
    ...
    setFeatureBits(YSX::FeatureExactAssembly, "exact-asm");
    clearFeatureBits(YSX::FeatureRelax, "relax");
    return false;
  }
  ...
  if (Option == "noexact") {
    ...
    clearFeatureBits(YSX::FeatureExactAssembly, "exact-asm");
    setFeatureBits(YSX::FeatureRelax, "relax");
    return false;
  }
...
bool YSXAsmParser::parseDirectiveAttribute() {
  MCAsmParser &Parser = getParser();
  int64_t Tag;
  SMLoc TagLoc;
...
/// ::= .insn [ length, value ]
/// ::= .insn [ value ]
bool YSXAsmParser::parseDirectiveInsn(SMLoc L) {
  MCAsmParser &Parser = getParser();
...
void YSXAsmParser::emitAuipcInstPair(MCRegister DestReg, MCRegister TmpReg,
  ...
  emitToStreamer(Out,
                 MCInstBuilder(YSX::AUIPC).addReg(TmpReg).addExpr(SymbolHi));

  const MCExpr *RefToLinkTmpLabel = MCSpecifierExpr::create(
      MCSymbolRefExpr::create(TmpLabel, Ctx), YSX::S_PCREL_LO, Ctx);
...
void YSXAsmParser::emitLoadGlobalAddress(MCInst &Inst, SMLoc IDLoc,
  ...
  MCRegister DestReg = Inst.getOperand(0).getReg();
  const MCExpr *Symbol = Inst.getOperand(1).getExpr();
  unsigned SecondOpcode = isRV64() ? YSX::LD : YSX::LW;
...
void YSXAsmParser::emitLoadStoreSymbol(MCInst &Inst, unsigned Opcode,
  ...
  if (YSXMCRegisterClasses[YSX::GPRPairRegClassID].contains(TmpReg)) {
    const MCRegisterInfo *RI = getContext().getRegisterInfo();
    TmpReg = RI->getSubReg(TmpReg, YSX::sub_gpr_even);
  }
...
void YSXAsmParser::emitPseudoExtend(MCInst &Inst, bool SignExtend,
  ...
  unsigned SecondOpcode = SignExtend ? YSX::SRAI : YSX::SRLI;
...
bool YSXAsmParser::checkPseudoAddTPRel(MCInst &Inst,
  ...
  if (Inst.getOperand(2).getReg() != YSX::X4) {
    SMLoc ErrorLoc = ((YSXOperand &)*Operands[3]).getStartLoc();
...
bool YSXAsmParser::processInstruction(MCInst &Inst, SMLoc IDLoc,
  ...
  case YSX::PseudoQC_E_LW:
    ...
    return false;
  case YSX::PseudoLWU:
    emitLoadStoreSymbol(Inst, YSX::LWU, IDLoc, Out, /*HasTmpReg=*/false);
    return false;
  case YSX::PseudoLD:
...
extern "C" LLVM_ABI LLVM_EXTERNAL_VISIBILITY void
LLVMInitializeYuShuXinAsmParser() {
  LLVMInitializeYSXAsmParser();
}
