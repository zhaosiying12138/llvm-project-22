...
/// \file
///
/// This file defines the YSXCustomBehaviour class which inherits from
...
#ifndef LLVM_LIB_TARGET_YSX_MCA_YSXCUSTOMBEHAVIOUR_H
#define LLVM_LIB_TARGET_YSX_MCA_YSXCUSTOMBEHAVIOUR_H
...
class YSXLMULInstrument : public Instrument {
public:
  static const StringRef DESC_NAME;
  static bool isDataValid(StringRef Data);

  explicit YSXLMULInstrument(StringRef Data) : Instrument(DESC_NAME, Data) {}

  ~YSXLMULInstrument() override = default;

  uint8_t getLMUL() const;
};

class YSXSEWInstrument : public Instrument {
public:
  static const StringRef DESC_NAME;
  static bool isDataValid(StringRef Data);

  explicit YSXSEWInstrument(StringRef Data) : Instrument(DESC_NAME, Data) {}

  ~YSXSEWInstrument() override = default;

  uint8_t getSEW() const;
};

class YSXInstrumentManager : public InstrumentManager {
public:
  YSXInstrumentManager(const MCSubtargetInfo &STI, const MCInstrInfo &MCII)
