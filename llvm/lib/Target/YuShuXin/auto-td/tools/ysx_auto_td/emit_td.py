...
from .validate import SUPPORTED_BUILTIN_CODEGEN_OPERATIONS
...
def write_clang_builtins_td(path: Path, instructions) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(_emit_clang_builtins_td(instructions))


def write_clang_builtin_cg_inc(path: Path, instructions) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(_emit_clang_builtin_cg_inc(instructions))


def write_llvm_intrinsics_td(path: Path, instructions) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(_emit_llvm_intrinsics_td(instructions))
...
def _emit_tiny_v_builtins(instructions) -> str:
    ...
    for instruction in sorted(instructions, key=lambda record: record.mnemonic):
        ...
        codegen = _manifest_bool(instruction.builtin.get("codegen", False))
        for name in instruction.builtin["names"]:
            lines.append(
                ...
                f"codegen={codegen} record={_record_name(instruction)}"
            )
    if len(lines) == 1:
...
def _emit_clang_builtins_td(instructions) -> str:
    lines = [GENERATED_HEADER.rstrip(), "let Attributes = [NoThrow, Const] in {"]
    for instruction, pattern in _codegen_builtin_patterns(instructions):
        for name in instruction.builtin["names"]:
            def_name = _clang_builtin_def_name(name)
            prototype = _clang_builtin_prototype(name)
            lines.append(
                f'def {def_name} : YSXBuiltin<"{prototype}", '
                '"xtinyv,zvl128b">;'
            )
    lines.append("} // Attributes = [Const, NoThrow]")
    return "\n".join(lines).rstrip() + "\n"


def _emit_clang_builtin_cg_inc(instructions) -> str:
    lines = [GENERATED_HEADER.rstrip()]
    for instruction, pattern in _codegen_builtin_patterns(instructions):
        intrinsic = _cpp_intrinsic_name(pattern)
        overloads = _cpp_intrinsic_overloads(pattern)
        for name in instruction.builtin["names"]:
            lines.extend(
                [
                    f"case YSX::BI__builtin_ysx_{_clang_builtin_def_name(name)}: {{",
                    f"  llvm::Function *Fn = CGM.getIntrinsic({intrinsic},",
                    f"                                            {{{overloads}}});",
...
def _emit_llvm_intrinsics_td(instructions) -> str:
    lines = [GENERATED_HEADER.rstrip()]
    seen: set[str] = set()
    for _instruction, pattern in _codegen_builtin_patterns(instructions):
        intrinsic = pattern["intrinsic"]
        if intrinsic in seen:
            continue
        seen.add(intrinsic)
        def_name = "int_" + intrinsic.replace(".", "_")
        params = _llvm_intrinsic_params(pattern)
        lines.extend(
            [
                f"def {def_name}",
...
def _codegen_builtin_patterns(instructions):
    result = []
    for instruction in sorted(instructions, key=lambda record: record.mnemonic):
        if not instruction.builtin or not instruction.builtin.get("codegen", False):
            continue
        pattern = _intrinsic_pattern(instruction)
        _ensure_supported_codegen_pattern(instruction, pattern)
        result.append((instruction, pattern))
    return result
...
def _ensure_supported_codegen_pattern(instruction, pattern) -> None:
...
def _clang_builtin_def_name(name: str) -> str:
    if not name.startswith("ysx_"):
        raise ValueError(f"builtin name must start with ysx_: {name}")
    return name.removeprefix("ysx_")


def _clang_builtin_prototype(name: str) -> str:
    if name.endswith("_i32m1"):
        vector = "_ExtVector<4, int>"
    elif name.endswith("_f32m1"):
        vector = "_ExtVector<4, float>"
    else:
        raise ValueError(f"unsupported builtin vector type: {name}")

    if "_vv_" in name or "_vs_" in name:
        return f"{vector}({vector}, {vector}, unsigned long int)"
    if "_vx_" in name:
        return f"{vector}({vector}, unsigned long int, unsigned long int)"
    if "_v_" in name:
        return f"{vector}({vector}, unsigned long int)"
    raise ValueError(f"unsupported builtin prototype shape: {name}")


def _cpp_intrinsic_name(pattern) -> str:
    return "Intrinsic::" + pattern["intrinsic"].replace(".", "_")


def _cpp_intrinsic_overloads(pattern) -> str:
    operation = pattern["operation"]
    if operation in {"add", "sub", "mul", "ireduce_sum", "freduce_sum", "gather"}:
        return "ResultType, Ops[2]->getType()"
    if operation == "slideup":
        return "ResultType, Ops[1]->getType(), Ops[2]->getType()"
    if operation == "fexp":
        return "ResultType, Ops[1]->getType()"
    raise ValueError(f"unsupported builtin.codegen operation {operation}")


def _llvm_intrinsic_params(pattern) -> str:
    operation = pattern["operation"]
    if operation in {"add", "sub", "mul", "ireduce_sum", "freduce_sum", "gather"}:
        return "LLVMMatchType<0>, LLVMMatchType<0>, llvm_anyint_ty"
    if operation == "slideup":
        return "LLVMMatchType<0>, llvm_anyint_ty, llvm_anyint_ty"
    if operation == "fexp":
        return "LLVMMatchType<0>, llvm_anyint_ty"
    raise ValueError(f"unsupported builtin.codegen operation {operation}")
