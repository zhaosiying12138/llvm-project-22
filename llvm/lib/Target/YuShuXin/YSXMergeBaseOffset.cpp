//===----- YSXMergeBaseOffset.cpp - Optimise address calculations  ------===//
//
...
#define DEBUG_TYPE "ysx-merge-base-offset"
#define YSX_MERGE_BASE_OFFSET_NAME "RISC-V Merge Base Offset"
namespace {

class YSXMergeBaseOffsetOpt : public MachineFunctionPass {
  const YSXSubtarget *ST = nullptr;
...
public:
  ...
  YSXMergeBaseOffsetOpt() : MachineFunctionPass(ID) {}
  ...
  StringRef getPassName() const override {
    return YSX_MERGE_BASE_OFFSET_NAME;
  }
};
} // end anonymous namespace

char YSXMergeBaseOffsetOpt::ID = 0;
INITIALIZE_PASS(YSXMergeBaseOffsetOpt, DEBUG_TYPE,
                YSX_MERGE_BASE_OFFSET_NAME, false, false)
...
bool YSXMergeBaseOffsetOpt::detectFoldable(MachineInstr &Hi,
                                             MachineInstr *&Lo) {
  auto HiOpc = Hi.getOpcode();
  if (HiOpc != YSX::LUI && HiOpc != YSX::AUIPC &&
      HiOpc != YSX::PseudoMovAddr && HiOpc != YSX::QC_E_LI)
    return false;

  const MachineOperand &HiOp1 = Hi.getOperand(1);
  unsigned ExpectedFlags = HiOpc == YSX::AUIPC     ? YSXII::MO_PCREL_HI
                           : HiOpc == YSX::QC_E_LI ? YSXII::MO_None
                                                     : YSXII::MO_HI;
  ...
  if (HiOpc == YSX::PseudoMovAddr || HiOpc == YSX::QC_E_LI) {
    ...
    if (Lo->getOpcode() != YSX::ADDI)
      return false;
  }

  if (HiOpc != YSX::QC_E_LI) {
    const MachineOperand &LoOp2 = Lo->getOperand(2);
    if (HiOpc == YSX::LUI || HiOpc == YSX::PseudoMovAddr) {
      if (LoOp2.getTargetFlags() != YSXII::MO_LO ||
          !(LoOp2.isGlobal() || LoOp2.isCPI() || LoOp2.isBlockAddress()) ||
          LoOp2.getOffset() != 0)
        return false;
    } else {
      assert(HiOpc == YSX::AUIPC);
      if (LoOp2.getTargetFlags() != YSXII::MO_PCREL_LO ||
...
// output from Lo.
bool YSXMergeBaseOffsetOpt::foldOffset(MachineInstr &Hi, MachineInstr &Lo,
  ...
  auto HiOpc = Hi.getOpcode();
  if (HiOpc == YSX::AUIPC && Hi.getOperand(1).isGlobal()) {
  ...
  if (Hi.getOpcode() != YSX::AUIPC && Hi.getOpcode() != YSX::QC_E_LI)
...
bool YSXMergeBaseOffsetOpt::foldLargeOffset(MachineInstr &Hi,
                                              MachineInstr &Lo,
                                              MachineInstr &TailAdd,
                                              Register GAReg) {
  assert((TailAdd.getOpcode() == YSX::ADD) && "Expected ADD instruction!");
  ...
  if (OffsetTailOpc == YSX::ADDI || OffsetTailOpc == YSX::ADDIW) {
    ...
    if (AddiImmOp.getTargetFlags() != YSXII::MO_None)
      return false;
    ...
    if (OffsetLui.getOpcode() != YSX::LUI ||
        LuiImmOp.getTargetFlags() != YSXII::MO_None ||
    ...
    if (!ST->is64Bit() || OffsetTailOpc == YSX::ADDIW)
...
bool YSXMergeBaseOffsetOpt::foldShiftedOffset(MachineInstr &Hi,
                                                MachineInstr &Lo,
                                                MachineInstr &TailShXAdd,
                                                Register GAReg) {
  assert((TailShXAdd.getOpcode() == YSX::SH1ADD ||
          TailShXAdd.getOpcode() == YSX::SH2ADD ||
          TailShXAdd.getOpcode() == YSX::SH3ADD) &&
  ...
  if (OffsetTail.getOpcode() != YSX::ADDI)
    return false;
  if (!OffsetTail.getOperand(1).isReg() ||
      OffsetTail.getOperand(1).getReg() != YSX::X0 ||
  ...
  case YSX::SH1ADD: ShAmt = 1; break;
  case YSX::SH2ADD: ShAmt = 2; break;
  case YSX::SH3ADD: ShAmt = 3; break;
  }
...
bool YSXMergeBaseOffsetOpt::detectAndFoldOffset(MachineInstr &Hi,
                                                  MachineInstr &Lo) {
  ...
  case YSX::QC_E_ADDAI: {
    ...
    if (Tail.getOpcode() == YSX::ADDI) {
      ...
      if (MRI->hasOneUse(TailDestReg)) {
        ...
        if (TailTail.getOpcode() == YSX::ADDI) {
...
bool YSXMergeBaseOffsetOpt::foldIntoMemoryOps(MachineInstr &Hi,
                                                MachineInstr &Lo) {
  Register DestReg = Lo.getOperand(0).getReg();
  ...
  MachineOperand &ImmOp =
      Hi.getOpcode() == YSX::QC_E_LI ? Lo.getOperand(1) : Lo.getOperand(2);
  auto HiOpc = Hi.getOpcode();
  // Expand PseudoMovAddr into LUI
  if (HiOpc == YSX::PseudoMovAddr) {
    auto *TII = ST->getInstrInfo();
    Hi.setDesc(TII->get(YSX::LUI));
    Hi.removeOperand(2);
  }
  ...
  for (MachineInstr &UseMI :
    ...
    if (UseMI.getOpcode() == YSX::INLINEASM ||
        UseMI.getOpcode() == YSX::INLINEASM_BR) {
...
bool YSXMergeBaseOffsetOpt::runOnMachineFunction(MachineFunction &Fn) {
  if (skipFunction(Fn.getFunction()))
    return false;

  ST = &Fn.getSubtarget<YSXSubtarget>();
...
FunctionPass *llvm::createYSXMergeBaseOffsetOptPass() {
  return new YSXMergeBaseOffsetOpt();
}
