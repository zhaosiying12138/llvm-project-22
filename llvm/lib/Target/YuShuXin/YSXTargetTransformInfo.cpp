//===-- YSXTargetTransformInfo.cpp - RISC-V specific TTI ----------------===//
//
...
#define DEBUG_TYPE "ysxtti"

static cl::opt<unsigned> RVVRegisterWidthLMUL(
    "ysx-v-register-bit-width-lmul",
    cl::desc(
...
static cl::opt<unsigned>
    RVVMinTripCount("ysx-v-min-trip-count",
                    cl::desc("Set the lower bound of a trip count to decide on "
                             "vectorization while tail-folding."),
                    cl::init(5), cl::Hidden);

InstructionCost
YSXTTIImpl::getYSXInstructionCost(ArrayRef<unsigned> OpCodes, MVT VT,
  ...
  for (auto Op : OpCodes) {
    switch (Op) {
    case YSX::VRGATHER_VI:
      Cost += TLI->getVRGatherVICost(VT);
      break;
    case YSX::VRGATHER_VV:
      Cost += TLI->getVRGatherVVCost(VT);
      break;
    case YSX::VSLIDEUP_VI:
    case YSX::VSLIDEDOWN_VI:
      Cost += TLI->getVSlideVICost(VT);
      break;
    case YSX::VSLIDEUP_VX:
    case YSX::VSLIDEDOWN_VX:
      Cost += TLI->getVSlideVXCost(VT);
      break;
    case YSX::VREDMAX_VS:
    case YSX::VREDMIN_VS:
    case YSX::VREDMAXU_VS:
    case YSX::VREDMINU_VS:
    case YSX::VREDSUM_VS:
    case YSX::VREDAND_VS:
    case YSX::VREDOR_VS:
    case YSX::VREDXOR_VS:
    case YSX::VFREDMAX_VS:
    case YSX::VFREDMIN_VS:
    case YSX::VFREDUSUM_VS: {
...
static InstructionCost getIntImmCostImpl(const DataLayout &DL,
  ...
  return YSXMatInt::getIntMatCost(Imm, DL.getTypeSizeInBits(Ty), *ST,
                                    /*CompressionCost=*/false, FreeZeroes);
}

InstructionCost
YSXTTIImpl::getIntImmCost(const APInt &Imm, Type *Ty,
                            TTI::TargetCostKind CostKind) const {
...
InstructionCost YSXTTIImpl::getIntImmCostInst(unsigned Opcode, unsigned Idx,
                                                const APInt &Imm, Type *Ty,
                                                TTI::TargetCostKind CostKind,
...
InstructionCost
YSXTTIImpl::getIntImmCostIntrin(Intrinsic::ID IID, unsigned Idx,
                                  const APInt &Imm, Type *Ty,
                                  TTI::TargetCostKind CostKind) const {
...
bool YSXTTIImpl::hasActiveVectorLength() const {
  return ST->hasVInstructions();
}

TargetTransformInfo::PopcntSupportKind
YSXTTIImpl::getPopcntSupport(unsigned TyWidth) const {
  assert(isPowerOf2_32(TyWidth) && "Ty width must be power of 2");
  return ST->hasCPOPLike() ? TTI::PSK_FastHardware : TTI::PSK_Software;
}

InstructionCost YSXTTIImpl::getPartialReductionCost(
  ...
  return LT.first *
         getYSXInstructionCost(YSX::VQDOT_VV, LT.second, CostKind);
}

bool YSXTTIImpl::shouldExpandReduction(const IntrinsicInst *II) const {
...
std::optional<unsigned> YSXTTIImpl::getMaxVScale() const {
  if (ST->hasVInstructions())
    return ST->getRealMaxVLen() / YSX::RVVBitsPerBlock;
  return BaseT::getMaxVScale();
}

std::optional<unsigned> YSXTTIImpl::getVScaleForTuning() const {
  if (ST->hasVInstructions())
    if (unsigned MinVLen = ST->getRealMinVLen();
        MinVLen >= YSX::RVVBitsPerBlock)
      return MinVLen / YSX::RVVBitsPerBlock;
  return BaseT::getVScaleForTuning();
}

TypeSize
YSXTTIImpl::getRegisterBitWidth(TargetTransformInfo::RegisterKind K) const {
  unsigned LMUL =
  ...
  case TargetTransformInfo::RGK_ScalableVector:
    return TypeSize::getScalable(
        (ST->hasVInstructions() &&
         ST->getRealMinVLen() >= YSX::RVVBitsPerBlock)
            ? LMUL * YSX::RVVBitsPerBlock
            : 0);
  }
...
InstructionCost YSXTTIImpl::getStaticDataAddrGenerationCost(
...
InstructionCost
YSXTTIImpl::getConstantPoolLoadCost(Type *Ty,
                                      TTI::TargetCostKind CostKind) const {
...
static VectorType *getVRGatherIndexType(MVT DataVT, const YSXSubtarget &ST,
                                        LLVMContext &C) {
...
static InstructionCost costShuffleViaSplitting(const YSXTTIImpl &TTI,
...
costShuffleViaVRegSplitting(const YSXTTIImpl &TTI, MVT LegalVT,
...
InstructionCost YSXTTIImpl::getSlideCost(FixedVectorType *Tp,
                                           ArrayRef<int> Mask,
                                           TTI::TargetCostKind CostKind) const {
  ...
  auto GetSlideOpcode = [&](int SlideAmt) {
    ...
    if (SlideAmt < 0)
      return IsVI ? YSX::VSLIDEDOWN_VI : YSX::VSLIDEDOWN_VX;
    return IsVI ? YSX::VSLIDEUP_VI : YSX::VSLIDEUP_VX;
  };
  ...
  if (SrcInfo[0].second != 0) {
    ...
    FirstSlideCost = getYSXInstructionCost(Opcode, LT.second, CostKind);
  }
  ...
  if (SrcInfo[1].second != 0) {
    ...
    SecondSlideCost = getYSXInstructionCost(Opcode, LT.second, CostKind);
  } else {
    SecondSlideCost =
        getYSXInstructionCost(YSX::VMERGE_VVM, LT.second, CostKind);
  }
...
InstructionCost
YSXTTIImpl::getShuffleCost(TTI::ShuffleKind Kind, VectorType *DstTy,
  ...
  if (auto *FVTp = dyn_cast<FixedVectorType>(SrcTy);
    ...
    case TTI::SK_PermuteSingleSrc: {
        ...
        if (EltTp.getScalarSizeInBits() < ST->getELen()) {
          ...
          if (Mask[0] == 0 || Mask[0] == 1) {
            ...
            if (equal(DeinterleaveMask, Mask))
              return LT.first * getYSXInstructionCost(YSX::VNSRL_WI,
                                                        LT.second, CostKind);
          }
        }
        int SubVectorSize;
      ...
      if (LT.first == 1 && (LT.second.getScalarSizeInBits() != 8 ||
        ...
        return IndexCost +
               getYSXInstructionCost(YSX::VRGATHER_VV, LT.second, CostKind);
      }
      break;
    }
    ...
    case TTI::SK_PermuteTwoSrc: {
      ...
      if (LT.first == 1 && (LT.second.getScalarSizeInBits() != 8 ||
        ...
        InstructionCost MaskCost = getConstantPoolLoadCost(MaskTy, CostKind);
        return 2 * IndexCost +
               getYSXInstructionCost({YSX::VRGATHER_VV, YSX::VRGATHER_VV},
                                       LT.second, CostKind) +
               MaskCost;
      }
      break;
  ...
  case TTI::SK_ExtractSubvector:
    ...
    return LT.first *
           getYSXInstructionCost(YSX::VSLIDEDOWN_VI, LT.second, CostKind);
  case TTI::SK_InsertSubvector:
    ...
    return LT.first *
           getYSXInstructionCost(YSX::VSLIDEUP_VI, LT.second, CostKind);
  case TTI::SK_Select: {
    // Example sequence:
    // li           a0, 90
    ...
    return LT.first *
           (1 + getYSXInstructionCost({YSX::VMV_S_X, YSX::VMERGE_VVM},
                                        LT.second, CostKind));
  }
  case TTI::SK_Broadcast: {
    ...
    if (LT.second.getScalarSizeInBits() == 1) {
      if (HasScalar) {
        ...
        //   vmsne.vi v0, v8, 0
        return LT.first *
               (1 + getYSXInstructionCost({YSX::VMV_V_X, YSX::VMSNE_VI},
                                            LT.second, CostKind));
      }
      // Example sequence:
      ...
      return LT.first *
             (1 + getYSXInstructionCost({YSX::VMV_V_I, YSX::VMERGE_VIM,
                                           YSX::VMV_X_S, YSX::VMV_V_X,
                                           YSX::VMSNE_VI},
                                          LT.second, CostKind));
    }
    ...
    if (HasScalar) {
      ...
      //   vmv.v.x v8, a0
      return LT.first *
             getYSXInstructionCost(YSX::VMV_V_X, LT.second, CostKind);
    }

    // Example sequence:
    //   vrgather.vi     v9, v8, 0
    return LT.first *
           getYSXInstructionCost(YSX::VRGATHER_VI, LT.second, CostKind);
  }
  case TTI::SK_Splice: {
    // vslidedown+vslideup.
    // TODO: Multiplying by LT.first implies this legalizes into multiple copies
    // of similar code, but I think we expand through memory.
    unsigned Opcodes[2] = {YSX::VSLIDEDOWN_VX, YSX::VSLIDEUP_VX};
    if (Index >= 0 && Index < 32)
      Opcodes[0] = YSX::VSLIDEDOWN_VI;
    else if (Index < 0 && Index > -32)
      Opcodes[1] = YSX::VSLIDEUP_VI;
    return LT.first * getYSXInstructionCost(Opcodes, LT.second, CostKind);
  }
  case TTI::SK_Reverse: {
    ...
    if (ContainerVT.bitsLE(M1VT)) {
      ...
      unsigned Opcodes[] = {YSX::VID_V, YSX::VRSUB_VX, YSX::VRGATHER_VV};
      if (LT.second.isFixedLengthVector() &&
          isInt<5>(LT.second.getVectorNumElements() - 1))
        Opcodes[1] = YSX::VRSUB_VI;
      InstructionCost GatherCost =
          getYSXInstructionCost(Opcodes, LT.second, CostKind);
    ...
    unsigned M1Opcodes[] = {YSX::VID_V, YSX::VRSUB_VX};
    InstructionCost FixedCost =
        getYSXInstructionCost(M1Opcodes, M1VT, CostKind) + 3;
    unsigned Ratio =
        ContainerVT.getVectorMinNumElements() / M1VT.getVectorMinNumElements();
    InstructionCost GatherCost =
        getYSXInstructionCost({YSX::VRGATHER_VV}, M1VT, CostKind) * Ratio;
    InstructionCost SlideCost = !LT.second.isFixedLengthVector() ? 0 :
      getYSXInstructionCost({YSX::VSLIDEDOWN_VX}, LT.second, CostKind);
...
static unsigned isM1OrSmaller(MVT VT) {
  ...
  return (LMUL == YSXVType::VLMUL::LMUL_F8 ||
          LMUL == YSXVType::VLMUL::LMUL_F4 ||
          LMUL == YSXVType::VLMUL::LMUL_F2 ||
          LMUL == YSXVType::VLMUL::LMUL_1);
}

InstructionCost YSXTTIImpl::getScalarizationOverhead(
  ...
  if (Insert && !Extract && LT.first.isValid() && LT.second.isVector()) {
    ...
    if (isM1OrSmaller(ContainerVT)) {
      InstructionCost BV =
          ...
          getYSXInstructionCost(YSX::VSLIDE1DOWN_VX, LT.second, CostKind);
      if (BV < Cost)
        Cost = BV;
    }
  }
...
InstructionCost
YSXTTIImpl::getMemIntrinsicInstrCost(const MemIntrinsicCostAttributes &MICA,
                                       TTI::TargetCostKind CostKind) const {
...
InstructionCost
YSXTTIImpl::getMaskedMemoryOpCost(const MemIntrinsicCostAttributes &MICA,
                                    TTI::TargetCostKind CostKind) const {
...
InstructionCost YSXTTIImpl::getInterleavedMemoryOpCost(
...
InstructionCost
YSXTTIImpl::getGatherScatterOpCost(const MemIntrinsicCostAttributes &MICA,
                                     TTI::TargetCostKind CostKind) const {
...
InstructionCost YSXTTIImpl::getExpandCompressMemoryOpCost(
  ...
  SmallVector<unsigned, 4> Opcodes{YSX::VSETVLI};
  if (VariableMask)
    Opcodes.push_back(YSX::VCPOP_M);
  if (Opcode == Instruction::Store)
    Opcodes.append({YSX::VCOMPRESS_VM});
  else
    Opcodes.append({YSX::VSETIVLI, YSX::VIOTA_M, YSX::VRGATHER_VV});
  return MemOpCost +
         LT.first * getYSXInstructionCost(Opcodes, LT.second, CostKind);
}

InstructionCost
YSXTTIImpl::getStridedMemoryOpCost(const MemIntrinsicCostAttributes &MICA,
                                     TTI::TargetCostKind CostKind) const {
...
InstructionCost
YSXTTIImpl::getCostOfKeepingLiveOverCall(ArrayRef<Type *> Tys) const {
...
InstructionCost
YSXTTIImpl::getIntrinsicInstrCost(const IntrinsicCostAttributes &ICA,
                                    TTI::TargetCostKind CostKind) const {
    ...
    if (ST->hasVInstructions() && LT.second.isVector()) {
      ...
      if (LT.second.getVectorElementType() == MVT::bf16) {
        if (!ST->hasVInstructionsBF16Minimal())
          return InstructionCost::getInvalid();
        if (DstEltSz == 32)
          Ops = {YSX::VFWCVTBF16_F_F_V, YSX::VFCVT_X_F_V};
        else
          Ops = {YSX::VFWCVTBF16_F_F_V, YSX::VFWCVT_X_F_V};
      } else if (LT.second.getVectorElementType() == MVT::f16 &&
                 !ST->hasVInstructionsF16()) {
        if (!ST->hasVInstructionsF16Minimal())
          return InstructionCost::getInvalid();
        if (DstEltSz == 32)
          Ops = {YSX::VFWCVT_F_F_V, YSX::VFCVT_X_F_V};
        else
          Ops = {YSX::VFWCVT_F_F_V, YSX::VFWCVT_X_F_V};

      } else if (SrcEltSz > DstEltSz) {
        Ops = {YSX::VFNCVT_X_F_W};
      } else if (SrcEltSz < DstEltSz) {
        Ops = {YSX::VFWCVT_X_F_V};
      } else {
        Ops = {YSX::VFCVT_X_F_V};
      }
      ...
      if (SrcEltSz > DstEltSz)
        return SrcLT.first *
               getYSXInstructionCost(Ops, SrcLT.second, CostKind);
      return LT.first * getYSXInstructionCost(Ops, LT.second, CostKind);
    }
    break;
  }
  case Intrinsic::ceil:
  ...
  case Intrinsic::smax: {
    ...
    if (ST->hasVInstructions() && LT.second.isVector()) {
      ...
      return LT.first * getYSXInstructionCost(Op, LT.second, CostKind);
    }
    break;
  }
  ...
  case Intrinsic::fmuladd: {
    ...
    if (ST->hasVInstructions() && LT.second.isVector())
      return LT.first *
             getYSXInstructionCost(YSX::VFMADD_VV, LT.second, CostKind);
    break;
  }
  case Intrinsic::fabs: {
    auto LT = getTypeLegalizationCost(RetTy);
    if (ST->hasVInstructions() && LT.second.isVector()) {
      ...
      if (LT.second.getVectorElementType() == MVT::bf16 ||
          (LT.second.getVectorElementType() == MVT::f16 &&
           !ST->hasVInstructionsF16()))
        return LT.first * getYSXInstructionCost(YSX::VAND_VX, LT.second,
                                                  CostKind) +
               2;
      else
        return LT.first *
               getYSXInstructionCost(YSX::VFSGNJX_VV, LT.second, CostKind);
    }
    break;
  }
  case Intrinsic::sqrt: {
    ...
    if (ST->hasVInstructions() && LT.second.isVector()) {
      ...
      if (LT.second.getVectorElementType() == MVT::bf16) {
        if (LT.second == MVT::nxv32bf16) {
          ConvOp = {YSX::VFWCVTBF16_F_F_V, YSX::VFWCVTBF16_F_F_V,
                    YSX::VFNCVTBF16_F_F_W, YSX::VFNCVTBF16_F_F_W};
          FsqrtOp = {YSX::VFSQRT_V, YSX::VFSQRT_V};
          ConvType = MVT::nxv16f16;
          FsqrtType = MVT::nxv16f32;
        } else {
          ConvOp = {YSX::VFWCVTBF16_F_F_V, YSX::VFNCVTBF16_F_F_W};
          FsqrtOp = {YSX::VFSQRT_V};
          FsqrtType = TLI->getTypeToPromoteTo(ISD::FSQRT, FsqrtType);
        }
      } else if (LT.second.getVectorElementType() == MVT::f16 &&
                 !ST->hasVInstructionsF16()) {
        if (LT.second == MVT::nxv32f16) {
          ConvOp = {YSX::VFWCVT_F_F_V, YSX::VFWCVT_F_F_V,
                    YSX::VFNCVT_F_F_W, YSX::VFNCVT_F_F_W};
          FsqrtOp = {YSX::VFSQRT_V, YSX::VFSQRT_V};
          ConvType = MVT::nxv16f16;
          FsqrtType = MVT::nxv16f32;
        } else {
          ConvOp = {YSX::VFWCVT_F_F_V, YSX::VFNCVT_F_F_W};
          FsqrtOp = {YSX::VFSQRT_V};
          FsqrtType = TLI->getTypeToPromoteTo(ISD::FSQRT, FsqrtType);
        }
      } else {
      ...
      return LT.first * (getYSXInstructionCost(FsqrtOp, FsqrtType, CostKind) +
                         getYSXInstructionCost(ConvOp, ConvType, CostKind));
    }
    break;
  }
  case Intrinsic::cttz:
  ...
  case Intrinsic::abs: {
    ...
    if (ST->hasVInstructions() && LT.second.isVector()) {
      ...
      // vmax.vv v8, v8, v10
      return LT.first *
             getYSXInstructionCost({YSX::VRSUB_VI, YSX::VMAX_VV},
                                     LT.second, CostKind);
    }
    break;
  }
  ...
  case Intrinsic::get_active_lane_mask: {
    if (ST->hasVInstructions()) {
      ...
      // vmsltu.vx   v0, v8, a1
      return LT.first *
             getYSXInstructionCost({YSX::VSADDU_VX, YSX::VMSLTU_VX},
                                     LT.second, CostKind);
    }
    break;
  }
  ...
  case Intrinsic::stepvector: {
    ...
    if (ST->hasVInstructions())
      return getYSXInstructionCost(YSX::VID_V, LT.second, CostKind) +
             (LT.first - 1) *
                 getYSXInstructionCost(YSX::VADD_VX, LT.second, CostKind);
    return 1 + (LT.first - 1);
  }
  case Intrinsic::experimental_cttz_elts: {
    ...
    if (getTLI()->shouldExpandCttzElements(ArgType))
      break;
    InstructionCost Cost = getYSXInstructionCost(
        YSX::VFIRST_M, getTypeLegalizationCost(ArgTy).second, CostKind);
...
InstructionCost
YSXTTIImpl::getAddressComputationCost(Type *PtrTy, ScalarEvolution *SE,
                                        const SCEV *Ptr,
                                        TTI::TargetCostKind CostKind) const {
...
InstructionCost YSXTTIImpl::getCastInstrCost(unsigned Opcode, Type *Dst,
                                               Type *Src,
  ...
  case ISD::ZERO_EXTEND:
    if (Src->getScalarSizeInBits() == 1) {
      ...
      return getYSXInstructionCost(YSX::VMV_V_I, DstLT.second, CostKind) +
             DstLT.first * getYSXInstructionCost(YSX::VMERGE_VIM,
  ...
  case ISD::TRUNCATE:
    if (Dst->getScalarSizeInBits() == 1) {
      ...
      // vmsne.vi v0, v8, 0
      return SrcLT.first *
                 getYSXInstructionCost({YSX::VAND_VI, YSX::VMSNE_VI},
  ...
  case ISD::ZERO_EXTEND: {
    if ((PowDiff < 1) || (PowDiff > 3))
      return BaseT::getCastInstrCost(Opcode, Dst, Src, CCH, CostKind, I);
    unsigned SExtOp[] = {YSX::VSEXT_VF2, YSX::VSEXT_VF4, YSX::VSEXT_VF8};
    unsigned ZExtOp[] = {YSX::VZEXT_VF2, YSX::VZEXT_VF4, YSX::VZEXT_VF8};
    unsigned Op =
        (ISD == ISD::SIGN_EXTEND) ? SExtOp[PowDiff - 1] : ZExtOp[PowDiff - 1];
    return getYSXInstructionCost(Op, DstLT.second, CostKind);
  }
  case ISD::TRUNCATE:
  case ISD::FP_EXTEND:
  case ISD::FP_ROUND: {
    ...
    unsigned Op = (ISD == ISD::TRUNCATE)    ? YSX::VNSRL_WI
                  : (ISD == ISD::FP_EXTEND) ? YSX::VFWCVT_F_F_V
                                            : YSX::VFNCVT_F_F_W;
    InstructionCost Cost = 0;
    for (; SrcEltSize != DstEltSize;) {
      ...
      Cost += getYSXInstructionCost(Op, DstMVT, CostKind);
    }
    return Cost;
  }
  case ISD::FP_TO_SINT:
  case ISD::FP_TO_UINT: {
    ...
    unsigned FCVT = IsSigned ? YSX::VFCVT_RTZ_X_F_V : YSX::VFCVT_RTZ_XU_F_V;
    unsigned FWCVT =
        IsSigned ? YSX::VFWCVT_RTZ_X_F_V : YSX::VFWCVT_RTZ_XU_F_V;
    unsigned FNCVT =
        IsSigned ? YSX::VFNCVT_RTZ_X_F_W : YSX::VFNCVT_RTZ_XU_F_W;
    ...
    if ((SrcEltSize == 16) &&
      ...
      Cost +=
          VecF32LT.first * getYSXInstructionCost(YSX::VFWCVT_F_F_V,
                                                   VecF32LT.second, CostKind);
      Cost += getCastInstrCost(Opcode, Dst, VecF32Ty, CCH, CostKind, I);
      return Cost;
    }
    if (DstEltSize == SrcEltSize)
      Cost += getYSXInstructionCost(FCVT, DstLT.second, CostKind);
    else if (DstEltSize > SrcEltSize)
      Cost += getYSXInstructionCost(FWCVT, DstLT.second, CostKind);
      ...
      Cost += getYSXInstructionCost(FNCVT, VecVT, CostKind);
  ...
  case ISD::UINT_TO_FP: {
    ...
    unsigned FCVT = IsSigned ? YSX::VFCVT_F_X_V : YSX::VFCVT_F_XU_V;
    unsigned FWCVT = IsSigned ? YSX::VFWCVT_F_X_V : YSX::VFWCVT_F_XU_V;
    unsigned FNCVT = IsSigned ? YSX::VFNCVT_F_X_W : YSX::VFNCVT_F_XU_W;
    unsigned SrcEltSize = Src->getScalarSizeInBits();
    unsigned DstEltSize = Dst->getScalarSizeInBits();
    ...
    if ((DstEltSize == 16) &&
      ...
      Cost += VecF32LT.first * getYSXInstructionCost(YSX::VFNCVT_F_F_W,
                                                       DstLT.second, CostKind);
      return Cost;
    }
    ...
    if (DstEltSize == SrcEltSize)
      ...
      Cost += getYSXInstructionCost(FNCVT, DstLT.second, CostKind);
    return Cost;
  }
  }
  return BaseT::getCastInstrCost(Opcode, Dst, Src, CCH, CostKind, I);
...
unsigned YSXTTIImpl::getEstimatedVLFor(VectorType *Ty) const {
  if (isa<ScalableVectorType>(Ty)) {
    const unsigned EltSize = DL.getTypeSizeInBits(Ty->getElementType());
    const unsigned MinSize = DL.getTypeSizeInBits(Ty).getKnownMinValue();
    const unsigned VectorBits = *getVScaleForTuning() * YSX::RVVBitsPerBlock;
    return YSXTargetLowering::computeVLMAX(VectorBits, EltSize, MinSize);
  }
  return cast<FixedVectorType>(Ty)->getNumElements();
}

InstructionCost
YSXTTIImpl::getMinMaxReductionCost(Intrinsic::ID IID, VectorType *Ty,
                                     FastMathFlags FMF,
                                     TTI::TargetCostKind CostKind) const {
  ...
  if (IID == Intrinsic::maximum || IID == Intrinsic::minimum) {
    ...
    case Intrinsic::maximum:
      if (FMF.noNaNs()) {
        Opcodes = {YSX::VFREDMAX_VS, YSX::VFMV_F_S};
      } else {
        Opcodes = {YSX::VMFNE_VV, YSX::VCPOP_M, YSX::VFREDMAX_VS,
                   YSX::VFMV_F_S};
    ...
    case Intrinsic::minimum:
      if (FMF.noNaNs()) {
        Opcodes = {YSX::VFREDMIN_VS, YSX::VFMV_F_S};
      } else {
        Opcodes = {YSX::VMFNE_VV, YSX::VCPOP_M, YSX::VFREDMIN_VS,
                   YSX::VFMV_F_S};
        // Cost of Canonical Nan + branch
        // lui a0, 523264
        // fmv.w.x fa0, a0
    ...
    return ExtraCost + getYSXInstructionCost(Opcodes, LT.second, CostKind);
  }
  ...
  case Intrinsic::smax:
    SplitOp = YSX::VMAX_VV;
    Opcodes = {YSX::VREDMAX_VS, YSX::VMV_X_S};
    break;
  case Intrinsic::smin:
    SplitOp = YSX::VMIN_VV;
    Opcodes = {YSX::VREDMIN_VS, YSX::VMV_X_S};
    break;
  case Intrinsic::umax:
    SplitOp = YSX::VMAXU_VV;
    Opcodes = {YSX::VREDMAXU_VS, YSX::VMV_X_S};
    break;
  case Intrinsic::umin:
    SplitOp = YSX::VMINU_VV;
    Opcodes = {YSX::VREDMINU_VS, YSX::VMV_X_S};
    break;
  case Intrinsic::maxnum:
    SplitOp = YSX::VFMAX_VV;
    Opcodes = {YSX::VFREDMAX_VS, YSX::VFMV_F_S};
    break;
  case Intrinsic::minnum:
    SplitOp = YSX::VFMIN_VV;
    Opcodes = {YSX::VFREDMIN_VS, YSX::VFMV_F_S};
    break;
  }
  ...
  InstructionCost SplitCost =
      (LT.first > 1) ? (LT.first - 1) *
                           getYSXInstructionCost(SplitOp, LT.second, CostKind)
                     : 0;
  return SplitCost + getYSXInstructionCost(Opcodes, LT.second, CostKind);
}

InstructionCost
YSXTTIImpl::getArithmeticReductionCost(unsigned Opcode, VectorType *Ty,
  ...
  if (ElementTy->isIntegerTy(1)) {
    ...
    if (LT.second == MVT::v1i1)
      return getYSXInstructionCost(YSX::VFIRST_M, LT.second, CostKind) +
    ...
    if (ISD == ISD::AND) {
      ...
      return ((LT.first > 2) ? (LT.first - 2) : 0) *
                 getYSXInstructionCost(YSX::VMAND_MM, LT.second, CostKind) +
             getYSXInstructionCost(YSX::VMNAND_MM, LT.second, CostKind) +
             getYSXInstructionCost(YSX::VCPOP_M, LT.second, CostKind) +
             getCmpSelInstrCost(Instruction::ICmp, ElementTy, ElementTy,
                                CmpInst::ICMP_EQ, CostKind);
      ...
      //   andi a0, a0, 1
      return (LT.first - 1) *
                 getYSXInstructionCost(YSX::VMXOR_MM, LT.second, CostKind) +
             getYSXInstructionCost(YSX::VCPOP_M, LT.second, CostKind) + 1;
    } else {
      assert(ISD == ISD::OR);
      // Example sequences:
      //   vsetvli a0, zero, e8, mf8, ta, ma
      ...
      //   snez a0, a0
      return (LT.first - 1) *
                 getYSXInstructionCost(YSX::VMOR_MM, LT.second, CostKind) +
             getYSXInstructionCost(YSX::VCPOP_M, LT.second, CostKind) +
             getCmpSelInstrCost(Instruction::ICmp, ElementTy, ElementTy,
  ...
  case ISD::ADD:
    SplitOp = YSX::VADD_VV;
    Opcodes = {YSX::VMV_S_X, YSX::VREDSUM_VS, YSX::VMV_X_S};
    break;
  case ISD::OR:
    SplitOp = YSX::VOR_VV;
    Opcodes = {YSX::VREDOR_VS, YSX::VMV_X_S};
    break;
  case ISD::XOR:
    SplitOp = YSX::VXOR_VV;
    Opcodes = {YSX::VMV_S_X, YSX::VREDXOR_VS, YSX::VMV_X_S};
    break;
  case ISD::AND:
    SplitOp = YSX::VAND_VV;
    Opcodes = {YSX::VREDAND_VS, YSX::VMV_X_S};
    break;
  case ISD::FADD:
    ...
    if (TTI::requiresOrderedReduction(FMF)) {
      Opcodes.push_back(YSX::VFMV_S_F);
      for (unsigned i = 0; i < LT.first.getValue(); i++)
        Opcodes.push_back(YSX::VFREDOSUM_VS);
      Opcodes.push_back(YSX::VFMV_F_S);
      return getYSXInstructionCost(Opcodes, LT.second, CostKind);
    }
    SplitOp = YSX::VFADD_VV;
    Opcodes = {YSX::VFMV_S_F, YSX::VFREDUSUM_VS, YSX::VFMV_F_S};
    break;
  }
  // Add a cost for data larger than LMUL8
  InstructionCost SplitCost =
...
InstructionCost YSXTTIImpl::getExtendedReductionCost(
...
InstructionCost
YSXTTIImpl::getStoreImmCost(Type *Ty, TTI::OperandValueInfo OpInfo,
                              TTI::TargetCostKind CostKind) const {
...
InstructionCost YSXTTIImpl::getMemoryOpCost(unsigned Opcode, Type *Src,
                                              Align Alignment,
...
InstructionCost YSXTTIImpl::getCmpSelInstrCost(
  ...
  if (Opcode == Instruction::Select && ValTy->isVectorTy()) {
    if (CondTy->isVectorTy()) {
      if (ValTy->getScalarSizeInBits() == 1) {
        ...
        return ConstantMatCost +
               LT.first *
                   getYSXInstructionCost(
                       {YSX::VMANDN_MM, YSX::VMAND_MM, YSX::VMOR_MM},
                       LT.second, CostKind);
      }
      // vselect and max/min are supported natively.
      return ConstantMatCost +
             LT.first * getYSXInstructionCost(YSX::VMERGE_VVM, LT.second,
                                                CostKind);
    }
    ...
    if (ValTy->getScalarSizeInBits() == 1) {
      ...
      return ConstantMatCost +
             LT.first *
                 getYSXInstructionCost({YSX::VMV_V_X, YSX::VMSNE_VI},
                                         InterimVT, CostKind) +
             LT.first * getYSXInstructionCost(
                            {YSX::VMANDN_MM, YSX::VMAND_MM, YSX::VMOR_MM},
                            LT.second, CostKind);
    }
    ...
    return ConstantMatCost +
           LT.first * getYSXInstructionCost(
                          {YSX::VMV_V_X, YSX::VMSNE_VI, YSX::VMERGE_VVM},
                          LT.second, CostKind);
  }
  ...
  if ((Opcode == Instruction::ICmp) && ValTy->isVectorTy() &&
    ...
    return ConstantMatCost + LT.first * getYSXInstructionCost(YSX::VMSLT_VV,
                                                                LT.second,
                                                                CostKind);
  }
  ...
  if ((Opcode == Instruction::FCmp) && ValTy->isVectorTy() &&
    ...
    case CmpInst::FCMP_UEQ: // vmflt.vv + vmflt.vv + vmnor.mm
      return ConstantMatCost +
             LT.first * getYSXInstructionCost(
                            {YSX::VMFLT_VV, YSX::VMFLT_VV, YSX::VMOR_MM},
                            LT.second, CostKind);
    ...
    case CmpInst::FCMP_ULE: // vmflt.vv + vmnot.m
      return ConstantMatCost +
             LT.first *
                 getYSXInstructionCost({YSX::VMFLT_VV, YSX::VMNAND_MM},
                                         LT.second, CostKind);
    ...
    case CmpInst::FCMP_UNE: // vmfne.vv
      return ConstantMatCost +
             LT.first *
                 getYSXInstructionCost(YSX::VMFLT_VV, LT.second, CostKind);
    default:
      break;
    }
  }
...
InstructionCost YSXTTIImpl::getCFInstrCost(unsigned Opcode,
                                             TTI::TargetCostKind CostKind,
                                             const Instruction *I) const {
  if (CostKind != TTI::TCK_RecipThroughput)
...
InstructionCost YSXTTIImpl::getVectorInstrCost(unsigned Opcode, Type *Val,
                                                 TTI::TargetCostKind CostKind,
                                                 unsigned Index,
                                                 const Value *Op0,
...
InstructionCost
YSXTTIImpl::getIndexedVectorInstrCostFromEnd(unsigned Opcode, Type *Val,
                                               TTI::TargetCostKind CostKind,
...
InstructionCost YSXTTIImpl::getArithmeticInstrCost(
  ...
  case ISD::OR:
  case ISD::XOR:
    Op = (Ty->getScalarSizeInBits() == 1) ? YSX::VMAND_MM : YSX::VAND_VV;
    break;
  case ISD::MUL:
  case ISD::MULHS:
  case ISD::MULHU:
  ...
  InstructionCost InstrCost = getYSXInstructionCost(Op, LT.second, CostKind);
...
InstructionCost YSXTTIImpl::getPointersChainCost(
...
void YSXTTIImpl::getUnrollingPreferences(
...
void YSXTTIImpl::getPeelingPreferences(Loop *L, ScalarEvolution &SE,
                                         TTI::PeelingPreferences &PP) const {
  BaseT::getPeelingPreferences(L, SE, PP);
}

bool YSXTTIImpl::getTgtMemIntrinsic(IntrinsicInst *Inst,
  ...
  case Intrinsic::riscv_vsseg8: {
    // Intrinsic interface:
    // ysx_vle(merge, ptr, vl)
    // ysx_vle_mask(merge, ptr, mask, vl, policy)
    // ysx_vse(val, ptr, vl)
    // ysx_vse_mask(val, ptr, mask, vl, policy)
    // ysx_vlseg#(merge, ptr, vl, sew)
    // ysx_vlseg#_mask(merge, ptr, mask, vl, policy, sew)
    // ysx_vsseg#(val, ptr, vl, sew)
    // ysx_vsseg#_mask(val, ptr, mask, vl, sew)
    ...
    const auto *RVVIInfo = YSXVIntrinsicsTable::getYSXVIntrinsicInfo(IID);
  ...
  case Intrinsic::riscv_vssseg8: {
    // Intrinsic interface:
    // ysx_vlse(merge, ptr, stride, vl)
    // ysx_vlse_mask(merge, ptr, stride, mask, vl, policy)
    // ysx_vsse(val, ptr, stride, vl)
    // ysx_vsse_mask(val, ptr, stride, mask, vl, policy)
    // ysx_vlsseg#(merge, ptr, offset, vl, sew)
    // ysx_vlsseg#_mask(merge, ptr, offset, mask, vl, policy, sew)
    // ysx_vssseg#(val, ptr, offset, vl, sew)
    // ysx_vssseg#_mask(val, ptr, offset, mask, vl, sew)
    bool IsWrite = Inst->getType()->isVoidTy();
    Type *Ty = IsWrite ? Inst->getArgOperand(0)->getType() : Inst->getType();
    // The results of segment loads are TargetExtType.
    if (auto *TarExtTy = dyn_cast<TargetExtType>(Ty)) {
  ...
  case Intrinsic::riscv_vsuxseg8: {
    ...
    // ysx_vloxei(merge, ptr, index, vl)
    // ysx_vloxei_mask(merge, ptr, index, mask, vl, policy)
    // ysx_vsoxei(val, ptr, index, vl)
    // ysx_vsoxei_mask(val, ptr, index, mask, vl, policy)
    // ysx_vloxseg#(merge, ptr, index, vl, sew)
    // ysx_vloxseg#_mask(merge, ptr, index, mask, vl, policy, sew)
    // ysx_vsoxseg#(val, ptr, index, vl, sew)
    // ysx_vsoxseg#_mask(val, ptr, index, mask, vl, sew)
    bool IsWrite = Inst->getType()->isVoidTy();
    Type *Ty = IsWrite ? Inst->getArgOperand(0)->getType() : Inst->getType();
    // The results of segment loads are TargetExtType.
    if (auto *TarExtTy = dyn_cast<TargetExtType>(Ty)) {
...
unsigned YSXTTIImpl::getRegUsageForType(Type *Ty) const {
  if (Ty->isVectorTy()) {
    ...
    if (Size.isScalable() && ST->hasVInstructions())
      return divideCeil(Size.getKnownMinValue(), YSX::RVVBitsPerBlock);
...
unsigned YSXTTIImpl::getMaximumVF(unsigned ElemWidth, unsigned Opcode) const {
...
unsigned YSXTTIImpl::getMinTripCountTailFoldingThreshold() const {
  return RVVMinTripCount;
}

bool YSXTTIImpl::preferAlternateOpcodeVectorization() const {
  return ST->enableUnalignedVectorMem();
}

TTI::AddressingModeKind
YSXTTIImpl::getPreferredAddressingMode(const Loop *L,
...
bool YSXTTIImpl::isLSRCostLess(const TargetTransformInfo::LSRCost &C1,
...
bool YSXTTIImpl::isLegalMaskedExpandLoad(Type *DataTy,
...
bool YSXTTIImpl::isLegalMaskedCompressStore(Type *DataTy,
                                              Align Alignment) const {
  auto *VTy = dyn_cast<VectorType>(DataTy);
  if (!VTy || VTy->isScalableTy())
    return false;
...
bool YSXTTIImpl::shouldConsiderAddressTypePromotion(
...
bool YSXTTIImpl::canSplatOperand(unsigned Opcode, int Operand) const {
  switch (Opcode) {
  case Instruction::Add:
  case Instruction::Sub:
  case Instruction::Mul:
...
bool YSXTTIImpl::canSplatOperand(Instruction *I, int Operand) const {
...
bool YSXTTIImpl::isProfitableToSinkOperands(
...
YSXTTIImpl::TTI::MemCmpExpansionOptions
YSXTTIImpl::enableMemCmpExpansion(bool OptSize, bool IsZeroCmp) const {
