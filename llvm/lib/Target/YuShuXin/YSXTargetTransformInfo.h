//===- YSXTargetTransformInfo.h - RISC-V specific TTI ---------*- C++ -*-===//
//
...
class YSXTTIImpl final : public BasicTTIImplBase<YSXTTIImpl> {
  ...
  InstructionCost getYSXInstructionCost(ArrayRef<unsigned> OpCodes, MVT VT,
...
public:
  ...
  enum YSXRegisterClass { GPRRC, FPRRC, VRRC };
  unsigned getNumberOfRegisters(unsigned ClassID) const override {
    switch (ClassID) {
    case YSXRegisterClass::GPRRC:
      // 31 = 32 GPR - x0 (zero register)
      // FIXME: Should we exclude fixed registers like SP, TP or GP?
      return 31;
    case YSXRegisterClass::FPRRC:
      if (ST->hasStdExtF())
        return 32;
      return 0;
    case YSXRegisterClass::VRRC:
