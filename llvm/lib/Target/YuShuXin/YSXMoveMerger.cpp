//===-- YSXMoveMerger.cpp - RISC-V move merge pass ----------------------===//
//
...
#define YSX_MOVE_MERGE_NAME "RISC-V Zcmp move merging pass"

namespace {
struct YSXMoveMerge : public MachineFunctionPass {
  static char ID;

  YSXMoveMerge() : MachineFunctionPass(ID) {}
  ...
  StringRef getPassName() const override { return YSX_MOVE_MERGE_NAME; }
};

char YSXMoveMerge::ID = 0;

} // end of anonymous namespace

INITIALIZE_PASS(YSXMoveMerge, "ysx-move-merge", YSX_MOVE_MERGE_NAME,
                false, false)

static unsigned getMoveFromSToAOpcode(const YSXSubtarget &ST) {
  if (ST.hasStdExtZcmp())
    return YSX::CM_MVA01S;

  if (ST.hasVendorXqccmp())
    return YSX::QC_CM_MVA01S;

  llvm_unreachable("Unhandled subtarget with paired A to S move.");
}

static unsigned getMoveFromAToSOpcode(const YSXSubtarget &ST) {
  if (ST.hasStdExtZcmp())
    return YSX::CM_MVSA01;

  if (ST.hasVendorXqccmp())
    return YSX::QC_CM_MVSA01;

  llvm_unreachable("Unhandled subtarget with paired S to A move");
}

// Check if registers meet CM.MVA01S constraints.
bool YSXMoveMerge::isCandidateToMergeMVA01S(const DestSourcePair &RegPair) {
  Register Destination = RegPair.Destination->getReg();
  Register Source = RegPair.Source->getReg();
  // If destination is not a0 or a1.
  if ((Destination == YSX::X10 || Destination == YSX::X11) &&
      YSX::SR07RegClass.contains(Source))
    return true;
  return false;
}

// Check if registers meet CM.MVSA01 constraints.
bool YSXMoveMerge::isCandidateToMergeMVSA01(const DestSourcePair &RegPair) {
  Register Destination = RegPair.Destination->getReg();
  Register Source = RegPair.Source->getReg();
  // If Source is s0 - s7.
  if ((Source == YSX::X10 || Source == YSX::X11) &&
      YSX::SR07RegClass.contains(Destination))
    return true;
  return false;
}

MachineBasicBlock::iterator
YSXMoveMerge::mergePairedInsns(MachineBasicBlock::iterator I,
...
MachineBasicBlock::iterator
YSXMoveMerge::findMatchingInst(MachineBasicBlock::iterator &MBBI,
                                 bool MoveFromSToA,
                                 const DestSourcePair &RegPair) {
...
bool YSXMoveMerge::mergeMoveSARegPair(MachineBasicBlock &MBB) {
  bool Modified = false;
...
bool YSXMoveMerge::runOnMachineFunction(MachineFunction &Fn) {
...
/// createYSXMoveMergePass - returns an instance of the
/// move merge pass.
FunctionPass *llvm::createYSXMoveMergePass() { return new YSXMoveMerge(); }
