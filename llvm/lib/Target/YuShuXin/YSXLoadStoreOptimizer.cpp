...
#define DEBUG_TYPE "ysx-load-store-opt"
#define YSX_LOAD_STORE_OPT_NAME "RISC-V Load / Store Optimizer"

// The LdStLimit limits number of instructions how far we search for load/store
// pairs.
static cl::opt<unsigned> LdStLimit("ysx-load-store-scan-limit", cl::init(128),
                                   cl::Hidden);
...
struct YSXLoadStoreOpt : public MachineFunctionPass {
  static char ID;
  bool runOnMachineFunction(MachineFunction &Fn) override;

  YSXLoadStoreOpt() : MachineFunctionPass(ID) {}
  ...
  StringRef getPassName() const override { return YSX_LOAD_STORE_OPT_NAME; }
...
private:
  ...
  const YSXSubtarget *STI = nullptr;
  LiveRegUnits ModifiedRegUnits, UsedRegUnits;
};
} // end anonymous namespace

char YSXLoadStoreOpt::ID = 0;
INITIALIZE_PASS(YSXLoadStoreOpt, DEBUG_TYPE, YSX_LOAD_STORE_OPT_NAME, false,
                false)

bool YSXLoadStoreOpt::runOnMachineFunction(MachineFunction &Fn) {
  if (skipFunction(Fn.getFunction()))
    return false;

  bool MadeChange = false;
  STI = &Fn.getSubtarget<YSXSubtarget>();
...
// instruction.
bool YSXLoadStoreOpt::tryToPairLdStInst(MachineBasicBlock::iterator &MBBI) {
...
bool YSXLoadStoreOpt::tryConvertToXqcilsmMultiLdSt(
...
bool YSXLoadStoreOpt::tryConvertToXqcilsmLdStPair(
    MachineFunction *MF, MachineBasicBlock::iterator First,
    MachineBasicBlock::iterator Second) {
  unsigned Opc = First->getOpcode();
  if ((Opc != YSX::LW && Opc != YSX::SW) || Second->getOpcode() != Opc)
    return false;
  ...
  if (Opc == YSX::LW) {
    ...
    assert(Opc == YSX::SW && "Expected a SW instruction");
    if (StartReg == NextReg) {
      XqciOpc = YSX::QC_SETWMI;
      StartRegState = getKillRegState(FirstOp0.isKill() || SecondOp0.isKill());
      AddNextReg = false;
    } else if (NextReg == StartReg + 1 && StartReg != YSX::X0) {
      XqciOpc = YSX::QC_SWMI;
...
bool YSXLoadStoreOpt::tryConvertToMIPSLdStPair(
    MachineFunction *MF, MachineBasicBlock::iterator First,
    MachineBasicBlock::iterator Second) {
  ...
  case YSX::SW:
    PairOpc = YSX::MIPS_SWP;
    RequiredAlignment = Align(8);
    break;
  case YSX::LW:
    PairOpc = YSX::MIPS_LWP;
    RequiredAlignment = Align(8);
    break;
  case YSX::SD:
    PairOpc = YSX::MIPS_SDP;
    RequiredAlignment = Align(16);
    break;
  case YSX::LD:
    PairOpc = YSX::MIPS_LDP;
    RequiredAlignment = Align(16);
    break;
  }
...
bool YSXLoadStoreOpt::tryConvertToLdStPair(
...
MachineBasicBlock::iterator
YSXLoadStoreOpt::findMatchingInsn(MachineBasicBlock::iterator I,
                                    bool &MergeForward) {
...
MachineBasicBlock::iterator
YSXLoadStoreOpt::mergePairedInsns(MachineBasicBlock::iterator I,
...
bool YSXLoadStoreOpt::isValidZilsdRegPair(Register First, Register Second) {
...
void YSXLoadStoreOpt::splitLdSdIntoTwo(MachineBasicBlock &MBB,
                                         MachineBasicBlock::iterator &MBBI,
                                         bool IsLoad) {
  MachineInstr *MI = &*MBBI;
  ...
  unsigned Opc = IsLoad ? YSX::LW : YSX::SW;
...
bool YSXLoadStoreOpt::fixInvalidRegPairOp(MachineBasicBlock &MBB,
                                            MachineBasicBlock::iterator &MBBI) {
  MachineInstr *MI = &*MBBI;
  unsigned Opcode = MI->getOpcode();

  // Check if this is a Zilsd pseudo that needs fixing
  if (Opcode != YSX::PseudoLD_RV32_OPT && Opcode != YSX::PseudoSD_RV32_OPT)
    return false;

  bool IsLoad = Opcode == YSX::PseudoLD_RV32_OPT;
  ...
  unsigned RealOpc = IsLoad ? YSX::LD_RV32 : YSX::SD_RV32;

  // Create register pair from the two individual registers
  unsigned RegPair = TRI->getMatchingSuperReg(FirstReg, YSX::sub_gpr_even,
                                              &YSX::GPRPairRegClass);
...
FunctionPass *llvm::createYSXLoadStoreOptPass() {
  return new YSXLoadStoreOpt();
}
