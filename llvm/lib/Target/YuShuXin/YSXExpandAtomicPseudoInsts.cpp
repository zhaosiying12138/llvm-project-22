//===-- YSXExpandAtomicPseudoInsts.cpp - Expand atomic pseudo instrs. ---===//
//
...
#define YSX_EXPAND_ATOMIC_PSEUDO_NAME                                        \
  "RISC-V atomic pseudo instruction expansion pass"

namespace {

class YSXExpandAtomicPseudo : public MachineFunctionPass {
public:
  const YSXSubtarget *STI;
  const YSXInstrInfo *TII;
  static char ID;

  YSXExpandAtomicPseudo() : MachineFunctionPass(ID) {}

  bool runOnMachineFunction(MachineFunction &MF) override;

  StringRef getPassName() const override {
    return YSX_EXPAND_ATOMIC_PSEUDO_NAME;
  }
...
char YSXExpandAtomicPseudo::ID = 0;

bool YSXExpandAtomicPseudo::runOnMachineFunction(MachineFunction &MF) {
...
bool YSXExpandAtomicPseudo::expandMBB(MachineBasicBlock &MBB) {
  bool Modified = false;
...
bool YSXExpandAtomicPseudo::expandMI(MachineBasicBlock &MBB,
                                       MachineBasicBlock::iterator MBBI,
                                       MachineBasicBlock::iterator &NextMBBI) {
...
static void doAtomicBinOpExpansion(const YSXInstrInfo *TII, MachineInstr &MI,
  ...
  case AtomicRMWInst::Xchg:
    BuildMI(LoopMBB, DL, TII->get(YSX::ADDI), ScratchReg)
        .addReg(IncrReg)
        .addImm(0);
    break;
  case AtomicRMWInst::Add:
    BuildMI(LoopMBB, DL, TII->get(YSX::ADD), ScratchReg)
        .addReg(DestReg)
        .addReg(IncrReg);
    break;
  case AtomicRMWInst::Sub:
    BuildMI(LoopMBB, DL, TII->get(YSX::SUB), ScratchReg)
        .addReg(DestReg)
        .addReg(IncrReg);
    break;
  case AtomicRMWInst::And:
    BuildMI(LoopMBB, DL, TII->get(YSX::AND), ScratchReg)
        .addReg(DestReg)
        .addReg(IncrReg);
    break;
  case AtomicRMWInst::Or:
    BuildMI(LoopMBB, DL, TII->get(YSX::OR), ScratchReg)
        .addReg(DestReg)
        .addReg(IncrReg);
    break;
  case AtomicRMWInst::Xor:
    BuildMI(LoopMBB, DL, TII->get(YSX::XOR), ScratchReg)
        .addReg(DestReg)
        .addReg(IncrReg);
    break;
  case AtomicRMWInst::Nand:
    BuildMI(LoopMBB, DL, TII->get(YSX::AND), ScratchReg)
        .addReg(DestReg)
        .addReg(IncrReg);
    BuildMI(LoopMBB, DL, TII->get(YSX::XORI), ScratchReg)
        .addReg(ScratchReg)
        .addImm(-1);
    break;
  }
...
static void insertMaskedMerge(const YSXInstrInfo *TII, DebugLoc DL,
  ...
  BuildMI(MBB, DL, TII->get(YSX::XOR), ScratchReg)
      .addReg(OldValReg)
      .addReg(NewValReg);
  BuildMI(MBB, DL, TII->get(YSX::AND), ScratchReg)
      .addReg(ScratchReg)
      .addReg(MaskReg);
  BuildMI(MBB, DL, TII->get(YSX::XOR), DestReg)
      .addReg(OldValReg)
      .addReg(ScratchReg);
}

static void doMaskedAtomicBinOpExpansion(const YSXInstrInfo *TII,
...
bool YSXExpandAtomicPseudo::expandAtomicBinOp(
...
static void insertSext(const YSXInstrInfo *TII, DebugLoc DL,
                       MachineBasicBlock *MBB, Register ValReg,
                       Register ShamtReg) {
  BuildMI(MBB, DL, TII->get(YSX::SLL), ValReg)
      .addReg(ValReg)
      .addReg(ShamtReg);
  BuildMI(MBB, DL, TII->get(YSX::SRA), ValReg)
      .addReg(ValReg)
      .addReg(ShamtReg);
}

static void doAtomicMinMaxOpExpansion(
    const YSXInstrInfo *TII, MachineInstr &MI, DebugLoc DL,
  ...
  BuildMI(LoopHeadMBB, DL, TII->get(getLRForRMW(Ordering, Width, STI)), DestReg)
      .addReg(AddrReg);
  BuildMI(LoopHeadMBB, DL, TII->get(YSX::ADDI), ScratchReg)
      .addReg(DestReg)
      .addImm(0);
  switch (BinOp) {
  default:
  ...
  case AtomicRMWInst::Max: {
    BuildMI(LoopHeadMBB, DL, TII->get(YSX::BGE))
        .addReg(ScratchReg)
        .addReg(IncrReg)
        .addMBB(LoopTailMBB);
    break;
  ...
  case AtomicRMWInst::UMax:
    BuildMI(LoopHeadMBB, DL, TII->get(YSX::BGEU))
        .addReg(ScratchReg)
        .addReg(IncrReg)
        .addMBB(LoopTailMBB);
    break;
  ...
  // .loopifbody:
  //   mv scratch, incr
  BuildMI(LoopIfBodyMBB, DL, TII->get(YSX::ADDI), ScratchReg)
      .addReg(IncrReg)
      .addImm(0);
  ...
  BuildMI(LoopTailMBB, DL, TII->get(getSCForRMW(Ordering, Width, STI)),
      ...
      .addReg(ScratchReg)
      .addReg(AddrReg);
  BuildMI(LoopTailMBB, DL, TII->get(YSX::BNE))
      .addReg(ScratchReg)
      .addReg(YSX::X0)
      .addMBB(LoopHeadMBB);
}
...
static void doMaskedAtomicMinMaxOpExpansion(
  ...
  BuildMI(LoopHeadMBB, DL, TII->get(getLRForRMW32(Ordering, STI)), DestReg)
      .addReg(AddrReg);
  BuildMI(LoopHeadMBB, DL, TII->get(YSX::AND), Scratch2Reg)
      .addReg(DestReg)
      .addReg(MaskReg);
  BuildMI(LoopHeadMBB, DL, TII->get(YSX::ADDI), Scratch1Reg)
      .addReg(DestReg)
      .addImm(0);
...
bool YSXExpandAtomicPseudo::expandAtomicMinMaxOp(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MBBI,
    AtomicRMWInst::BinOp BinOp, bool IsMasked, int Width,
    MachineBasicBlock::iterator &NextMBBI) {
  MachineInstr &MI = *MBBI;
...
bool tryToFoldBNEOnCmpXchgResult(MachineBasicBlock &MBB,
  ...
  if (MaskReg.isValid()) {
    if (MBBI == E || MBBI->getOpcode() != YSX::AND)
      return false;
  ...
  if (MBBI == E || MBBI->getOpcode() != YSX::BNE)
    return false;
...
bool YSXExpandAtomicPseudo::expandAtomicCmpXchg(
  ...
  if (!IsMasked) {
    ...
    BuildMI(LoopHeadMBB, DL, TII->get(getLRForRMW(Ordering, Width, STI)),
            DestReg)
        .addReg(AddrReg);
    BuildMI(LoopHeadMBB, DL, TII->get(YSX::BNE))
        .addReg(DestReg)
        .addReg(CmpValReg)
    ...
    BuildMI(LoopHeadMBB, DL, TII->get(getLRForRMW(Ordering, Width, STI)),
            DestReg)
        .addReg(AddrReg);
    BuildMI(LoopHeadMBB, DL, TII->get(YSX::AND), ScratchReg)
        .addReg(DestReg)
        .addReg(MaskReg);
    BuildMI(LoopHeadMBB, DL, TII->get(YSX::BNE))
        .addReg(ScratchReg)
...
INITIALIZE_PASS(YSXExpandAtomicPseudo, "ysx-expand-atomic-pseudo",
                YSX_EXPAND_ATOMIC_PSEUDO_NAME, false, false)

FunctionPass *llvm::createYSXExpandAtomicPseudoPass() {
  return new YSXExpandAtomicPseudo();
}
