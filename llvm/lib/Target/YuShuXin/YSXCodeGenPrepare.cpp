...
public:
  YSXCodeGenPrepare(Function &F, const DominatorTree *DT,
                      const YSXSubtarget *ST)
...
bool YSXCodeGenPrepare::widenVPMerge(IntrinsicInst &II) {
...
// selection.
bool YSXCodeGenPrepare::visitIntrinsicInst(IntrinsicInst &I) {
...
// we have +optimized-zero-stride-loads. YSXDAGToDAGISel::Select will convert
// it back to a strided load if it's optimized.
bool YSXCodeGenPrepare::expandVPStrideLoad(IntrinsicInst &II) {
  Value *BasePtr, *VL;
...
bool YSXCodeGenPrepareLegacyPass::runOnFunction(Function &F) {
  ...
  YSXCodeGenPrepare RVCGP(F, DT, ST);
  return RVCGP.run();
}
...
PreservedAnalyses YSXCodeGenPreparePass::run(Function &F,
  ...
  auto ST = &TM->getSubtarget<YSXSubtarget>(F);
  bool Changed = YSXCodeGenPrepare(F, DT, ST).run();
  if (!Changed)
