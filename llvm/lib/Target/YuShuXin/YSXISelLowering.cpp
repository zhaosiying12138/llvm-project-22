...
static SDValue getVLOperand(SDValue Op) {
  ...
  const YSXVIntrinsicsTable::YSXVIntrinsicInfo *II =
      YSXVIntrinsicsTable::getYSXVIntrinsicInfo(IntNo);
  if (!II)
    return SDValue();
...
MVT YSXTargetLowering::getContainerForFixedLengthVector(MVT VT) const {
...
static SDValue getAllOnesMask(MVT VecVT, SDValue VL, const SDLoc &DL,
  ...
  return DAG.getNode(YSXISD::VMSET_VL, DL, MaskVT, VL);
}
...
getDefaultScalableVLOps(MVT VecVT, const SDLoc &DL, SelectionDAG &DAG,
  ...
  SDValue VL = DAG.getRegister(YSX::X0, Subtarget.getXLenVT());
...
getDefaultVLOps(uint64_t NumElts, MVT ContainerVT, const SDLoc &DL,
                SelectionDAG &DAG, const YSXSubtarget &Subtarget) {
...
SDValue YSXTargetLowering::computeVLMax(MVT VecVT, const SDLoc &DL,
                                          SelectionDAG &DAG) const {
...
std::pair<unsigned, unsigned>
YSXTargetLowering::computeVLMAXBounds(MVT VecVT,
                                        const YSXSubtarget &Subtarget) {
  assert(VecVT.isScalableVector() && "Expected scalable vector");
  ...
  unsigned MaxVLMAX =
      YSXTargetLowering::computeVLMAX(VectorBitsMax, EltSize, MinSize);

  unsigned VectorBitsMin = Subtarget.getRealMinVLen();
  unsigned MinVLMAX =
      YSXTargetLowering::computeVLMAX(VectorBitsMin, EltSize, MinSize);
...
InstructionCost YSXTargetLowering::getLMULCost(MVT VT) const {
...
InstructionCost YSXTargetLowering::getVRGatherVICost(MVT VT) const {
...
/// or may track the vrgather.vv cost. It is implementation-dependent.
InstructionCost YSXTargetLowering::getVSlideVXCost(MVT VT) const {
  return getLMULCost(VT);
}
...
/// variants.)  Slides may be linear in the number of vregs implied by LMUL,
/// or may track the vrgather.vv cost. It is implementation-dependent.
InstructionCost YSXTargetLowering::getVSlideVICost(MVT VT) const {
  return getLMULCost(VT);
}
...
static SDValue lowerFP_TO_INT_SAT(SDValue Op, SelectionDAG &DAG,
  ...
  if (!DstVT.isVector()) {
    ...
    unsigned Opc;
    if (SatVT == DstVT)
      Opc = IsSigned ? YSXISD::FCVT_X : YSXISD::FCVT_XU;
    else if (DstVT == MVT::i64 && SatVT == MVT::i32)
      Opc = IsSigned ? YSXISD::FCVT_W_RV64 : YSXISD::FCVT_WU_RV64;
    else
      return SDValue();
    ...
    SDValue FpToInt = DAG.getNode(
        Opc, DL, DstVT, Src,
        DAG.getTargetConstant(YSXFPRndMode::RTZ, DL, Subtarget.getXLenVT()));

    if (Opc == YSXISD::FCVT_WU_RV64)
  ...
  SDValue IsNan = DAG.getNode(YSXISD::SETCC_VL, DL, Mask.getValueType(),
  ...
  if (DstEltSize > (2 * SrcEltSize)) {
    ...
    Src = DAG.getNode(YSXISD::FP_EXTEND_VL, DL, InterVT, Src, Mask, VL);
  }
  ...
  while (CvtContainerVT != DstContainerVT) {
    ...
    unsigned ClipOpc = IsSigned ? YSXISD::TRUNCATE_VECTOR_VL_SSAT
                                : YSXISD::TRUNCATE_VECTOR_VL_USAT;
    Res = DAG.getNode(ClipOpc, DL, CvtContainerVT, Res, Mask, VL);
  }

  SDValue SplatZero = DAG.getNode(
      YSXISD::VMV_V_X_VL, DL, DstContainerVT, DAG.getUNDEF(DstContainerVT),
      DAG.getConstant(0, DL, Subtarget.getXLenVT()), VL);
  Res = DAG.getNode(YSXISD::VMERGE_VL, DL, DstContainerVT, IsNan, SplatZero,
...
static YSXFPRndMode::RoundingMode matchRoundingOp(unsigned Opc) {
  switch (Opc) {
  case ISD::FROUNDEVEN:
  case ISD::STRICT_FROUNDEVEN:
  case ISD::VP_FROUNDEVEN:
    return YSXFPRndMode::RNE;
  case ISD::FTRUNC:
  case ISD::STRICT_FTRUNC:
  case ISD::VP_FROUNDTOZERO:
    return YSXFPRndMode::RTZ;
  case ISD::FFLOOR:
  case ISD::STRICT_FFLOOR:
  case ISD::VP_FFLOOR:
    return YSXFPRndMode::RDN;
  case ISD::FCEIL:
  case ISD::STRICT_FCEIL:
  case ISD::VP_FCEIL:
    return YSXFPRndMode::RUP;
  case ISD::FROUND:
  case ISD::LROUND:
  case ISD::LLROUND:
  ...
  case ISD::VP_LRINT:
  case ISD::VP_LLRINT:
    return YSXFPRndMode::DYN;
  }

  return YSXFPRndMode::Invalid;
}
...
lowerVectorFTRUNC_FCEIL_FFLOOR_FROUND(SDValue Op, SelectionDAG &DAG,
  ...
  SDValue Abs = DAG.getNode(YSXISD::FABS_VL, DL, ContainerVT, Src, Mask, VL);
  ...
  SDValue MaxValSplat = DAG.getNode(YSXISD::VFMV_V_F_VL, DL, ContainerVT,
  ...
  Mask =
      DAG.getNode(YSXISD::SETCC_VL, DL, SetccVT,
  ...
  case ISD::VP_FROUNDEVEN:
  case ISD::VP_FROUNDTOZERO: {
    YSXFPRndMode::RoundingMode FRM = matchRoundingOp(Op.getOpcode());
    assert(FRM != YSXFPRndMode::Invalid);
    Truncated = DAG.getNode(YSXISD::VFCVT_RM_X_F_VL, DL, IntVT, Src, Mask,
                            DAG.getTargetConstant(FRM, DL, XLenVT), VL);
    break;
  }
  case ISD::FTRUNC:
    Truncated = DAG.getNode(YSXISD::VFCVT_RTZ_X_F_VL, DL, IntVT, Src,
                            Mask, VL);
    break;
  case ISD::FNEARBYINT:
  case ISD::VP_FNEARBYINT:
    Truncated = DAG.getNode(YSXISD::VFROUND_NOEXCEPT_VL, DL, ContainerVT, Src,
                            Mask, VL);
    break;
  }

  // VFROUND_NOEXCEPT_VL includes SINT_TO_FP_VL.
  if (Truncated.getOpcode() != YSXISD::VFROUND_NOEXCEPT_VL)
    Truncated = DAG.getNode(YSXISD::SINT_TO_FP_VL, DL, ContainerVT, Truncated,
                            Mask, VL);

  // Restore the original sign so that -0.0 is preserved.
  Truncated = DAG.getNode(YSXISD::FCOPYSIGN_VL, DL, ContainerVT, Truncated,
                          Src, Src, Mask, VL);
...
lowerVectorStrictFTRUNC_FCEIL_FFLOOR_FROUND(SDValue Op, SelectionDAG &DAG,
  ...
  SDValue Unorder = DAG.getNode(YSXISD::STRICT_FSETCC_VL, DL,
  ...
  Src = DAG.getNode(YSXISD::STRICT_FADD_VL, DL,
  ...
  case ISD::STRICT_FROUNDEVEN: {
    ...
    Truncated = DAG.getNode(
        YSXISD::STRICT_VFCVT_RM_X_F_VL, DL, DAG.getVTList(IntVT, MVT::Other),
        {Chain, Src, Mask, DAG.getTargetConstant(FRM, DL, XLenVT), VL});
    break;
  }
  case ISD::STRICT_FTRUNC:
    Truncated =
        DAG.getNode(YSXISD::STRICT_VFCVT_RTZ_X_F_VL, DL,
                    DAG.getVTList(IntVT, MVT::Other), Chain, Src, Mask, VL);
    break;
  case ISD::STRICT_FNEARBYINT:
    Truncated = DAG.getNode(YSXISD::STRICT_VFROUND_NOEXCEPT_VL, DL,
  ...
  if (Op.getOpcode() != ISD::STRICT_FNEARBYINT) {
    Truncated = DAG.getNode(YSXISD::STRICT_SINT_TO_FP_VL, DL,
...
lowerFTRUNC_FCEIL_FFLOOR_FROUND(SDValue Op, SelectionDAG &DAG,
  ...
  YSXFPRndMode::RoundingMode FRM = matchRoundingOp(Op.getOpcode());
  return DAG.getNode(YSXISD::FROUND, DL, VT, Src, MaxValNode,
...
static SDValue lowerVectorXRINT_XROUND(SDValue Op, SelectionDAG &DAG,
  ...
  if (SrcElemType == MVT::f16 || SrcElemType == MVT::bf16) {
    ...
    Src = DAG.getNode(YSXISD::FP_EXTEND_VL, DL, F32VT, Src, Mask, VL);
  }

  SDValue Res =
      DAG.getNode(YSXISD::VFCVT_RM_X_F_VL, DL, DstContainerVT, Src, Mask,
...
static SDValue matchSplatAsGather(SDValue SplatVal, MVT VT, const SDLoc &DL,
  ...
  auto [Mask, VL] = getDefaultVLOps(VT, ContainerVT, DL, DAG, Subtarget);
  SDValue Gather = DAG.getNode(YSXISD::VRGATHER_VX_VL, DL, ContainerVT, Src,
...
static SDValue lowerBuildVectorViaVID(SDValue Op, SelectionDAG &DAG,
  ...
  if (auto SimpleVID = isSimpleVIDSequence(Op, Op.getScalarValueSizeInBits())) {
    ...
    if (((StepOpcode == ISD::MUL && isInt<12>(SplatStepVal)) ||
      ...
      SDValue VID = DAG.getNode(YSXISD::VID_VL, DL, VIDContainerVT, Mask, VL);
...
static SDValue lowerBuildVectorViaDominantValues(SDValue Op, SelectionDAG &DAG,
  ...
  if (!DAG.shouldOptForSize() && NumScalarLoads < NumElts &&
    ...
    if (SDValue LastOp = Op->getOperand(Op->getNumOperands() - 1);
      ...
      auto OpCode =
        VT.isFloatingPoint() ? YSXISD::VFSLIDE1DOWN_VL : YSXISD::VSLIDE1DOWN_VL;
      if (!VT.isFloatingPoint())
...
static SDValue lowerBuildVectorOfConstants(SDValue Op, SelectionDAG &DAG,
  ...
  if (VT.getVectorElementType() == MVT::i1) {
    if (ISD::isBuildVectorAllZeros(Op.getNode())) {
      SDValue VMClr = DAG.getNode(YSXISD::VMCLR_VL, DL, ContainerVT, VL);
      return convertFromScalableVector(VT, VMClr, DAG, Subtarget);
    }

    if (ISD::isBuildVectorAllOnes(Op.getNode())) {
      SDValue VMSet = DAG.getNode(YSXISD::VMSET_VL, DL, ContainerVT, VL);
  ...
  if (SDValue Splat = cast<BuildVectorSDNode>(Op)->getSplatValue()) {
    unsigned Opc = VT.isFloatingPoint() ? YSXISD::VFMV_V_F_VL
                                        : YSXISD::VMV_V_X_VL;
    if (!VT.isFloatingPoint())
  ...
  if (VT.isInteger() && EltBitSize < Subtarget.getELen() &&
    ...
    if (ViaIntVT.bitsLE(XLenVT) || isInt<32>(SplatValue)) {
      ...
      SDValue Splat =
          DAG.getNode(YSXISD::VMV_V_X_VL, DL, ViaContainerVT,
  ...
  if (EltBitSize > 8 && VT.isInteger() &&
    ...
    SDValue Res = DAG.getNode(YSXISD::VSEXT_VL, DL, ContainerVT, Source, Mask, VL);
...
static SDValue lowerBUILD_VECTOR(SDValue Op, SelectionDAG &DAG,
    ...
    for (const auto &[I, U] : enumerate(Op->ops())) {
      ...
      if ((EltVT == MVT::bf16 && Subtarget.hasStdExtZfbfmin()) ||
        ...
        if (auto *C = dyn_cast<ConstantFPSDNode>(Elem)) {
          ...
          NewOps[I] = DAG.getNode(YSXISD::FMV_X_ANYEXTH, DL, XLenVT, Elem);
        }
      } else {
  ...
  if (SDValue Splat = cast<BuildVectorSDNode>(Op)->getSplatValue()) {
    ...
    if (all_of(Op->ops().drop_front(),
      ...
      unsigned Opc =
          VT.isFloatingPoint() ? YSXISD::VFMV_S_F_VL : YSXISD::VMV_S_X_VL;
      if (!VT.isFloatingPoint())
        Splat = DAG.getNode(ISD::ANY_EXTEND, DL, XLenVT, Splat);
    ...
    unsigned Opc =
        VT.isFloatingPoint() ? YSXISD::VFMV_V_F_VL : YSXISD::VMV_V_X_VL;
    if (!VT.isFloatingPoint())
      Splat = DAG.getNode(ISD::ANY_EXTEND, DL, XLenVT, Splat);
    Splat =
        DAG.getNode(Opc, DL, ContainerVT, DAG.getUNDEF(ContainerVT), Splat, VL);
  ...
  if (const auto VLen = Subtarget.getRealVLen();
    ...
    assert(M1VT == YSXTargetLowering::getM1VT(M1VT));
  ...
  if (NumDefElts >= 8 && NumDefElts > NumElts / 2 &&
      ContainerVT.bitsLE(YSXTargetLowering::getM1VT(ContainerVT))) {
  ...
  for (SDValue V : Operands) {
    ...
    unsigned Opcode;
    if (VT.isFloatingPoint())
      Opcode = SlideUp ? YSXISD::VFSLIDE1UP_VL : YSXISD::VFSLIDE1DOWN_VL;
    else
      Opcode = SlideUp ? YSXISD::VSLIDE1UP_VL : YSXISD::VSLIDE1DOWN_VL;
...
static SDValue splatPartsI64WithVL(const SDLoc &DL, MVT VT, SDValue Passthru,
  ...
  if (isa<ConstantSDNode>(Lo) && isa<ConstantSDNode>(Hi)) {
    ...
    if (LoC == HiC) {
      ...
      if (isa<ConstantSDNode>(VL) && isUInt<4>(VL->getAsZExtVal()))
        ...
        NewVL = DAG.getRegister(YSX::X0, MVT::i32);
      MVT InterVT =
          MVT::getVectorVT(MVT::i32, VT.getVectorElementCount() * 2);
      auto InterVec = DAG.getNode(YSXISD::VMV_V_X_VL, DL, InterVT,
  ...
  return DAG.getNode(YSXISD::SPLAT_VECTOR_SPLIT_I64_VL, DL, VT, Passthru, Lo,
                     Hi, VL);
}
...
static SDValue lowerScalarSplat(SDValue Passthru, SDValue Scalar, SDValue VL,
  ...
  if (VT.isFloatingPoint()) {
    if ((EltVT == MVT::f16 && !Subtarget.hasStdExtZvfh()) ||
      ...
      if ((EltVT == MVT::bf16 && Subtarget.hasStdExtZfbfmin()) ||
          (EltVT == MVT::f16 && Subtarget.hasStdExtZfhmin()))
        Scalar = DAG.getNode(YSXISD::FMV_X_ANYEXTH, DL, XLenVT, Scalar);
      else
    ...
    return DAG.getNode(YSXISD::VFMV_V_F_VL, DL, VT, Passthru, Scalar, VL);
  }
  ...
  if (Scalar.getValueType().bitsLE(XLenVT)) {
    ...
    return DAG.getNode(YSXISD::VMV_V_X_VL, DL, VT, Passthru, Scalar, VL);
  }
  ...
  if (isOneConstant(VL) && isNullConstant(Scalar))
    return DAG.getNode(YSXISD::VMV_S_X_VL, DL, VT, Passthru,
...
static SDValue lowerScalarInsert(SDValue Scalar, SDValue VL, MVT VT,
  ...
  if (VT.isFloatingPoint())
    return DAG.getNode(YSXISD::VFMV_S_F_VL, DL, VT, DAG.getUNDEF(VT), Scalar,
                       VL);
  ...
  unsigned ExtOpc =
      isa<ConstantSDNode>(Scalar) ? ISD::SIGN_EXTEND : ISD::ANY_EXTEND;
  Scalar = DAG.getNode(ExtOpc, DL, XLenVT, Scalar);
  return DAG.getNode(YSXISD::VMV_S_X_VL, DL, VT, DAG.getUNDEF(VT), Scalar,
                     VL);
}
...
static bool isInterleaveShuffle(ArrayRef<int> Mask, MVT VT, int &EvenSrc,
                                int &OddSrc, const YSXSubtarget &Subtarget) {
...
static SDValue lowerVECTOR_SHUFFLEAsVSlideup(const SDLoc &DL, MVT VT,
  ...
  // vmv.v.v.
  if (Index == 0)
    Res = DAG.getNode(YSXISD::VMV_V_V_VL, DL, ContainerVT, InPlace, ToInsert,
                      VL);
  else
...
static SDValue lowerVECTOR_SHUFFLEAsVSlide1(const SDLoc &DL, MVT VT,
  ...
  if (VT.getVectorElementType() == MVT::bf16 ||
    ...
    MVT IntVT = ContainerVT.changeVectorElementTypeToInteger();
    Splat =
        DAG.getNode(YSXISD::FMV_X_ANYEXTH, DL, Subtarget.getXLenVT(), Splat);
    V2 = DAG.getBitcast(
        IntVT, convertToScalableVector(ContainerVT, V2, DAG, Subtarget));
    SDValue Vec = DAG.getNode(
        IsVSlidedown ? YSXISD::VSLIDE1DOWN_VL : YSXISD::VSLIDE1UP_VL, DL,
  ...
  auto OpCode = IsVSlidedown ?
    (VT.isFloatingPoint() ? YSXISD::VFSLIDE1DOWN_VL : YSXISD::VSLIDE1DOWN_VL) :
    (VT.isFloatingPoint() ? YSXISD::VFSLIDE1UP_VL : YSXISD::VSLIDE1UP_VL);
  if (!VT.isFloatingPoint())
...
bool YSXTargetLowering::isSpreadMask(ArrayRef<int> Mask, unsigned Factor,
                                       unsigned &Index) {
...
static SDValue lowerVZIP(unsigned Opc, SDValue Op0, SDValue Op1,
                         const SDLoc &DL, SelectionDAG &DAG,
                         const YSXSubtarget &Subtarget) {
  assert(YSXISD::RI_VZIPEVEN_VL == Opc || YSXISD::RI_VZIPODD_VL == Opc ||
         YSXISD::RI_VZIP2A_VL == Opc || YSXISD::RI_VZIP2B_VL == Opc ||
         YSXISD::RI_VUNZIP2A_VL == Opc || YSXISD::RI_VUNZIP2B_VL == Opc);
  ...
  if (Op1.isUndef() &&
      ContainerVT.bitsGT(YSXTargetLowering::getM1VT(ContainerVT)) &&
      (YSXISD::RI_VUNZIP2A_VL == Opc || YSXISD::RI_VUNZIP2B_VL == Opc)) {
...
static SDValue getWideningInterleave(SDValue EvenV, SDValue OddV,
  ...
  if (Subtarget.hasStdExtZvbb()) {
    ...
    Interleaved = DAG.getNode(YSXISD::VWSLL_VL, DL, WideContainerVT, OddV,
                              OffsetVec, Passthru, Mask, VL);
    Interleaved = DAG.getNode(YSXISD::VWADDU_W_VL, DL, WideContainerVT,
    ...
    // vwaddu.vv
    Interleaved = DAG.getNode(YSXISD::VWADDU_VL, DL, WideContainerVT, EvenV,
    ...
    SDValue OddsMul = DAG.getNode(YSXISD::VWMULU_VL, DL, WideContainerVT,
    ...
    Interleaved = DAG.getNode(YSXISD::ADD_VL, DL, WideContainerVT,
...
static SDValue lowerVECTOR_SHUFFLE(SDValue Op, SelectionDAG &DAG,
  ...
  if (SVN->isSplat()) {
    ...
    if (Lane >= 0) {
      ...
      if (ISD::isNormalLoad(V.getNode()) && cast<LoadSDNode>(V)->isSimple()) {
        ...
        unsigned Opc = SplatVT.isFloatingPoint() ? YSXISD::VFMV_V_F_VL
                                                 : YSXISD::VMV_V_X_VL;
        SDValue Splat =
      ...
      SDValue Gather = DAG.getNode(YSXISD::VRGATHER_VX_VL, DL, ContainerVT,
  ...
  if (Subtarget.hasVendorXRivosVizip() &&
      ...
      1 < count_if(Mask, [](int Idx) { return Idx != -1; })) {
    unsigned Opc =
        Index == 0 ? YSXISD::RI_VUNZIP2A_VL : YSXISD::RI_VUNZIP2B_VL;
    if (V2.isUndef())
  ...
  if (isInterleaveShuffle(Mask, VT, EvenSrc, OddSrc, Subtarget) &&
    ...
    if (Subtarget.hasVendorXRivosVizip()) {
      ...
      return lowerVZIP(YSXISD::RI_VZIP2A_VL, EvenV, OddV, DL, DAG, Subtarget);
    }
    ...
    if (Subtarget.hasVendorXRivosVizip()) {
      ...
      if (isZipEven(SrcInfo, Mask, Factor)) {
        if (Factor == 1) {
          ...
          return lowerVZIP(YSXISD::RI_VZIPEVEN_VL, Src1, Src2, DL, DAG,
                           Subtarget);
        }
        TryWiden = true;
      }
      if (isZipOdd(SrcInfo, Mask, Factor)) {
        if (Factor == 1) {
          ...
          return lowerVZIP(YSXISD::RI_VZIPODD_VL, Src1, Src2, DL, DAG,
                           Subtarget);
        }
        TryWiden = true;
      }
  ...
  if (V2.isUndef()) {
    ...
    if (VT.getScalarSizeInBits() < Subtarget.getELen()) {
      ...
      for (unsigned Factor = 4; Factor <= MaxFactor; Factor <<= 1) {
        unsigned Index;
        if (YSXTargetLowering::isSpreadMask(Mask, Factor, Index)) {
          MVT NarrowVT =
    ...
    unsigned GatherVVOpc = YSXISD::VRGATHER_VV_VL;
    ...
    if (IndexVT.getScalarType().bitsGT(XLenVT)) {
      GatherVVOpc = YSXISD::VRGATHEREI16_VV_VL;
    ...
    const MVT M1VT = YSXTargetLowering::getM1VT(ContainerVT);
...
// the exponent.
SDValue
YSXTargetLowering::lowerCTLZ_CTTZ_ZERO_UNDEF(SDValue Op,
                                               SelectionDAG &DAG) const {
  MVT VT = Op.getSimpleValueType();
  ...
  // Legal types should have been checked in the YSXTargetLowering
  // constructor.
  ...
  if (FloatVT.bitsGT(VT)) {
    ...
    FloatVal = DAG.getNode(YSXISD::VFCVT_RM_F_XU_VL, DL, ContainerFloatVT,
                           Src, Mask, RTZRM, VL);
    if (VT.isFixedLengthVector())
...
SDValue YSXTargetLowering::lowerVPCttzElements(SDValue Op,
                                                 SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT XLenVT = Subtarget.getXLenVT();
  ...
  if (SrcVT.getScalarType() != MVT::i1) {
    ...
    Source = DAG.getNode(YSXISD::SETCC_VL, DL, SrcVT,
                         {Source, AllZero, DAG.getCondCode(ISD::SETNE),
                          DAG.getUNDEF(SrcVT), Mask, EVL});
  }

  SDValue Res = DAG.getNode(YSXISD::VFIRST_VL, DL, XLenVT, Source, Mask, EVL);
...
// the load is already correctly-aligned, it returns SDValue().
SDValue YSXTargetLowering::expandUnalignedVPLoad(SDValue Op,
                                                   SelectionDAG &DAG) const {
...
SDValue YSXTargetLowering::expandUnalignedVPStore(SDValue Op,
                                                    SelectionDAG &DAG) const {
...
SDValue YSXTargetLowering::LowerIS_FPCLASS(SDValue Op,
                                             SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT VT = Op.getSimpleValueType();
  MVT XLenVT = Subtarget.getXLenVT();
  ...
  unsigned TDCMask = 0;
  if (Check & fcSNan)
    TDCMask |= YSX::FPMASK_Signaling_NaN;
  if (Check & fcQNan)
    TDCMask |= YSX::FPMASK_Quiet_NaN;
  if (Check & fcPosInf)
    TDCMask |= YSX::FPMASK_Positive_Infinity;
  if (Check & fcNegInf)
    TDCMask |= YSX::FPMASK_Negative_Infinity;
  if (Check & fcPosNormal)
    TDCMask |= YSX::FPMASK_Positive_Normal;
  if (Check & fcNegNormal)
    TDCMask |= YSX::FPMASK_Negative_Normal;
  if (Check & fcPosSubnormal)
    TDCMask |= YSX::FPMASK_Positive_Subnormal;
  if (Check & fcNegSubnormal)
    TDCMask |= YSX::FPMASK_Negative_Subnormal;
  if (Check & fcPosZero)
    TDCMask |= YSX::FPMASK_Positive_Zero;
  if (Check & fcNegZero)
    TDCMask |= YSX::FPMASK_Negative_Zero;
  ...
  if (VT.isVector()) {
    ...
    if (VT.isScalableVector()) {
      ...
      SDValue FPCLASS = DAG.getNode(YSXISD::FCLASS_VL, DL, DstVT, Op0, Mask,
                                    VL, Op->getFlags());
      if (IsOneBitMask)
    ...
    SDValue FPCLASS = DAG.getNode(YSXISD::FCLASS_VL, DL, ContainerDstVT, Op0,
                                  Mask, VL, Op->getFlags());

    TDCMaskV = DAG.getNode(YSXISD::VMV_V_X_VL, DL, ContainerDstVT,
                           DAG.getUNDEF(ContainerDstVT), TDCMaskV, VL);
    if (IsOneBitMask) {
      SDValue VMSEQ =
          DAG.getNode(YSXISD::SETCC_VL, DL, ContainerVT,
                      {FPCLASS, TDCMaskV, DAG.getCondCode(ISD::SETEQ),
                       DAG.getUNDEF(ContainerVT), Mask, VL});
      return convertFromScalableVector(VT, VMSEQ, DAG, Subtarget);
    }
    SDValue AND = DAG.getNode(YSXISD::AND_VL, DL, ContainerDstVT, FPCLASS,
                              TDCMaskV, DAG.getUNDEF(ContainerDstVT), Mask, VL);

    SDValue SplatZero = DAG.getConstant(0, DL, XLenVT);
    SplatZero = DAG.getNode(YSXISD::VMV_V_X_VL, DL, ContainerDstVT,
                            DAG.getUNDEF(ContainerDstVT), SplatZero, VL);

    SDValue VMSNE = DAG.getNode(YSXISD::SETCC_VL, DL, ContainerVT,
                                {AND, SplatZero, DAG.getCondCode(ISD::SETNE),
                                 DAG.getUNDEF(ContainerVT), Mask, VL});
    return convertFromScalableVector(VT, VMSNE, DAG, Subtarget);
  }

  SDValue FCLASS = DAG.getNode(YSXISD::FCLASS, DL, XLenVT, Op.getOperand(0));
...
static SDValue lowerFMAXIMUM_FMINIMUM(SDValue Op, SelectionDAG &DAG,
  ...
  if (!VT.isVector()) {
    ...
    unsigned Opc =
        Op.getOpcode() == ISD::FMAXIMUM ? YSXISD::FMAX : YSXISD::FMIN;
  ...
  SDValue NewY = Y;
  if (!XIsNeverNan) {
    SDValue XIsNonNan = DAG.getNode(YSXISD::SETCC_VL, DL, Mask.getValueType(),
                                    {X, X, DAG.getCondCode(ISD::SETOEQ),
                                     DAG.getUNDEF(ContainerVT), Mask, VL});
    NewY = DAG.getNode(YSXISD::VMERGE_VL, DL, ContainerVT, XIsNonNan, Y, X,
                       DAG.getUNDEF(ContainerVT), VL);
  }

  SDValue NewX = X;
  if (!YIsNeverNan) {
    SDValue YIsNonNan = DAG.getNode(YSXISD::SETCC_VL, DL, Mask.getValueType(),
                                    {Y, Y, DAG.getCondCode(ISD::SETOEQ),
                                     DAG.getUNDEF(ContainerVT), Mask, VL});
    NewX = DAG.getNode(YSXISD::VMERGE_VL, DL, ContainerVT, YIsNonNan, X, Y,
                       DAG.getUNDEF(ContainerVT), VL);
  }
...
static SDValue lowerFABSorFNEG(SDValue Op, SelectionDAG &DAG,
  ...
  SDLoc DL(Op);
  SDValue Fmv =
      DAG.getNode(YSXISD::FMV_X_ANYEXTH, DL, XLenVT, Op.getOperand(0));
  ...
  return DAG.getNode(YSXISD::FMV_H_X, DL, VT, Logic);
}
...
static SDValue lowerFCOPYSIGN(SDValue Op, SelectionDAG &DAG,
  ...
  SDValue SignAsInt = [&]() {
    ...
    switch (SignSize) {
    case 16:
      return DAG.getNode(YSXISD::FMV_X_ANYEXTH, DL, XLenVT, Sign);
    case 32:
      return DAG.getNode(YSXISD::FMV_X_ANYEXTW_RV64, DL, XLenVT, Sign);
    case 64: {
      assert(XLenVT == MVT::i32 && "Unexpected type");
      // Copy the upper word to integer.
      SignSize = 32;
      return DAG.getNode(YSXISD::SplitF64, DL, {MVT::i32, MVT::i32}, Sign)
          .getValue(1);
    }
    default:
  ...
  SDValue MagAsInt = DAG.getNode(YSXISD::FMV_X_ANYEXTH, DL, XLenVT, Mag);
  ...
  return DAG.getNode(YSXISD::FMV_H_X, DL, VT, CopiedSign);
}
...
SDValue
YSXTargetLowering::lowerXAndesBfHCvtBFloat16Load(SDValue Op,
                                                   SelectionDAG &DAG) const {
  ...
  SDValue ConvertedResult =
      DAG.getNode(YSXISD::NDS_FMV_BF16_X, DL, MVT::bf16, OrSixteenOne);
  return DAG.getMergeValues({ConvertedResult, Load.getValue(1)}, DL);
}

SDValue
YSXTargetLowering::lowerXAndesBfHCvtBFloat16Store(SDValue Op,
                                                    SelectionDAG &DAG) const {
  assert(Subtarget.hasVendorXAndesBFHCvt() && !Subtarget.hasStdExtZfh() &&
  ...
  SDLoc DL(Op);
  SDValue FMV = DAG.getNode(YSXISD::NDS_FMV_X_ANYEXTBF16, DL,
...
SDValue YSXTargetLowering::LowerOperation(SDValue Op,
  ...
  default:
    reportFatalInternalError(
        "Unimplemented YSXTargetLowering::LowerOperation Case");
  case ISD::PREFETCH:
  ...
  case ISD::BITCAST: {
    ...
    if (Op0VT == MVT::i16 &&
      ...
      return DAG.getNode(YSXISD::FMV_H_X, DL, VT, NewOp0);
    }
    if (VT == MVT::f32 && Op0VT == MVT::i32 && Subtarget.is64Bit() &&
        Subtarget.hasStdExtFOrZfinx()) {
      SDValue NewOp0 = DAG.getNode(ISD::ANY_EXTEND, DL, MVT::i64, Op0);
      return DAG.getNode(YSXISD::FMV_W_X_RV64, DL, MVT::f32, NewOp0);
    }
    if (VT == MVT::f64 && Op0VT == MVT::i64 && !Subtarget.is64Bit() &&
      ...
      return DAG.getNode(YSXISD::BuildPairF64, DL, MVT::f64, Lo, Hi);
    }
  ...
  case ISD::BITREVERSE: {
    ...
    return DAG.getNode(YSXISD::BREV8, DL, VT, BSwap);
  }
  case ISD::TRUNCATE:
  ...
  case ISD::VSCALE: {
    ...
    MVT VT = Op.getSimpleValueType();
    SDLoc DL(Op);
    SDValue Res = DAG.getNode(YSXISD::READ_VLENB, DL, XLenVT);
  ...
  case ISD::FP_TO_BF16: {
    ...
    if (Subtarget.is64Bit())
      return DAG.getNode(YSXISD::FMV_X_ANYEXTW_RV64, DL, MVT::i64, Res);
  ...
  case ISD::BF16_TO_FP: {
    ...
    SDValue Res = Subtarget.is64Bit()
                      ? DAG.getNode(YSXISD::FMV_W_X_RV64, DL, MVT::f32, Op)
  ...
  case ISD::FP16_TO_FP: {
    ...
    SDValue Arg = Subtarget.is64Bit()
                      ? DAG.getNode(YSXISD::FMV_W_X_RV64, DL, MVT::f32, Op0)
  ...
  case ISD::SPLAT_VECTOR: {
    ...
    if ((EltVT == MVT::f16 && !Subtarget.hasStdExtZvfh()) ||
      ...
      if ((EltVT == MVT::bf16 && Subtarget.hasStdExtZfbfmin()) ||
          (EltVT == MVT::f16 && Subtarget.hasStdExtZfhmin()))
        Elt = DAG.getNode(YSXISD::FMV_X_ANYEXTH, DL, Subtarget.getXLenVT(),
                          Op.getOperand(0));
      else
  ...
  case ISD::CONCAT_VECTORS: {
    ...
    if (Op.getNumOperands() > 2 &&
        ContainerVT.bitsGE(YSXTargetLowering::getM1VT(ContainerVT))) {
      MVT HalfVT = VT.getHalfNumVectorElementsVT();
  ...
  case ISD::LOAD: {
    ...
    if (VT == MVT::f64) {
      ...
      SDValue Pair = DAG.getNode(YSXISD::BuildPairF64, DL, MVT::f64, Lo, Hi);
    ...
    if (VT.isRISCVVectorTuple()) {
      ...
      SDValue BasePtr = Load->getBasePtr();
      SDValue VROffset = DAG.getNode(YSXISD::READ_VLENB, DL, XLenVT);
      VROffset =
      ...
      for (unsigned i = 0; i < NF; ++i) {
        ...
        Ret = DAG.getNode(YSXISD::TUPLE_INSERT, DL, VT, Ret, LoadVal,
  ...
  case ISD::STORE: {
    ...
    if (VT == MVT::f64) {
      ...
      SDValue Split = DAG.getNode(YSXISD::SplitF64, DL,
    ...
    if (VT == MVT::i64) {
      ...
      return DAG.getMemIntrinsicNode(
          YSXISD::SD_RV32, DL, DAG.getVTList(MVT::Other),
    ...
    if (VT.isRISCVVectorTuple()) {
      ...
      for (unsigned i = 0; i < NF; ++i) {
        auto Extract =
            DAG.getNode(YSXISD::TUPLE_EXTRACT, DL,
...
SDValue YSXTargetLowering::lowerPARTIAL_REDUCE_MLA(SDValue Op,
                                                     SelectionDAG &DAG) const {
  ...
  case ISD::PARTIAL_REDUCE_SMLA:
    ...
    break;
  case ISD::PARTIAL_REDUCE_UMLA:
    Opc = YSXISD::VQDOTU_VL;
    break;
  case ISD::PARTIAL_REDUCE_SUMLA:
    Opc = YSXISD::VQDOTSU_VL;
    break;
  default:
    llvm_unreachable("Unexpected opcode");
  }
...
SDValue YSXTargetLowering::getAddr(NodeTy *N, SelectionDAG &DAG,
  ...
  case CodeModel::Small: {
    ...
    if (Subtarget.hasVendorXRemovedQcili()) {
      ...
      // LUI/ADDI.
      SDValue Addr = getTargetNode(N, DL, Ty, DAG, 0);
      return DAG.getNode(YSXISD::QC_E_LI, DL, Ty, Addr);
    }
      ...
      if (auto *TrueC = dyn_cast<ConstantSDNode>(TrueV)) {
        ...
        if (isPowerOf2_64(TrueM1)) {
          ...
          if (Subtarget.hasShlAdd(ShAmount))
            return DAG.getNode(YSXISD::SHL_ADD, DL, VT, CondV,
...
SDValue YSXTargetLowering::lowerSELECT(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (UseZicondForFPSel) {
    ...
    auto CastToInt = [&](SDValue V) -> SDValue {
      ...
      if (VT == MVT::f16)
        return DAG.getNode(YSXISD::FMV_X_ANYEXTH, DL, XLenVT, V);

      if (VT == MVT::f32 && Subtarget.is64Bit())
        return DAG.getNode(YSXISD::FMV_X_ANYEXTW_RV64, DL, XLenVT, V);
    ...
    if (VT == MVT::f32 && Subtarget.is64Bit())
      return DAG.getNode(YSXISD::FMV_W_X_RV64, DL, VT, ResultInt);

    if (VT == MVT::f16)
      return DAG.getNode(YSXISD::FMV_H_X, DL, VT, ResultInt);
  ...
  // sequence or YSXISD::SELECT_CC node (branch-based select).
  if (Subtarget.hasCZEROLike() && VT.isScalarInteger()) {

    // (select c, t, 0) -> (czero_eqz t, c)
    if (isNullConstant(FalseV))
      return DAG.getNode(YSXISD::CZERO_EQZ, DL, VT, TrueV, CondV);
    // (select c, 0, f) -> (czero_nez f, c)
    if (isNullConstant(TrueV))
      return DAG.getNode(YSXISD::CZERO_NEZ, DL, VT, FalseV, CondV);
    ...
    if (TrueV.getOpcode() == ISD::AND &&
      ...
      if (NotOperand) {
        SDValue CMOV =
            DAG.getNode(YSXISD::CZERO_EQZ, DL, VT, *NotOperand, CondV);
        SDValue NOT = DAG.getNOT(DL, CMOV, VT);
        return DAG.getNode(ISD::AND, DL, VT, FalseV, NOT);
      }
      return DAG.getNode(
          ISD::OR, DL, VT, TrueV,
          DAG.getNode(YSXISD::CZERO_NEZ, DL, VT, FalseV, CondV));
    }
    ...
    if (FalseV.getOpcode() == ISD::AND &&
      ...
      if (NotOperand) {
        SDValue CMOV =
            DAG.getNode(YSXISD::CZERO_NEZ, DL, VT, *NotOperand, CondV);
        SDValue NOT = DAG.getNOT(DL, CMOV, VT);
        return DAG.getNode(ISD::AND, DL, VT, TrueV, NOT);
      }
      return DAG.getNode(
          ISD::OR, DL, VT, FalseV,
          DAG.getNode(YSXISD::CZERO_EQZ, DL, VT, TrueV, CondV));
    }
    ...
    if (isa<ConstantSDNode>(TrueV) && isa<ConstantSDNode>(FalseV)) {
      ...
      SDValue CMOV =
          DAG.getNode(IsCZERO_NEZ ? YSXISD::CZERO_NEZ : YSXISD::CZERO_EQZ,
                      DL, VT, LHSVal, CondV);
  ...
  // to emit a YSXISD::SELECT_CC comparing the condition to zero. i.e.:
  // (select condv, truev, falsev)
  // -> (ysxisd::select_cc condv, zero, setne, truev, falsev)
  if (CondV.getOpcode() != ISD::SETCC ||
  ...
  // then merge the SETCC node into the lowered YSXISD::SELECT_CC to take
  // advantage of the integer compare+branch instructions. i.e.:
  // (select (setcc lhs, rhs, cc), truev, falsev)
  // -> (ysxisd::select_cc lhs, rhs, cc, truev, falsev)
  SDValue LHS = CondV.getOperand(0);
  SDValue RHS = CondV.getOperand(1);
  ISD::CondCode CCVal = cast<CondCodeSDNode>(CondV.getOperand(2))->get();
...
SDValue YSXTargetLowering::lowerVectorMaskSplat(SDValue Op,
                                                  SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT VT = Op.getSimpleValueType();
  ...
  if (ISD::isConstantSplatVectorAllOnes(Op.getNode())) {
    ...
    return DAG.getNode(YSXISD::VMSET_VL, DL, VT, VL);
  }
  if (ISD::isConstantSplatVectorAllZeros(Op.getNode())) {
    SDValue VL = getDefaultScalableVLOps(VT, DL, DAG, Subtarget).second;
    return DAG.getNode(YSXISD::VMCLR_VL, DL, VT, VL);
  }
...
// them to VMV_V_X_VL.
SDValue YSXTargetLowering::lowerSPLAT_VECTOR_PARTS(SDValue Op,
                                                     SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue YSXTargetLowering::lowerVectorMaskExt(SDValue Op, SelectionDAG &DAG,
  ...
  if (Src.getOpcode() == ISD::EXTRACT_SUBVECTOR) {
    ...
    if (Xor.getOpcode() == YSXISD::VMXOR_VL) {
  ...
  SplatZero = DAG.getNode(YSXISD::VMV_V_X_VL, DL, ContainerVT,
                          DAG.getUNDEF(ContainerVT), SplatZero, VL);
  SplatTrueVal = DAG.getNode(YSXISD::VMV_V_X_VL, DL, ContainerVT,
                             DAG.getUNDEF(ContainerVT), SplatTrueVal, VL);
  SDValue Select =
      DAG.getNode(YSXISD::VMERGE_VL, DL, ContainerVT, CC, SplatTrueVal,
...
SDValue YSXTargetLowering::lowerVectorMaskTruncLike(SDValue Op,
                                                      SelectionDAG &DAG) const {
  ...
  SplatOne = DAG.getNode(YSXISD::VMV_V_X_VL, DL, ContainerVT,
                         DAG.getUNDEF(ContainerVT), SplatOne, VL);
  SplatZero = DAG.getNode(YSXISD::VMV_V_X_VL, DL, ContainerVT,
                          DAG.getUNDEF(ContainerVT), SplatZero, VL);

  MVT MaskContainerVT = ContainerVT.changeVectorElementType(MVT::i1);
  SDValue Trunc = DAG.getNode(YSXISD::AND_VL, DL, ContainerVT, Src, SplatOne,
                              DAG.getUNDEF(ContainerVT), Mask, VL);
  Trunc = DAG.getNode(YSXISD::SETCC_VL, DL, MaskContainerVT,
...
SDValue YSXTargetLowering::lowerVectorTruncLike(SDValue Op,
                                                  SelectionDAG &DAG) const {
  ...
  if (Opc == ISD::TRUNCATE_SSAT_S)
    NewOpc = YSXISD::TRUNCATE_VECTOR_VL_SSAT;
  else if (Opc == ISD::TRUNCATE_USAT_U)
    NewOpc = YSXISD::TRUNCATE_VECTOR_VL_USAT;
  else
    NewOpc = YSXISD::TRUNCATE_VECTOR_VL;
...
SDValue
YSXTargetLowering::lowerStrictFPExtendOrRoundLike(SDValue Op,
                                                    SelectionDAG &DAG) const {
  SDLoc DL(Op);
  SDValue Chain = Op.getOperand(0);
  SDValue Src = Op.getOperand(1);
  ...
  if ((VT.getVectorElementType() == MVT::f64 &&
    ...
    unsigned InterConvOpc = Op.getOpcode() == ISD::STRICT_FP_EXTEND
                                ? YSXISD::STRICT_FP_EXTEND_VL
                                : YSXISD::STRICT_VFNCVT_ROD_VL;
...
SDValue
YSXTargetLowering::lowerVectorFPExtendOrRoundLike(SDValue Op,
                                                    SelectionDAG &DAG) const {
  bool IsVP =
  ...
  unsigned ConvOpc = IsExtend ? YSXISD::FP_EXTEND_VL : YSXISD::FP_ROUND_VL;
  ...
  unsigned InterConvOpc =
      IsExtend ? YSXISD::FP_EXTEND_VL : YSXISD::VFNCVT_ROD_VL;
...
getSmallestVTForIndex(MVT VecVT, unsigned MaxIdx, SDLoc DL, SelectionDAG &DAG,
  ...
  MVT SmallerVT;
  if (MaxIdx < MinVLMAX)
    SmallerVT = YSXTargetLowering::getM1VT(VecVT);
  else if (MaxIdx < MinVLMAX * 2)
    SmallerVT =
        YSXTargetLowering::getM1VT(VecVT).getDoubleNumVectorElementsVT();
...
SDValue YSXTargetLowering::lowerINSERT_VECTOR_ELT(SDValue Op,
                                                    SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT VecVT = Op.getSimpleValueType();
  MVT XLenVT = Subtarget.getXLenVT();
  ...
  if ((ValVT == MVT::f16 && !Subtarget.hasVInstructionsF16()) ||
    ...
    SDValue IntInsert = DAG.getNode(
        ...
        DAG.getNode(YSXISD::FMV_X_ANYEXTH, DL, XLenVT, Val), Idx);
  ...
  if (IsLegalInsert) {
    unsigned Opc =
        VecVT.isFloatingPoint() ? YSXISD::VFMV_S_F_VL : YSXISD::VMV_S_X_VL;
    ...
    if (isNullConstant(Idx)) {
      ...
      ValInVec = DAG.getNode(YSXISD::VSLIDE1DOWN_VL, DL, I32ContainerVT,
...
SDValue YSXTargetLowering::lowerEXTRACT_VECTOR_ELT(SDValue Op,
                                                     SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  if (VecVT.getVectorElementType() == MVT::i1) {
    ...
    if (isNullConstant(Idx)) {
      ...
      auto [Mask, VL] = getDefaultVLOps(VecVT, ContainerVT, DL, DAG, Subtarget);
      SDValue Vfirst =
          DAG.getNode(YSXISD::VFIRST_VL, DL, XLenVT, Vec, Mask, VL);
  ...
  if ((EltVT == MVT::f16 && !Subtarget.hasVInstructionsF16()) ||
    ...
    return DAG.getNode(YSXISD::FMV_H_X, DL, EltVT, IntExtract);
  }
  ...
  if (auto *IdxC = dyn_cast<ConstantSDNode>(Idx);
    ...
    MVT M1VT = YSXTargetLowering::getM1VT(ContainerVT);
  ...
  if (Subtarget.hasVendorXRivosVisni() && EltVT.isInteger() &&
      isValidVisniInsertExtractIndex(Idx)) {
    SDValue Elt = DAG.getNode(YSXISD::RI_VEXTRACT, DL, XLenVT, Vec, Idx);
  ...
  MVT LMUL2VT =
      YSXTargetLowering::getM1VT(ContainerVT).getDoubleNumVectorElementsVT();
  ...
  SDValue Elt0 = DAG.getNode(YSXISD::VMV_X_S, DL, XLenVT, Vec);
  ...
  case Intrinsic::riscv_vslide1down_mask: {
    ...
    if (isa<ConstantSDNode>(AVL)) {
      const auto [MinVLMAX, MaxVLMAX] =
          YSXTargetLowering::computeVLMAXBounds(VT, Subtarget);
      ...
      if (AVLInt <= MinVLMAX) {
        ...
        I32VL = DAG.getRegister(YSX::X0, XLenVT);
      } else {
    ...
    if (IntNo == Intrinsic::riscv_vslide1up ||
      ...
      Vec = DAG.getNode(YSXISD::VSLIDE1UP_VL, DL, I32VT, Passthru, Vec,
                        ScalarHi, I32Mask, I32VL);
      Vec = DAG.getNode(YSXISD::VSLIDE1UP_VL, DL, I32VT, Passthru, Vec,
                        ScalarLo, I32Mask, I32VL);
    } else {
      Vec = DAG.getNode(YSXISD::VSLIDE1DOWN_VL, DL, I32VT, Passthru, Vec,
                        ScalarLo, I32Mask, I32VL);
      Vec = DAG.getNode(YSXISD::VSLIDE1DOWN_VL, DL, I32VT, Passthru, Vec,
                        ScalarHi, I32Mask, I32VL);
    }
...
static SDValue lowerCttzElts(SDNode *N, SelectionDAG &DAG,
  ...
  SDValue Res = DAG.getNode(YSXISD::VFIRST_VL, DL, XLenVT, Op0, Mask, VL);
...
static inline void promoteVCIXScalar(SDValue Op,
                                     ...
                                     SelectionDAG &DAG) {
  const YSXSubtarget &Subtarget =
      DAG.getMachineFunction().getSubtarget<YSXSubtarget>();
...
SDValue YSXTargetLowering::LowerINTRINSIC_WO_CHAIN(SDValue Op,
  ...
  case Intrinsic::riscv_tuple_insert: {
    ...
    return DAG.getNode(YSXISD::TUPLE_INSERT, DL, Op.getValueType(), Vec,
                       SubVec, Index);
  }
  case Intrinsic::riscv_tuple_extract: {
    ...
    return DAG.getNode(YSXISD::TUPLE_EXTRACT, DL, Op.getValueType(), Vec,
                       Index);
  }
  ...
  case Intrinsic::riscv_sm3p1: {
    unsigned Opc;
    switch (IntNo) {
    case Intrinsic::riscv_orc_b:      Opc = YSXISD::ORC_B;      break;
    case Intrinsic::riscv_brev8:      Opc = YSXISD::BREV8;      break;
    case Intrinsic::riscv_sha256sig0: Opc = YSXISD::SHA256SIG0; break;
    case Intrinsic::riscv_sha256sig1: Opc = YSXISD::SHA256SIG1; break;
    case Intrinsic::riscv_sha256sum0: Opc = YSXISD::SHA256SUM0; break;
    case Intrinsic::riscv_sha256sum1: Opc = YSXISD::SHA256SUM1; break;
    case Intrinsic::riscv_sm3p0:      Opc = YSXISD::SM3P0;      break;
    case Intrinsic::riscv_sm3p1:      Opc = YSXISD::SM3P1;      break;
    }
  ...
  case Intrinsic::riscv_sm4ed: {
    unsigned Opc =
        IntNo == Intrinsic::riscv_sm4ks ? YSXISD::SM4KS : YSXISD::SM4ED;
  ...
  case Intrinsic::riscv_unzip: {
    unsigned Opc =
        IntNo == Intrinsic::riscv_zip ? YSXISD::ZIP : YSXISD::UNZIP;
    return DAG.getNode(Opc, DL, XLenVT, Op.getOperand(1));
  }
  case Intrinsic::riscv_mopr:
    return DAG.getNode(YSXISD::MOP_R, DL, XLenVT, Op.getOperand(1),
                       Op.getOperand(2));

  case Intrinsic::riscv_moprr: {
    return DAG.getNode(YSXISD::MOP_RR, DL, XLenVT, Op.getOperand(1),
                       Op.getOperand(2), Op.getOperand(3));
  }
  case Intrinsic::riscv_clmul:
    return DAG.getNode(YSXISD::CLMUL, DL, XLenVT, Op.getOperand(1),
                       Op.getOperand(2));
  case Intrinsic::riscv_clmulh:
  case Intrinsic::riscv_clmulr: {
    unsigned Opc =
        IntNo == Intrinsic::riscv_clmulh ? YSXISD::CLMULH : YSXISD::CLMULR;
    return DAG.getNode(Opc, DL, XLenVT, Op.getOperand(1), Op.getOperand(2));
  }
  ...
  case Intrinsic::riscv_vmv_x_s: {
    SDValue Res = DAG.getNode(YSXISD::VMV_X_S, DL, XLenVT, Op.getOperand(1));
    return DAG.getNode(ISD::TRUNCATE, DL, Op.getValueType(), Res);
  }
  ...
  case Intrinsic::riscv_vfmv_v_f:
    return DAG.getNode(YSXISD::VFMV_V_F_VL, DL, Op.getValueType(),
  ...
  case Intrinsic::riscv_vmv_s_x: {
    ...
    if (Scalar.getValueType().bitsLE(XLenVT)) {
      ...
      return DAG.getNode(YSXISD::VMV_S_X_VL, DL, Op.getValueType(),
    ...
    if (Op.getOperand(1).isUndef())
      return SplattedVal;
    SDValue SplattedIdx =
        DAG.getNode(YSXISD::VMV_V_X_VL, DL, VT, DAG.getUNDEF(VT),
                    DAG.getConstant(0, DL, MVT::i32), VL);

    MVT MaskVT = getMaskTypeFor(VT);
    SDValue Mask = getAllOnesMask(VT, VL, DL, DAG);
    SDValue VID = DAG.getNode(YSXISD::VID_VL, DL, VT, Mask, VL);
    SDValue SelectCond =
        DAG.getNode(YSXISD::SETCC_VL, DL, MaskVT,
                    {VID, SplattedIdx, DAG.getCondCode(ISD::SETEQ),
                     DAG.getUNDEF(MaskVT), Mask, VL});
    return DAG.getNode(YSXISD::VMERGE_VL, DL, VT, SelectCond, SplattedVal,
                       Vec, DAG.getUNDEF(VT), VL);
  }
  case Intrinsic::riscv_vfmv_s_f:
    return DAG.getNode(YSXISD::VFMV_S_F_VL, DL, Op.getSimpleValueType(),
                       Op.getOperand(1), Op.getOperand(2), Op.getOperand(3));
  // EGS * EEW >= 128 bits
...
lowerFixedVectorSegLoadIntrinsics(unsigned IntNo, SDValue Op,
  ...
  for (unsigned int RetIdx = 0; RetIdx < NF; RetIdx++) {
    SDValue SubVec = DAG.getNode(YSXISD::TUPLE_EXTRACT, DL, ContainerVT,
                                 Result.getValue(0),
...
SDValue YSXTargetLowering::LowerINTRINSIC_W_CHAIN(SDValue Op,
  ...
  case Intrinsic::riscv_sf_vc_v_x_se:
    return getVCIXISDNodeWCHAIN(Op, DAG, YSXISD::SF_VC_V_X_SE);
  case Intrinsic::riscv_sf_vc_v_i_se:
    return getVCIXISDNodeWCHAIN(Op, DAG, YSXISD::SF_VC_V_I_SE);
  case Intrinsic::riscv_sf_vc_v_xv_se:
    return getVCIXISDNodeWCHAIN(Op, DAG, YSXISD::SF_VC_V_XV_SE);
  case Intrinsic::riscv_sf_vc_v_iv_se:
    return getVCIXISDNodeWCHAIN(Op, DAG, YSXISD::SF_VC_V_IV_SE);
  case Intrinsic::riscv_sf_vc_v_vv_se:
    return getVCIXISDNodeWCHAIN(Op, DAG, YSXISD::SF_VC_V_VV_SE);
  case Intrinsic::riscv_sf_vc_v_fv_se:
    return getVCIXISDNodeWCHAIN(Op, DAG, YSXISD::SF_VC_V_FV_SE);
  case Intrinsic::riscv_sf_vc_v_xvv_se:
    return getVCIXISDNodeWCHAIN(Op, DAG, YSXISD::SF_VC_V_XVV_SE);
  case Intrinsic::riscv_sf_vc_v_ivv_se:
    return getVCIXISDNodeWCHAIN(Op, DAG, YSXISD::SF_VC_V_IVV_SE);
  case Intrinsic::riscv_sf_vc_v_vvv_se:
    return getVCIXISDNodeWCHAIN(Op, DAG, YSXISD::SF_VC_V_VVV_SE);
  case Intrinsic::riscv_sf_vc_v_fvv_se:
    return getVCIXISDNodeWCHAIN(Op, DAG, YSXISD::SF_VC_V_FVV_SE);
  case Intrinsic::riscv_sf_vc_v_xvw_se:
    return getVCIXISDNodeWCHAIN(Op, DAG, YSXISD::SF_VC_V_XVW_SE);
  case Intrinsic::riscv_sf_vc_v_ivw_se:
    return getVCIXISDNodeWCHAIN(Op, DAG, YSXISD::SF_VC_V_IVW_SE);
  case Intrinsic::riscv_sf_vc_v_vvw_se:
    return getVCIXISDNodeWCHAIN(Op, DAG, YSXISD::SF_VC_V_VVW_SE);
  case Intrinsic::riscv_sf_vc_v_fvw_se:
    return getVCIXISDNodeWCHAIN(Op, DAG, YSXISD::SF_VC_V_FVW_SE);
  }
...
lowerFixedVectorSegStoreIntrinsics(unsigned IntNo, SDValue Op,
  ...
  for (unsigned i = 0; i < NF; i++)
    StoredVal = DAG.getNode(
        YSXISD::TUPLE_INSERT, DL, VecTupTy, StoredVal,
...
SDValue YSXTargetLowering::LowerINTRINSIC_VOID(SDValue Op,
  ...
  case Intrinsic::riscv_sf_vc_xv_se:
    return getVCIXISDNodeVOID(Op, DAG, YSXISD::SF_VC_XV_SE);
  case Intrinsic::riscv_sf_vc_iv_se:
    return getVCIXISDNodeVOID(Op, DAG, YSXISD::SF_VC_IV_SE);
  case Intrinsic::riscv_sf_vc_vv_se:
    return getVCIXISDNodeVOID(Op, DAG, YSXISD::SF_VC_VV_SE);
  case Intrinsic::riscv_sf_vc_fv_se:
    return getVCIXISDNodeVOID(Op, DAG, YSXISD::SF_VC_FV_SE);
  case Intrinsic::riscv_sf_vc_xvv_se:
    return getVCIXISDNodeVOID(Op, DAG, YSXISD::SF_VC_XVV_SE);
  case Intrinsic::riscv_sf_vc_ivv_se:
    return getVCIXISDNodeVOID(Op, DAG, YSXISD::SF_VC_IVV_SE);
  case Intrinsic::riscv_sf_vc_vvv_se:
    return getVCIXISDNodeVOID(Op, DAG, YSXISD::SF_VC_VVV_SE);
  case Intrinsic::riscv_sf_vc_fvv_se:
    return getVCIXISDNodeVOID(Op, DAG, YSXISD::SF_VC_FVV_SE);
  case Intrinsic::riscv_sf_vc_xvw_se:
    return getVCIXISDNodeVOID(Op, DAG, YSXISD::SF_VC_XVW_SE);
  case Intrinsic::riscv_sf_vc_ivw_se:
    return getVCIXISDNodeVOID(Op, DAG, YSXISD::SF_VC_IVW_SE);
  case Intrinsic::riscv_sf_vc_vvw_se:
    return getVCIXISDNodeVOID(Op, DAG, YSXISD::SF_VC_VVW_SE);
  case Intrinsic::riscv_sf_vc_fvw_se:
    return getVCIXISDNodeVOID(Op, DAG, YSXISD::SF_VC_FVW_SE);
  }
...
static unsigned getYSXVecReductionOp(unsigned ISDOpcode) {
  ...
  case ISD::VP_REDUCE_ADD:
  case ISD::VECREDUCE_ADD:
    return YSXISD::VECREDUCE_ADD_VL;
  case ISD::VP_REDUCE_UMAX:
  case ISD::VECREDUCE_UMAX:
    return YSXISD::VECREDUCE_UMAX_VL;
  case ISD::VP_REDUCE_SMAX:
  case ISD::VECREDUCE_SMAX:
    return YSXISD::VECREDUCE_SMAX_VL;
  case ISD::VP_REDUCE_UMIN:
  case ISD::VECREDUCE_UMIN:
    return YSXISD::VECREDUCE_UMIN_VL;
  case ISD::VP_REDUCE_SMIN:
  case ISD::VECREDUCE_SMIN:
    return YSXISD::VECREDUCE_SMIN_VL;
  case ISD::VP_REDUCE_AND:
  case ISD::VECREDUCE_AND:
    return YSXISD::VECREDUCE_AND_VL;
  case ISD::VP_REDUCE_OR:
  case ISD::VECREDUCE_OR:
    return YSXISD::VECREDUCE_OR_VL;
  case ISD::VP_REDUCE_XOR:
  case ISD::VECREDUCE_XOR:
    return YSXISD::VECREDUCE_XOR_VL;
  case ISD::VP_REDUCE_FADD:
    return YSXISD::VECREDUCE_FADD_VL;
  case ISD::VP_REDUCE_SEQ_FADD:
    return YSXISD::VECREDUCE_SEQ_FADD_VL;
  case ISD::VP_REDUCE_FMAX:
  case ISD::VP_REDUCE_FMAXIMUM:
    return YSXISD::VECREDUCE_FMAX_VL;
  case ISD::VP_REDUCE_FMIN:
  case ISD::VP_REDUCE_FMINIMUM:
    return YSXISD::VECREDUCE_FMIN_VL;
  }

}

SDValue YSXTargetLowering::lowerVectorMaskVecReduction(SDValue Op,
                                                         SelectionDAG &DAG,
                                                         bool IsVP) const {
  SDLoc DL(Op);
  ...
  case ISD::VP_REDUCE_AND: {
    // vcpop ~x == 0
    SDValue TrueMask = DAG.getNode(YSXISD::VMSET_VL, DL, ContainerVT, VL);
    if (IsVP || VecVT.isFixedLengthVector())
      Vec = DAG.getNode(YSXISD::VMXOR_VL, DL, ContainerVT, Vec, TrueMask, VL);
    else
      Vec = DAG.getNode(ISD::XOR, DL, ContainerVT, Vec, TrueMask);
    Vec = DAG.getNode(YSXISD::VCPOP_VL, DL, XLenVT, Vec, Mask, VL);
    CC = ISD::SETEQ;
    break;
  }
  case ISD::VECREDUCE_OR:
...
static bool isNonZeroAVL(SDValue AVL) {
  ...
  return (RegisterAVL && RegisterAVL->getReg() == YSX::X0) ||
...
static SDValue lowerReductionSeq(unsigned YSXVecOpcode, MVT ResVT,
  ...
  const MVT M1VT = YSXTargetLowering::getM1VT(VecVT);
  const MVT XLenVT = Subtarget.getXLenVT();
...
SDValue YSXTargetLowering::lowerVECREDUCE(SDValue Op,
                                            SelectionDAG &DAG) const {
  SDLoc DL(Op);
  SDValue Vec = Op.getOperand(0);
...
getYSXVecFPReductionOpAndOperands(SDValue Op, SelectionDAG &DAG, EVT EltVT,
  ...
  case ISD::VECREDUCE_FADD: {
    ...
    return std::make_tuple(YSXISD::VECREDUCE_FADD_VL, Op.getOperand(0), Zero);
  }
  case ISD::VECREDUCE_SEQ_FADD:
    return std::make_tuple(YSXISD::VECREDUCE_SEQ_FADD_VL, Op.getOperand(1),
                           Op.getOperand(0));
  case ISD::VECREDUCE_FMINIMUM:
  case ISD::VECREDUCE_FMAXIMUM:
  case ISD::VECREDUCE_FMIN:
...
SDValue YSXTargetLowering::lowerFPVECREDUCE(SDValue Op,
                                              SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  SDValue IsNan =
      DAG.getNode(YSXISD::SETCC_VL, DL, Mask.getValueType(),
                  {VectorVal, VectorVal, DAG.getCondCode(ISD::SETNE),
                   DAG.getUNDEF(Mask.getValueType()), Mask, VL});
  MVT XLenVT = Subtarget.getXLenVT();
  SDValue CPop = DAG.getNode(YSXISD::VCPOP_VL, DL, XLenVT, IsNan, Mask, VL);
...
SDValue YSXTargetLowering::lowerVPREDUCE(SDValue Op,
                                           SelectionDAG &DAG) const {
  SDLoc DL(Op);
  unsigned Opc = Op.getOpcode();
  ...
  SDValue IsNaN = DAG.getNode(
      YSXISD::SETCC_VL, DL, PredVT,
      {Vec, Vec, DAG.getCondCode(ISD::SETNE), DAG.getUNDEF(PredVT), Mask, VL});
  SDValue VCPop = DAG.getNode(YSXISD::VCPOP_VL, DL, XLenVT, IsNaN, Mask, VL);
...
SDValue YSXTargetLowering::lowerINSERT_SUBVECTOR(SDValue Op,
                                                   SelectionDAG &DAG) const {
  SDValue Vec = Op.getOperand(0);
  ...
  if (SubVecVT.isFixedLengthVector() && !VLen) {
    ...
    if (OrigIdx == 0) {
      SubVec =
          DAG.getNode(YSXISD::VMV_V_V_VL, DL, ContainerVT, Vec, SubVec, VL);
    } else {
  ...
  // vmv.v.v.
  if (RemIdx.isZero()) {
    SubVec = DAG.getNode(YSXISD::VMV_V_V_VL, DL, InterSubVT, AlignedExtract,
                         SubVec, VL);
  } else {
...
SDValue YSXTargetLowering::lowerEXTRACT_SUBVECTOR(SDValue Op,
                                                    SelectionDAG &DAG) const {
  SDValue Vec = Op.getOperand(0);
  ...
  if (VecVT.bitsGT(YSXTargetLowering::getM1VT(VecVT))) {
    ...
    assert(SubRegIdx != YSX::NoSubRegister);
    (void)SubRegIdx;
...
SDValue YSXTargetLowering::lowerVECTOR_DEINTERLEAVE(SDValue Op,
                                                      SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT VecVT = Op.getSimpleValueType();
  ...
  if (Subtarget.hasVendorXRivosVizip() && Factor == 2) {
    ...
    if (SDValue Src = foldConcatVector(V1, V2);
        Src && YSXTargetLowering::getM1VT(VT).bitsGT(VT)) {
      EVT NewVT = VT.getDoubleNumVectorElementsVT();
      Src = DAG.getExtractSubvector(DL, NewVT, Src, 0);
      // Freeze the source so we can increase its use count.
      Src = DAG.getFreeze(Src);
      SDValue Even = lowerVZIP(YSXISD::RI_VUNZIP2A_VL, Src,
                               DAG.getUNDEF(NewVT), DL, DAG, Subtarget);
      SDValue Odd = lowerVZIP(YSXISD::RI_VUNZIP2B_VL, Src,
                              DAG.getUNDEF(NewVT), DL, DAG, Subtarget);
    ...
    V2 = DAG.getFreeze(V2);
    SDValue Even =
        lowerVZIP(YSXISD::RI_VUNZIP2A_VL, V1, V2, DL, DAG, Subtarget);
    SDValue Odd =
        lowerVZIP(YSXISD::RI_VUNZIP2B_VL, V1, V2, DL, DAG, Subtarget);
    return DAG.getMergeValues({Even, Odd}, DL);
  }
  ...
  for (unsigned i = 0U; i < Factor; ++i)
    Res[i] = DAG.getNode(YSXISD::TUPLE_EXTRACT, DL, VecVT, Load,
                         DAG.getTargetConstant(i, DL, MVT::i32));

  return DAG.getMergeValues(Res, DL);
}

SDValue YSXTargetLowering::lowerVECTOR_INTERLEAVE(SDValue Op,
                                                    SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT VecVT = Op.getSimpleValueType();
  ...
  if (Factor != 2) {
    ...
    for (unsigned i = 0; i < Factor; i++)
      StoredVal =
          DAG.getNode(YSXISD::TUPLE_INSERT, DL, VecTupTy, StoredVal,
  ...
  if (Subtarget.hasVendorXRivosVizip() && !Op.getOperand(0).isUndef() &&
    ...
    SDValue Lo = lowerVZIP(YSXISD::RI_VZIP2A_VL, V1, V2, DL, DAG, Subtarget);
    SDValue Hi = lowerVZIP(YSXISD::RI_VZIP2B_VL, V1, V2, DL, DAG, Subtarget);
  ...
  if (VecVT.getScalarSizeInBits() < Subtarget.getELen()) {
    ...
    Idx =
        DAG.getNode(YSXISD::ADD_VL, DL, IdxVT, Idx, VLMax, Idx, OddMask, VL);

    // Then perform the interleave
    //   v[0]   v[n]   v[1] v[n+1]   v[2] v[n+2]   v[3] v[n+3] ...
    SDValue TrueMask = getAllOnesMask(IdxVT, VL, DL, DAG);
    Interleaved = DAG.getNode(YSXISD::VRGATHEREI16_VV_VL, DL, ConcatVT,
...
SDValue YSXTargetLowering::lowerSTEP_VECTOR(SDValue Op,
                                              SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT VT = Op.getSimpleValueType();
  ...
  SDValue StepVec = DAG.getNode(YSXISD::VID_VL, DL, VT, Mask, VL);
...
SDValue YSXTargetLowering::lowerVECTOR_REVERSE(SDValue Op,
                                                 SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT VecVT = Op.getSimpleValueType();
  if (VecVT.getVectorElementType() == MVT::i1) {
  ...
  if (ContainerVT.bitsGT(YSXTargetLowering::getM1VT(ContainerVT)) &&
  ...
  unsigned GatherOpc = YSXISD::VRGATHER_VV_VL;
  MVT IntVT = ContainerVT.changeVectorElementTypeToInteger();
  ...
  if (!IsRV32E64)
    ...
    SplatVL = DAG.getNode(YSXISD::VMV_V_X_VL, DL, IntVT, DAG.getUNDEF(IntVT),
                          VLMinus1, DAG.getRegister(YSX::X0, XLenVT));

  SDValue VID = DAG.getNode(YSXISD::VID_VL, DL, IntVT, Mask, VL);
  SDValue Indices = DAG.getNode(YSXISD::SUB_VL, DL, IntVT, SplatVL, VID,
...
SDValue YSXTargetLowering::lowerVECTOR_SPLICE(SDValue Op,
                                                SelectionDAG &DAG) const {
  SDLoc DL(Op);
  SDValue V1 = Op.getOperand(0);
  SDValue V2 = Op.getOperand(1);
...
YSXTargetLowering::lowerFixedLengthVectorLoadToYSXVec(SDValue Op,
  ...
  const auto [MinVLMAX, MaxVLMAX] =
      YSXTargetLowering::computeVLMAXBounds(ContainerVT, Subtarget);
  if (MinVLMAX == MaxVLMAX && MinVLMAX == VT.getVectorNumElements() &&
      YSXTargetLowering::getM1VT(ContainerVT).bitsLE(ContainerVT)) {
...
SDValue YSXTargetLowering::lowerMaskedLoad(SDValue Op,
                                             SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT VT = Op.getSimpleValueType();
  ...
  if (!IsUnmasked && IsExpandingLoad) {
    ExpandingVL = VL;
    VL =
        DAG.getNode(YSXISD::VCPOP_VL, DL, XLenVT, Mask,
  ...
  if (ExpandingVL) {
    ...
    Result =
        DAG.getNode(UseVRGATHEREI16 ? YSXISD::VRGATHEREI16_VV_VL
                                    : YSXISD::VRGATHER_VV_VL,
...
SDValue YSXTargetLowering::lowerLoadFF(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT VT = Op->getSimpleValueType(0);
...
SDValue YSXTargetLowering::lowerMaskedStore(SDValue Op,
                                              SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue YSXTargetLowering::lowerVectorCompress(SDValue Op,
                                                 SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue YSXTargetLowering::lowerVectorStrictFSetcc(SDValue Op,
                                                     SelectionDAG &DAG) const {
  unsigned Opc = Op.getOpcode();
  SDLoc DL(Op);
  SDValue Chain = Op.getOperand(0);
  ...
  if (Opc == ISD::STRICT_FSETCC &&
    ...
    SDValue OrderMask1 = DAG.getNode(
        YSXISD::STRICT_FSETCC_VL, DL, DAG.getVTList(MaskVT, MVT::Other),
    ...
    SDValue OrderMask2 = DAG.getNode(
        ...
        {Chain, Op2, Op2, DAG.getCondCode(ISD::SETOEQ), DAG.getUNDEF(MaskVT),
         True, VL});
    Mask =
        DAG.getNode(YSXISD::VMAND_VL, DL, MaskVT, OrderMask1, OrderMask2, VL);
    // Use Mask as the passthru operand to let the result be 0 if either of the
    // inputs is unordered.
    Res = DAG.getNode(YSXISD::STRICT_FSETCCS_VL, DL,
                      DAG.getVTList(MaskVT, MVT::Other),
...
SDValue YSXTargetLowering::lowerABS(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT VT = Op.getSimpleValueType();
  SDValue X = Op.getOperand(0);
  ...
  SDValue SplatZero = DAG.getNode(
      YSXISD::VMV_V_X_VL, DL, ContainerVT, DAG.getUNDEF(ContainerVT),
      DAG.getConstant(0, DL, Subtarget.getXLenVT()), VL);
  SDValue NegX = DAG.getNode(YSXISD::SUB_VL, DL, ContainerVT, SplatZero, X,
                             DAG.getUNDEF(ContainerVT), Mask, VL);
  SDValue Max = DAG.getNode(YSXISD::SMAX_VL, DL, ContainerVT, X, NegX,
                            DAG.getUNDEF(ContainerVT), Mask, VL);
...
SDValue YSXTargetLowering::lowerToScalableOp(SDValue Op,
                                               SelectionDAG &DAG) const {
  const auto &TSInfo =
      static_cast<const YSXSelectionDAGInfo &>(DAG.getSelectionDAGInfo());

  unsigned NewOpc = getYSXVLOp(Op);
...
// Lower a VP_* ISD node to the corresponding YSXISD::*_VL node:
...
//   types.
SDValue YSXTargetLowering::lowerVPOp(SDValue Op, SelectionDAG &DAG) const {
  const auto &TSInfo =
      static_cast<const YSXSelectionDAGInfo &>(DAG.getSelectionDAGInfo());

  unsigned YSXISDOpc = getYSXVLOp(Op);
  bool HasPassthruOp = TSInfo.hasPassthruOp(YSXISDOpc);
  ...
  for (const auto &OpIdx : enumerate(Op->ops())) {
    ...
    if (YSXISDOpc == YSXISD::VFCVT_RM_X_F_VL &&
        ISD::getVPExplicitVectorLengthIdx(Op.getOpcode()) == OpIdx.index())
      Ops.push_back(DAG.getTargetConstant(YSXFPRndMode::DYN, DL,
                                          Subtarget.getXLenVT()));
  ...
  if (!VT.isFixedLengthVector())
    return DAG.getNode(YSXISDOpc, DL, VT, Ops, Op->getFlags());

  SDValue VPOp = DAG.getNode(YSXISDOpc, DL, ContainerVT, Ops, Op->getFlags());

  return convertFromScalableVector(VT, VPOp, DAG, Subtarget);
}

SDValue YSXTargetLowering::lowerVPExtMaskOp(SDValue Op,
                                              SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT VT = Op.getSimpleValueType();
  ...
  MVT XLenVT = Subtarget.getXLenVT();
  SDValue Zero = DAG.getConstant(0, DL, XLenVT);
  SDValue ZeroSplat = DAG.getNode(YSXISD::VMV_V_X_VL, DL, ContainerVT,
                                  DAG.getUNDEF(ContainerVT), Zero, VL);

  SDValue SplatValue = DAG.getSignedConstant(
      Op.getOpcode() == ISD::VP_ZERO_EXTEND ? 1 : -1, DL, XLenVT);
  SDValue Splat = DAG.getNode(YSXISD::VMV_V_X_VL, DL, ContainerVT,
                              DAG.getUNDEF(ContainerVT), SplatValue, VL);

  SDValue Result = DAG.getNode(YSXISD::VMERGE_VL, DL, ContainerVT, Src, Splat,
...
SDValue YSXTargetLowering::lowerVPSetCCMaskOp(SDValue Op,
                                                SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT VT = Op.getSimpleValueType();
  ...
  SDValue Result;
  SDValue AllOneMask = DAG.getNode(YSXISD::VMSET_VL, DL, ContainerVT, VL);
  ...
  // X != Y  --> (X^Y)
  case ISD::SETNE:
    Result = DAG.getNode(YSXISD::VMXOR_VL, DL, ContainerVT, Op1, Op2, VL);
    break;
  // X == Y  --> ~(X^Y)
  case ISD::SETEQ: {
    SDValue Temp =
        DAG.getNode(YSXISD::VMXOR_VL, DL, ContainerVT, Op1, Op2, VL);
    Result =
        DAG.getNode(YSXISD::VMXOR_VL, DL, ContainerVT, Temp, AllOneMask, VL);
    break;
  }
  // X >s Y   -->  X == 0 & Y == 1  -->  ~X & Y
  // X <u Y   -->  X == 0 & Y == 1  -->  ~X & Y
  ...
  case ISD::SETULT: {
    SDValue Temp =
        DAG.getNode(YSXISD::VMXOR_VL, DL, ContainerVT, Op1, AllOneMask, VL);
    Result = DAG.getNode(YSXISD::VMAND_VL, DL, ContainerVT, Temp, Op2, VL);
    break;
  }
  // X <s Y   --> X == 1 & Y == 0  -->  ~Y & X
  // X >u Y   --> X == 1 & Y == 0  -->  ~Y & X
  ...
  case ISD::SETUGT: {
    SDValue Temp =
        DAG.getNode(YSXISD::VMXOR_VL, DL, ContainerVT, Op2, AllOneMask, VL);
    Result = DAG.getNode(YSXISD::VMAND_VL, DL, ContainerVT, Op1, Temp, VL);
    break;
  }
  // X >=s Y  --> X == 0 | Y == 1  -->  ~X | Y
  // X <=u Y  --> X == 0 | Y == 1  -->  ~X | Y
  ...
  case ISD::SETULE: {
    SDValue Temp =
        DAG.getNode(YSXISD::VMXOR_VL, DL, ContainerVT, Op1, AllOneMask, VL);
    Result = DAG.getNode(YSXISD::VMXOR_VL, DL, ContainerVT, Temp, Op2, VL);
    break;
  }
  // X <=s Y  --> X == 1 | Y == 0  -->  ~Y | X
  // X >=u Y  --> X == 1 | Y == 0  -->  ~Y | X
  ...
  case ISD::SETUGE: {
    SDValue Temp =
        DAG.getNode(YSXISD::VMXOR_VL, DL, ContainerVT, Op2, AllOneMask, VL);
    Result = DAG.getNode(YSXISD::VMXOR_VL, DL, ContainerVT, Temp, Op1, VL);
    break;
  }
  }
...
SDValue YSXTargetLowering::lowerVPFPIntConvOp(SDValue Op,
                                                SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  if (DstEltSize >= SrcEltSize) { // Single-width and widening conversion.
    if (SrcVT.isInteger()) {
      ...
      unsigned YSXISDExtOpc = YSXISDOpc == YSXISD::SINT_TO_FP_VL
                                    ? YSXISD::VSEXT_VL
                                    : YSXISD::VZEXT_VL;
      ...
      if (SrcEltSize == 1) {
        ...
        MVT XLenVT = Subtarget.getXLenVT();
        SDValue Zero = DAG.getConstant(0, DL, XLenVT);
        SDValue ZeroSplat = DAG.getNode(YSXISD::VMV_V_X_VL, DL, IntVT,
                                        DAG.getUNDEF(IntVT), Zero, VL);
        SDValue One = DAG.getSignedConstant(
            YSXISDExtOpc == YSXISD::VZEXT_VL ? 1 : -1, DL, XLenVT);
        SDValue OneSplat = DAG.getNode(YSXISD::VMV_V_X_VL, DL, IntVT,
                                       DAG.getUNDEF(IntVT), One, VL);
        Src = DAG.getNode(YSXISD::VMERGE_VL, DL, IntVT, Src, OneSplat,
        ...
        Src = DAG.getNode(YSXISDExtOpc, DL, IntVT, Src, Mask, VL);
      }

      Result = DAG.getNode(YSXISDOpc, DL, DstVT, Src, Mask, VL);
    } else {
      ...
      if (DstEltSize > (2 * SrcEltSize)) {
        ...
        Src =
            DAG.getNode(YSXISD::FP_EXTEND_VL, DL, InterimFVT, Src, Mask, VL);
      }
    ...
    if (SrcVT.isInteger()) {
      ...
      Result = DAG.getNode(YSXISDOpc, DL, InterimFVT, Src, Mask, VL);

      if (InterimFVT != DstVT) {
        Src = Result;
        Result = DAG.getNode(YSXISD::FP_ROUND_VL, DL, DstVT, Src, Mask, VL);
      }
    } else {
      assert(SrcVT.isFloatingPoint() && DstVT.isInteger() &&
             "Wrong input/output vector types");
      ...
      if (DstEltSize == 1) {
        ...
        MVT InterimIVT = MVT::getVectorVT(MVT::getIntegerVT(SrcEltSize),
                                          DstVT.getVectorElementCount());
        Result = DAG.getNode(YSXISDOpc, DL, InterimIVT, Src, Mask, VL);

        // Compare the integer result to 0. The integer should be 0 or 1/-1,
        // otherwise the conversion was undefined.
        MVT XLenVT = Subtarget.getXLenVT();
        SDValue SplatZero = DAG.getConstant(0, DL, XLenVT);
        SplatZero = DAG.getNode(YSXISD::VMV_V_X_VL, DL, InterimIVT,
                                DAG.getUNDEF(InterimIVT), SplatZero, VL);
        Result = DAG.getNode(YSXISD::SETCC_VL, DL, DstVT,
        ...
        while (InterimIVT != DstVT) {
          ...
          InterimIVT = MVT::getVectorVT(MVT::getIntegerVT(SrcEltSize / 2),
                                        DstVT.getVectorElementCount());
          Result = DAG.getNode(YSXISD::TRUNCATE_VECTOR_VL, DL, InterimIVT,
                               Src, Mask, VL);
        }
      }
    }
...
SDValue YSXTargetLowering::lowerVPMergeMask(SDValue Op,
                                              SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT VT = Op.getSimpleValueType();
  MVT XLenVT = Subtarget.getXLenVT();
  ...
  SDValue VLMax = DAG.getRegister(YSX::X0, XLenVT);
  SDValue SplatOne = DAG.getNode(YSXISD::VMV_V_X_VL, DL, PromotedVT,
                                 DAG.getUNDEF(PromotedVT),
                                 DAG.getConstant(1, DL, XLenVT), VLMax);
  SDValue SplatZero = DAG.getNode(YSXISD::VMV_V_X_VL, DL, PromotedVT,
                                  DAG.getUNDEF(PromotedVT),
                                  DAG.getConstant(0, DL, XLenVT), VLMax);
  TrueVal = DAG.getNode(YSXISD::VMERGE_VL, DL, PromotedVT, TrueVal, SplatOne,
                        SplatZero, DAG.getUNDEF(PromotedVT), VL);
  // Any element past VL uses FalseVal, so use VLMax
  FalseVal = DAG.getNode(YSXISD::VMERGE_VL, DL, PromotedVT, FalseVal,
                         SplatOne, SplatZero, DAG.getUNDEF(PromotedVT), VLMax);

  // VP_MERGE the two promoted values.
  SDValue VPMerge = DAG.getNode(YSXISD::VMERGE_VL, DL, PromotedVT, Mask,
                                TrueVal, FalseVal, FalseVal, VL);

  // Convert back to mask.
  SDValue TrueMask = DAG.getNode(YSXISD::VMSET_VL, DL, ContainerVT, VL);
  SDValue Result = DAG.getNode(
      YSXISD::SETCC_VL, DL, ContainerVT,
...
SDValue
YSXTargetLowering::lowerVPSpliceExperimental(SDValue Op,
                                               SelectionDAG &DAG) const {
  using namespace SDPatternMatch;
  ...
  if (IsMaskVector) {
    ...
    SDValue SplatOneOp1 = DAG.getNode(YSXISD::VMV_V_X_VL, DL, ContainerVT,
                                      DAG.getUNDEF(ContainerVT),
                                      DAG.getConstant(1, DL, XLenVT), EVL1);
    SDValue SplatZeroOp1 = DAG.getNode(YSXISD::VMV_V_X_VL, DL, ContainerVT,
                                       DAG.getUNDEF(ContainerVT),
                                       DAG.getConstant(0, DL, XLenVT), EVL1);
    Op1 = DAG.getNode(YSXISD::VMERGE_VL, DL, ContainerVT, Op1, SplatOneOp1,
                      SplatZeroOp1, DAG.getUNDEF(ContainerVT), EVL1);

    SDValue SplatOneOp2 = DAG.getNode(YSXISD::VMV_V_X_VL, DL, ContainerVT,
                                      DAG.getUNDEF(ContainerVT),
                                      DAG.getConstant(1, DL, XLenVT), EVL2);
    SDValue SplatZeroOp2 = DAG.getNode(YSXISD::VMV_V_X_VL, DL, ContainerVT,
                                       DAG.getUNDEF(ContainerVT),
                                       DAG.getConstant(0, DL, XLenVT), EVL2);
    Op2 = DAG.getNode(YSXISD::VMERGE_VL, DL, ContainerVT, Op2, SplatOneOp2,
  ...
  if (!IsMaskVector && isNullConstant(Offset) && isOneConstant(EVL1))
    if (auto FirstEle = getVectorFirstEle(Op->getOperand(0))) {
      ...
      Result = DAG.getNode(EltVT.isFloatingPoint() ? YSXISD::VFSLIDE1UP_VL
                                                   : YSXISD::VSLIDE1UP_VL,
  ...
  if (IsMaskVector) {
    ...
    Result = DAG.getNode(
        YSXISD::SETCC_VL, DL, ContainerVT.changeVectorElementType(MVT::i1),
...
SDValue
YSXTargetLowering::lowerVPReverseExperimental(SDValue Op,
                                                SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT VT = Op.getSimpleValueType();
  MVT XLenVT = Subtarget.getXLenVT();
  ...
  if (IsMaskVector) {
    ...
    SDValue SplatOne = DAG.getNode(YSXISD::VMV_V_X_VL, DL, IndicesVT,
                                   DAG.getUNDEF(IndicesVT),
                                   DAG.getConstant(1, DL, XLenVT), EVL);
    SDValue SplatZero = DAG.getNode(YSXISD::VMV_V_X_VL, DL, IndicesVT,
                                    DAG.getUNDEF(IndicesVT),
                                    DAG.getConstant(0, DL, XLenVT), EVL);
    Op1 = DAG.getNode(YSXISD::VMERGE_VL, DL, IndicesVT, Op1, SplatOne,
  ...
  SDValue VID = DAG.getNode(YSXISD::VID_VL, DL, IndicesVT, Mask, EVL);
  SDValue VecLen =
      DAG.getNode(ISD::SUB, DL, XLenVT, EVL, DAG.getConstant(1, DL, XLenVT));
  SDValue VecLenSplat = DAG.getNode(YSXISD::VMV_V_X_VL, DL, IndicesVT,
                                    DAG.getUNDEF(IndicesVT), VecLen, EVL);
  SDValue VRSUB = DAG.getNode(YSXISD::SUB_VL, DL, IndicesVT, VecLenSplat, VID,
...
SDValue YSXTargetLowering::lowerLogicVPOp(SDValue Op,
                                            SelectionDAG &DAG) const {
  MVT VT = Op.getSimpleValueType();
  if (VT.getVectorElementType() != MVT::i1)
    return lowerVPOp(Op, DAG);
  ...
  SDLoc DL(Op);
  SDValue Val = DAG.getNode(getYSXVLOp(Op), DL, ContainerVT, Op1, Op2, VL);
  if (!IsFixed)
    return Val;
  return convertFromScalableVector(VT, Val, DAG, Subtarget);
}

SDValue YSXTargetLowering::lowerVPStridedLoad(SDValue Op,
                                                SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT XLenVT = Subtarget.getXLenVT();
  MVT VT = Op.getSimpleValueType();
...
SDValue YSXTargetLowering::lowerVPStridedStore(SDValue Op,
                                                 SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT XLenVT = Subtarget.getXLenVT();
...
// accordingly.
SDValue YSXTargetLowering::lowerMaskedGather(SDValue Op,
                                               SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT VT = Op.getSimpleValueType();
...
// signed or scaled indexing is extended to the XLEN value type and scaled
// accordingly.
SDValue YSXTargetLowering::lowerMaskedScatter(SDValue Op,
                                                SelectionDAG &DAG) const {
  SDLoc DL(Op);
  const auto *MemSD = cast<MemSDNode>(Op.getNode());
  EVT MemVT = MemSD->getMemoryVT();
...
SDValue YSXTargetLowering::lowerGET_ROUNDING(SDValue Op,
                                               SelectionDAG &DAG) const {
  const MVT XLenVT = Subtarget.getXLenVT();
  SDLoc DL(Op);
  SDValue Chain = Op->getOperand(0);
...
void YSXTargetLowering::ReplaceNodeResults(SDNode *N,
  ...
  case ISD::FP_TO_UINT: {
    ...
    if (getTypeAction(*DAG.getContext(), Op0.getValueType()) !=
      ...
      if (IsStrict) {
        ...
        unsigned Opc = IsSigned ? YSXISD::STRICT_FCVT_W_RV64
                                : YSXISD::STRICT_FCVT_WU_RV64;
        SDVTList VTs = DAG.getVTList(MVT::i64, MVT::Other);
        SDValue Res = DAG.getNode(
            Opc, DL, VTs, Chain, Op0,
            DAG.getTargetConstant(YSXFPRndMode::RTZ, DL, MVT::i64));
      ...
      unsigned Opc = IsSigned ? YSXISD::FCVT_W_RV64 : YSXISD::FCVT_WU_RV64;
      SDValue Res =
  ...
  case ISD::LROUND: {
    ...
    if (getTypeAction(*DAG.getContext(), Op0.getValueType()) !=
      ...
      SDValue Res =
          DAG.getNode(YSXISD::FCVT_W_RV64, DL, MVT::i64, Op0,
                      DAG.getTargetConstant(YSXFPRndMode::RMM, DL, MVT::i64));
      Results.push_back(DAG.getNode(ISD::TRUNCATE, DL, MVT::i32, Res));
      return;
    }
  ...
  case ISD::ABS: {
    ...
    if (Subtarget.hasStdExtP()) {
      SDValue Src =
          DAG.getNode(ISD::ANY_EXTEND, DL, MVT::i64, N->getOperand(0));
      SDValue Abs = DAG.getNode(YSXISD::ABSW, DL, MVT::i64, Src);
    ...
    if (Subtarget.hasStdExtZbb()) {
      ...
      SDValue Src = DAG.getNode(ISD::SIGN_EXTEND, DL, MVT::i64,
                                N->getOperand(0));
      SDValue Abs = DAG.getNode(YSXISD::NEGW_MAX, DL, MVT::i64, Src);
      Results.push_back(DAG.getNode(ISD::TRUNCATE, DL, MVT::i32, Abs));
      return;
    }
  ...
  case ISD::BITCAST: {
    ...
    if (VT == MVT::i16 &&
      ...
      SDValue FPConv = DAG.getNode(YSXISD::FMV_X_ANYEXTH, DL, XLenVT, Op0);
      Results.push_back(DAG.getNode(ISD::TRUNCATE, DL, MVT::i16, FPConv));
    } else if (VT == MVT::i32 && Op0VT == MVT::f32 && Subtarget.is64Bit() &&
               Subtarget.hasStdExtFOrZfinx()) {
      SDValue FPConv =
          DAG.getNode(YSXISD::FMV_X_ANYEXTW_RV64, DL, MVT::i64, Op0);
      Results.push_back(DAG.getNode(ISD::TRUNCATE, DL, MVT::i32, FPConv));
    } else if (VT == MVT::i64 && Op0VT == MVT::f64 && !Subtarget.is64Bit() &&
               Subtarget.hasStdExtDOrZdinx()) {
      SDValue NewReg = DAG.getNode(YSXISD::SplitF64, DL,
  ...
  case ISD::BITREVERSE: {
    ...
    SDValue NewRes = DAG.getNode(YSXISD::BREV8, DL, XLenVT, NewOp);
  ...
  case YSXISD::ORC_B: {
    ...
    assert((VT == MVT::i16 || (VT == MVT::i32 && Subtarget.is64Bit())) &&
           "Unexpected custom legalisation");
    assert(((N->getOpcode() == YSXISD::BREV8 && Subtarget.hasStdExtZbkb()) ||
            (N->getOpcode() == YSXISD::ORC_B && Subtarget.hasStdExtZbb())) &&
  ...
  case YSXISD::PMULHRSU: {
    ...
    [[maybe_unused]] bool IsMulH =
        Opcode == YSXISD::PMULHSU || Opcode == YSXISD::PMULHR ||
        Opcode == YSXISD::PMULHRU || Opcode == YSXISD::PMULHRSU;
  ...
  case ISD::EXTRACT_VECTOR_ELT: {
    ...
    SDValue EltLo = DAG.getNode(YSXISD::VMV_X_S, DL, XLenVT, Vec);
    ...
    SDValue ThirtyTwoV = DAG.getNode(YSXISD::VMV_V_X_VL, DL, ContainerVT,
                                     DAG.getUNDEF(ContainerVT),
                                     DAG.getConstant(32, DL, XLenVT), VL);
    SDValue LShr32 =
        DAG.getNode(YSXISD::SRL_VL, DL, ContainerVT, Vec, ThirtyTwoV,
                    DAG.getUNDEF(ContainerVT), Mask, VL);

    SDValue EltHi = DAG.getNode(YSXISD::VMV_X_S, DL, XLenVT, LShr32);
  ...
  case ISD::INTRINSIC_WO_CHAIN: {
    ...
    case Intrinsic::riscv_moprr: {
      ...
      SDValue NewOp1 =
          DAG.getNode(ISD::ANY_EXTEND, DL, MVT::i64, N->getOperand(2));
      SDValue Res = DAG.getNode(
          YSXISD::MOP_RR, DL, MVT::i64, NewOp0, NewOp1,
    ...
    case Intrinsic::riscv_clmul: {
      ...
      SDValue NewOp1 =
          DAG.getNode(ISD::ANY_EXTEND, DL, MVT::i64, N->getOperand(2));
      SDValue Res = DAG.getNode(YSXISD::CLMUL, DL, MVT::i64, NewOp0, NewOp1);
      Results.push_back(DAG.getNode(ISD::TRUNCATE, DL, MVT::i32, Res));
      return;
    }
    case Intrinsic::riscv_clmulh:
    case Intrinsic::riscv_clmulr: {
      ...
      NewOp1 = DAG.getNode(ISD::SHL, DL, MVT::i64, NewOp1,
                           DAG.getConstant(32, DL, MVT::i64));
      unsigned Opc = IntNo == Intrinsic::riscv_clmulh ? YSXISD::CLMULH
                                                      : YSXISD::CLMULR;
    ...
    case Intrinsic::riscv_vmv_x_s: {
      ...
      if (VT.bitsLT(XLenVT)) {
        ...
        SDValue Extract = DAG.getNode(YSXISD::VMV_X_S, DL,
      ...
      SDValue ThirtyTwoV =
          DAG.getNode(YSXISD::VMV_V_X_VL, DL, VecVT, DAG.getUNDEF(VecVT),
                      DAG.getConstant(32, DL, XLenVT), VL);
      SDValue LShr32 = DAG.getNode(YSXISD::SRL_VL, DL, VecVT, Vec, ThirtyTwoV,
...
static SDValue combineBinOpToReduce(SDNode *N, SelectionDAG &DAG,
  ...
  if (ScalarV.getOpcode() != YSXISD::VFMV_S_F_VL &&
      ScalarV.getOpcode() != YSXISD::VMV_S_X_VL &&
      ScalarV.getOpcode() != YSXISD::VMV_V_X_VL)
    return SDValue();
...
static SDValue transformAddShlImm(SDNode *N, SelectionDAG &DAG,
  ...
  SDValue SHADD = DAG.getNode(YSXISD::SHL_ADD, DL, VT, NL,
...
static SDValue combineShlAddIAddImpl(SDNode *N, SDValue AddI, SDValue Other,
  ...
  SDValue SHADD = DAG.getNode(YSXISD::SHL_ADD, DL, VT, SHLVal->getOperand(0),
...
static SDValue combineSelectAndUse(SDNode *N, SDValue Slct, SDValue OtherOp,
  ...
  if ((Slct.getOpcode() != ISD::SELECT &&
       Slct.getOpcode() != YSXISD::SELECT_CC) ||
      !Slct.hasOneUse())
    return SDValue();
  ...
  bool SwapSelectOps;
  unsigned OpOffset = Slct.getOpcode() == YSXISD::SELECT_CC ? 2 : 0;
  ...
  if (Slct.getOpcode() == YSXISD::SELECT_CC)
    return DAG.getNode(YSXISD::SELECT_CC, SDLoc(N), VT,
...
static SDValue combineSubShiftToOrcB(SDNode *N, SelectionDAG &DAG,
  ...
  return DAG.getNode(YSXISD::ORC_B, SDLoc(N), VT, LeftShiftOperand);
}
...
static SDValue performTRUNCATECombine(SDNode *N, SelectionDAG &DAG,
  ...
  // type legalization, we'll create YSXISD::SRLW and we can't recover it
...
static SDValue combineOrOfCZERO(SDNode *N, SDValue N0, SDValue N1,
  ...
  if (N0.getOpcode() != YSXISD::CZERO_EQZ ||
      N1.getOpcode() != YSXISD::CZERO_NEZ ||
      !N0.hasOneUse() || !N1.hasOneUse())
    return SDValue();
  ...
  SDValue NewN0 = DAG.getNode(YSXISD::CZERO_EQZ, DL, VT, TrueV.getOperand(0),
                              Cond);
  SDValue NewN1 =
      DAG.getNode(YSXISD::CZERO_NEZ, DL, VT, FalseV.getOperand(0), Cond);
  SDValue NewOr =
...
static SDValue combineXorToBitfieldInsert(SDNode *N, SelectionDAG &DAG,
  ...
  return DAG.getNode(YSXISD::QC_INSB, DL, MVT::i32, Ops);
}
...
static SDValue performXORCombine(SDNode *N, SelectionDAG &DAG,
  ...
  // YSXISD:::SLLW and we can't recover it to use a BSET instruction.
  if (Subtarget.is64Bit() && Subtarget.hasStdExtZbs() &&
  ...
  const TargetLowering &TLI = DAG.getTargetLoweringInfo();
  if (N0.getOpcode() == YSXISD::SLLW &&
      isAllOnesConstant(N1) && isOneConstant(N0.getOperand(0)) &&
      TLI.isOperationLegal(ISD::ROTL, MVT::i64)) {
    SDLoc DL(N);
    return DAG.getNode(YSXISD::ROLW, DL, MVT::i64,
...
static SDValue getShlAddShlAdd(SDNode *N, SelectionDAG &DAG, unsigned ShX,
  ...
  SDValue ShlAdd = DAG.getNode(YSXISD::SHL_ADD, DL, VT, X,
                               DAG.getTargetConstant(ShY, DL, VT), X);
  if (ShX != 0)
    ShlAdd = DAG.getNode(YSXISD::SHL_ADD, DL, VT, ShlAdd,
...
static SDValue expandMulToShlAddShlAdd(SDNode *N, SelectionDAG &DAG,
  ...
  if (int ShY = isShifted359(MulAmt - 1, ShX)) {
    ...
    if (Shift == 0) {
      ...
      SDValue ShlAdd = DAG.getNode(YSXISD::SHL_ADD, DL, VT, Shl,
...
static SDValue expandMul(SDNode *N, SelectionDAG &DAG,
  ...
  if (Subtarget.hasShlAdd(3)) {
    ...
    if (Shift >= 1 && Shift <= 3 && isPowerOf2_64(MulAmt & (MulAmt - 1))) {
      ...
      return DAG.getNode(YSXISD::SHL_ADD, DL, VT, X,
    ...
    if (MulAmt > 2 && isPowerOf2_64((MulAmt - 1) & (MulAmt - 2))) {
      ...
      if (ScaleShift >= 1 && ScaleShift < 4) {
        ...
        return DAG.getNode(
            ...
            DAG.getNode(YSXISD::SHL_ADD, DL, VT, X,
...
performSIGN_EXTEND_INREGCombine(SDNode *N, TargetLowering::DAGCombinerInfo &DCI,
  ...
  if (Opc == YSXISD::FMV_X_ANYEXTH && SrcVT.bitsGE(MVT::i16) &&
      Subtarget.hasStdExtZfhmin())
    return DAG.getNode(YSXISD::FMV_X_SIGNEXTH, DL, VT, Src.getOperand(0));
  ...
  if (Opc == ISD::SHL && Subtarget.is64Bit() && SrcVT == MVT::i32 &&
    ...
    return DAG.getNode(YSXISD::SLLW, DL, VT, Src.getOperand(0),
                       Src.getOperand(1));
...
struct NodeExtensionHelper {
  ...
  bool isSplat() const {
    return OrigOperand.getOpcode() == YSXISD::VMV_V_X_VL ||
  ...
  unsigned getExtOpc(ExtKind SupportsExt) const {
    switch (SupportsExt) {
    case ExtKind::SExt:
      return YSXISD::VSEXT_VL;
    case ExtKind::ZExt:
      return YSXISD::VZEXT_VL;
    case ExtKind::FPExt:
    case ExtKind::BF16Ext:
      return YSXISD::FP_EXTEND_VL;
    }
  ...
  SDValue getOrCreateExtendedOp(SDNode *Root, SelectionDAG &DAG,
    ...
    case YSXISD::VMV_V_X_VL:
      return DAG.getNode(YSXISD::VMV_V_X_VL, DL, NarrowVT,
    ...
    case YSXISD::VFMV_V_F_VL:
      ...
      return DAG.getNode(YSXISD::VFMV_V_F_VL, DL, NarrowVT,
  ...
  static unsigned getSExtOpcode(unsigned Opcode) {
    ...
    case ISD::ADD:
    case YSXISD::ADD_VL:
    case YSXISD::VWADD_W_VL:
    case YSXISD::VWADDU_W_VL:
    case ISD::OR:
    case YSXISD::OR_VL:
      return YSXISD::VWADD_VL;
    case ISD::SUB:
    case YSXISD::SUB_VL:
    case YSXISD::VWSUB_W_VL:
    case YSXISD::VWSUBU_W_VL:
      return YSXISD::VWSUB_VL;
    case ISD::MUL:
    case YSXISD::MUL_VL:
      return YSXISD::VWMUL_VL;
    default:
      llvm_unreachable("Unexpected opcode");
    }
  }
  ...
  static unsigned getZExtOpcode(unsigned Opcode) {
    ...
    case ISD::OR:
    case YSXISD::OR_VL:
      return YSXISD::VWADDU_VL;
    case ISD::SUB:
    case YSXISD::SUB_VL:
    case YSXISD::VWSUB_W_VL:
    case YSXISD::VWSUBU_W_VL:
      return YSXISD::VWSUBU_VL;
    case ISD::MUL:
    case YSXISD::MUL_VL:
      return YSXISD::VWMULU_VL;
    case ISD::SHL:
    case YSXISD::SHL_VL:
      return YSXISD::VWSLL_VL;
    default:
      llvm_unreachable("Unexpected opcode");
    }
  }
  ...
  static unsigned getFPExtOpcode(unsigned Opcode) {
    switch (Opcode) {
    case YSXISD::FADD_VL:
    case YSXISD::VFWADD_W_VL:
      return YSXISD::VFWADD_VL;
    case YSXISD::FSUB_VL:
    case YSXISD::VFWSUB_W_VL:
      return YSXISD::VFWSUB_VL;
    case YSXISD::FMUL_VL:
      return YSXISD::VFWMUL_VL;
    case YSXISD::VFMADD_VL:
      return YSXISD::VFWMADD_VL;
    case YSXISD::VFMSUB_VL:
      return YSXISD::VFWMSUB_VL;
    case YSXISD::VFNMADD_VL:
      return YSXISD::VFWNMADD_VL;
    case YSXISD::VFNMSUB_VL:
      return YSXISD::VFWNMSUB_VL;
    default:
      llvm_unreachable("Unexpected opcode");
    }
  }
  ...
  static unsigned getSUOpcode(unsigned Opcode) {
    assert((Opcode == YSXISD::MUL_VL || Opcode == ISD::MUL) &&
           "SU is only supported for MUL");
    return YSXISD::VWMULSU_VL;
  }
  ...
  static unsigned getWOpcode(unsigned Opcode, ExtKind SupportsExt) {
    ...
    case ISD::OR:
    case YSXISD::OR_VL:
      return SupportsExt == ExtKind::SExt ? YSXISD::VWADD_W_VL
                                          : YSXISD::VWADDU_W_VL;
    case ISD::SUB:
    case YSXISD::SUB_VL:
      return SupportsExt == ExtKind::SExt ? YSXISD::VWSUB_W_VL
                                          : YSXISD::VWSUBU_W_VL;
    case YSXISD::FADD_VL:
      return YSXISD::VFWADD_W_VL;
    case YSXISD::FSUB_VL:
      return YSXISD::VFWSUB_W_VL;
    default:
      llvm_unreachable("Unexpected opcode");
    }
  }
  ...
  void fillUpExtensionSupportForSplat(SDNode *Root, SelectionDAG &DAG,
    ...
    assert((Opc == ISD::SPLAT_VECTOR || Opc == YSXISD::VMV_V_X_VL) &&
           "Unexpected Opcode");

    // The pasthru must be undef for tail agnostic.
    if (Opc == YSXISD::VMV_V_X_VL && !OrigOperand.getOperand(0).isUndef())
      return;
    ...
    if (ScalarBits < EltBits) {
      ...
      assert(Opc == YSXISD::VMV_V_X_VL && EltBits == 64 && ScalarBits == 32 &&
  ...
  bool isSupportedFPExtend(MVT NarrowEltVT, const YSXSubtarget &Subtarget) {
    return (NarrowEltVT == MVT::f32 ||
            (NarrowEltVT == MVT::f16 && Subtarget.hasVInstructionsF16()));
  }

  bool isSupportedBF16Extend(MVT NarrowEltVT, const YSXSubtarget &Subtarget) {
  ...
  getSupportedFoldings(const SDNode *Root, const YSXSubtarget &Subtarget);
};
...
struct CombineResult {
  ...
  SDValue materialize(SelectionDAG &DAG,
                      const YSXSubtarget &Subtarget) const {
...
NodeExtensionHelper::getSupportedFoldings(const SDNode *Root,
  ...
  case YSXISD::VFNMSUB_VL:
    Strategies.push_back(canFoldToVWWithSameExtension);
    if (Subtarget.hasStdExtZvfbfa() && Root->getOpcode() != YSXISD::FMUL_VL)
      ...
      Strategies.push_back(canFoldToVWWithSameExtBF16);
             ...
             Root->getOpcode() == YSXISD::VFMADD_VL)
      Strategies.push_back(canFoldToVWWithSameExtBF16);
    break;
  case ISD::MUL:
  case YSXISD::MUL_VL:
...
static SDValue simplifyOp_VL(SDNode *N) {
  ...
  assert(N->getOpcode() == YSXISD::ADD_VL);
...
static SDValue combineVWADDSUBWSelect(SDNode *N, SelectionDAG &DAG) {
  unsigned Opc = N->getOpcode();
  assert(Opc == YSXISD::VWADD_W_VL || Opc == YSXISD::VWADDU_W_VL ||
         Opc == YSXISD::VWSUB_W_VL || Opc == YSXISD::VWSUBU_W_VL);

  SDValue Y = N->getOperand(0);
  SDValue MergeOp = N->getOperand(1);
  unsigned MergeOpc = MergeOp.getOpcode();

  if (MergeOpc != YSXISD::VMERGE_VL && MergeOpc != ISD::VSELECT)
    return SDValue();
  ...
  if (Mask.getOpcode() != YSXISD::VMSET_VL)
    return SDValue();
...
static SDValue tryMemPairCombine(SelectionDAG &DAG, LSBaseSDNode *LSNode1,
  ...
  if (LSNode1->getOpcode() == ISD::LOAD) {
    ...
    unsigned Opcode;
    if (MemVT == MVT::i32)
      Opcode = (Ext == ISD::ZEXTLOAD) ? YSXISD::TH_LWUD : YSXISD::TH_LWD;
    else
      Opcode = YSXISD::TH_LDD;
    ...
    unsigned Opcode = (MemVT == MVT::i32) ? YSXISD::TH_SWD : YSXISD::TH_SDD;
...
static SDValue performFP_TO_INTCombine(SDNode *N,
  ...
  YSXFPRndMode::RoundingMode FRM = matchRoundingOp(Src.getOpcode());
  ...
  if (VT.isVector() && TLI.isTypeLegal(VT)) {
    ...
    if (FRM == YSXFPRndMode::RTZ) {
      ...
      unsigned Opc =
          IsSigned ? YSXISD::VFCVT_RM_X_F_VL : YSXISD::VFCVT_RM_XU_F_VL;
...
static SDValue performBITREVERSECombine(SDNode *N, SelectionDAG &DAG,
  ...
  SDLoc DL(N);
  return DAG.getNode(YSXISD::BREV8, DL, VT, Src.getOperand(0));
}
...
static SDValue performVP_TRUNCATECombine(SDNode *N, SelectionDAG &DAG,
  ...
  // mode.
  SDLoc DL(N);
  return DAG.getNode(YSXISD::AVGCEILU_VL, DL, VT,
...
static unsigned negateFMAOpcode(unsigned Opcode, bool NegMul, bool NegAcc) {
  ...
  if (NegMul) {
    ...
    switch (Opcode) {
    default: llvm_unreachable("Unexpected opcode");
    case YSXISD::VFMADD_VL:  Opcode = YSXISD::VFNMSUB_VL; break;
    case YSXISD::VFNMSUB_VL: Opcode = YSXISD::VFMADD_VL;  break;
    case YSXISD::VFNMADD_VL: Opcode = YSXISD::VFMSUB_VL;  break;
    case YSXISD::VFMSUB_VL:  Opcode = YSXISD::VFNMADD_VL; break;
    case YSXISD::STRICT_VFMADD_VL:  Opcode = YSXISD::STRICT_VFNMSUB_VL; break;
    case YSXISD::STRICT_VFNMSUB_VL: Opcode = YSXISD::STRICT_VFMADD_VL;  break;
    case YSXISD::STRICT_VFNMADD_VL: Opcode = YSXISD::STRICT_VFMSUB_VL;  break;
    case YSXISD::STRICT_VFMSUB_VL:  Opcode = YSXISD::STRICT_VFNMADD_VL; break;
    }
    // clang-format on
  }
  ...
  if (NegAcc) {
    ...
    switch (Opcode) {
    default: llvm_unreachable("Unexpected opcode");
    case YSXISD::VFMADD_VL:  Opcode = YSXISD::VFMSUB_VL;  break;
    case YSXISD::VFMSUB_VL:  Opcode = YSXISD::VFMADD_VL;  break;
    case YSXISD::VFNMADD_VL: Opcode = YSXISD::VFNMSUB_VL; break;
    case YSXISD::VFNMSUB_VL: Opcode = YSXISD::VFNMADD_VL; break;
    case YSXISD::STRICT_VFMADD_VL:  Opcode = YSXISD::STRICT_VFMSUB_VL;  break;
    case YSXISD::STRICT_VFMSUB_VL:  Opcode = YSXISD::STRICT_VFMADD_VL;  break;
    case YSXISD::STRICT_VFNMADD_VL: Opcode = YSXISD::STRICT_VFNMSUB_VL; break;
    case YSXISD::STRICT_VFNMSUB_VL: Opcode = YSXISD::STRICT_VFNMADD_VL; break;
    }
    // clang-format on
  }
...
static SDValue combineVFMADD_VLWithVFNEG_VL(SDNode *N, SelectionDAG &DAG) {
  ...
  auto invertIfNegative = [&Mask, &VL](SDValue &V) {
    if (V.getOpcode() == YSXISD::FNEG_VL && V.getOperand(1) == Mask &&
        V.getOperand(2) == VL) {
...
static bool combine_CC(SDValue &LHS, SDValue &RHS, SDValue &CC, const SDLoc &DL,
  ...
  auto onlyUsedBySelectOrBR = [](const SDValue &Op) -> bool {
    for (const SDNode *UserNode : Op->users()) {
      ...
      if (Opcode != YSXISD::SELECT_CC && Opcode != YSXISD::BR_CC)
        return false;
    }
    return true;
  };
...
static SDValue combineToVWMACC(SDNode *N, SelectionDAG &DAG,
                               const YSXSubtarget &Subtarget) {
  assert(N->getOpcode() == YSXISD::ADD_VL || N->getOpcode() == ISD::ADD);
...
static SDValue combineVqdotAccum(SDNode *N, SelectionDAG &DAG,
  ...
  auto IsVqdotqOpc = [](unsigned Opc) {
    switch (Opc) {
    case YSXISD::VQDOT_VL:
    case YSXISD::VQDOTU_VL:
    case YSXISD::VQDOTSU_VL:
      return true;
    default:
      return false;
    }
  ...
  if (AddMask.getOpcode() != YSXISD::VMSET_VL ||
  ...
  SDLoc DL(N);
  EVT VT = N->getValueType(0);
  Addend = DAG.getNode(YSXISD::ADD_VL, DL, VT, Addend, AccumOp,
...
legalizeScatterGatherIndexType(SDLoc DL, SDValue &Index,
                               ...
                               YSXTargetLowering::DAGCombinerInfo &DCI) {
  if (!DCI.isBeforeLegalize())
    return false;

  SelectionDAG &DAG = DCI.DAG;
  const MVT XLenVT =
    DAG.getMachineFunction().getSubtarget<YSXSubtarget>().getXLenVT();
...
static bool matchIndexAsWiderOp(EVT VT, SDValue Index, SDValue Mask,
                                Align BaseAlign, const YSXSubtarget &ST) {
  if (!ISD::isConstantSplatVectorAllOnes(Mask.getNode()))
    return false;
  if (!ISD::isBuildVectorOfConstantSDNodes(Index.getNode()))
    return false;
...
static SDValue combineTruncOfSraSext(SDNode *N, SelectionDAG &DAG) {
  ...
  bool IsVLMAX = isAllOnesConstant(VL) ||
                 (isa<RegisterSDNode>(VL) &&
                  cast<RegisterSDNode>(VL)->getReg() == YSX::X0);
  if (!IsVLMAX || Mask.getOpcode() != YSXISD::VMSET_VL ||
      Mask.getOperand(0) != VL)
    return SDValue();

  auto IsTruncNode = [&](SDValue V) {
    return V.getOpcode() == YSXISD::TRUNCATE_VECTOR_VL &&
...
static SDValue combineTruncToVnclip(SDNode *N, SelectionDAG &DAG,
                                    const YSXSubtarget &Subtarget) {
  assert(N->getOpcode() == YSXISD::TRUNCATE_VECTOR_VL);
  ...
  auto MatchMinMax = [&VL, &Mask](SDValue V, unsigned Opc, unsigned OpcVL,
    ...
    if (Op.getOpcode() == YSXISD::VMV_V_X_VL && Op.getOperand(0).isUndef() &&
        Op.getOperand(2) == VL) {
  ...
  auto DetectUSatPattern = [&](SDValue V) {
    ...
    if (SDValue UMinOp = MatchMinMax(V, ISD::UMIN, YSXISD::UMIN_VL, HiC))
    ...
    if (SDValue SMinOp = MatchMinMax(V, ISD::SMIN, YSXISD::SMIN_VL, HiC))
      if (SDValue SMaxOp =
              MatchMinMax(SMinOp, ISD::SMAX, YSXISD::SMAX_VL, LoC))
    ...
    // first.
    if (SDValue SMaxOp = MatchMinMax(V, ISD::SMAX, YSXISD::SMAX_VL, LoC))
      if (SDValue SMinOp =
              MatchMinMax(SMaxOp, ISD::SMIN, YSXISD::SMIN_VL, HiC))
        if (LoC.isNonNegative() && HiC.isMask(VT.getScalarSizeInBits()) &&
            HiC.uge(LoC))
          return DAG.getNode(YSXISD::SMAX_VL, DL, V.getValueType(), SMinOp,
  ...
  while (Src.getOpcode() == YSXISD::TRUNCATE_VECTOR_VL &&
  ...
  if ((Val = DetectUSatPattern(Src)))
    ClipOpc = YSXISD::TRUNCATE_VECTOR_VL_USAT;
  else if ((Val = DetectSSatPattern(Src)))
    ClipOpc = YSXISD::TRUNCATE_VECTOR_VL_SSAT;
  else
    return SDValue();
...
static SDValue combineToVCPOP(SDNode *N, SelectionDAG &DAG,
  ...
  if (Opc == ISD::VECREDUCE_ADD) {
    ...
    unsigned MaxVLMAX = SrcMVT.isFixedLengthVector()
                            ...
                            : YSXTargetLowering::computeVLMAX(
  ...
  MVT XLenVT = Subtarget.getXLenVT();
  SDValue Pop = DAG.getNode(YSXISD::VCPOP_VL, DL, XLenVT, Src, Mask, VL);
...
static SDValue performSHLCombine(SDNode *N,
  ...
  case ISD::SIGN_EXTEND:
  case YSXISD::VSEXT_VL:
    Opcode = YSXISD::VWMULSU_VL;
    break;
  case ISD::ZERO_EXTEND:
  case YSXISD::VZEXT_VL:
    Opcode = YSXISD::VWMULU_VL;
    break;
  default:
    return SDValue();
  }
  ...
  else if (RHS.getOpcode() == YSXISD::VMV_V_X_VL &&
...
SDValue YSXTargetLowering::PerformDAGCombine(SDNode *N,
  ...
  case YSXISD::SplitF64: {
    ...
    if (Op0->getOpcode() == YSXISD::BuildPairF64)
    ...
    if (!(Op0.getOpcode() == ISD::FNEG || Op0.getOpcode() == ISD::FABS) ||
      ...
      break;
    SDValue NewSplitF64 =
        DAG.getNode(YSXISD::SplitF64, DL, DAG.getVTList(MVT::i32, MVT::i32),
                    Op0.getOperand(0));
  ...
  case YSXISD::FMV_W_X_RV64: {
    ...
    SDValue Op0 = N->getOperand(0);
    if (Op0.getOpcode() == YSXISD::FMV_X_ANYEXTW_RV64)
      return Op0.getOperand(0);
    break;
  }
  case YSXISD::FMV_X_ANYEXTH:
  case YSXISD::FMV_X_ANYEXTW_RV64: {
    SDLoc DL(N);
    SDValue Op0 = N->getOperand(0);
    MVT VT = N->getSimpleValueType(0);
    ...
    if ((N->getOpcode() == YSXISD::FMV_X_ANYEXTW_RV64 &&
         Op0->getOpcode() == YSXISD::FMV_W_X_RV64) ||
        (N->getOpcode() == YSXISD::FMV_X_ANYEXTH &&
         Op0->getOpcode() == YSXISD::FMV_H_X)) {
    ...
    unsigned FPBits = N->getOpcode() == YSXISD::FMV_X_ANYEXTW_RV64 ? 32 : 16;
  ...
  case ISD::FMUL: {
    ...
    if (Sign.getValueType() != VT)
      return SDValue();
    return DAG.getNode(YSXISD::FSGNJX, DL, VT, N1, N0->getOperand(1));
  }
  case ISD::FADD:
  case ISD::UMAX:
  case ISD::UMIN:
  ...
  case YSXISD::CZERO_EQZ:
  case YSXISD::CZERO_NEZ: {
    ...
    // czero_eqz x, x -> x
    if (Opc == YSXISD::CZERO_EQZ && Val == Cond)
      return Val;

    unsigned InvOpc =
        Opc == YSXISD::CZERO_EQZ ? YSXISD::CZERO_NEZ : YSXISD::CZERO_EQZ;
  ...
  case YSXISD::SELECT_CC: {
    ...
    if (combine_CC(LHS, RHS, CC, DL, DAG, Subtarget))
      return DAG.getNode(YSXISD::SELECT_CC, DL, N->getValueType(0),
    ...
    if (!Subtarget.hasConditionalMoveFusion()) {
      ...
      // (ysxisd::select_cc x, 0, ne, x, 1) -> (add x, (setcc x, 0, eq))
      // (ysxisd::select_cc x, 0, eq, 1, x) -> (add x, (setcc x, 0, eq))
    ...
    if (TrueV.getOpcode() == ISD::XOR && FalseV.getOpcode() == ISD::XOR &&
        ...
        isOneConstant(TrueV.getOperand(1)) &&
        TrueV.hasOneUse() && FalseV.hasOneUse()) {
      SDValue NewSel = DAG.getNode(YSXISD::SELECT_CC, DL, VT, LHS, RHS, CC,
  ...
  case YSXISD::BR_CC: {
    ...
    if (combine_CC(LHS, RHS, CC, DL, DAG, Subtarget))
      return DAG.getNode(YSXISD::BR_CC, DL, N->getValueType(0),
  ...
  case YSXISD::SRL_VL: {
    ...
    if (ShAmt.getOpcode() == YSXISD::SPLAT_VECTOR_SPLIT_I64_VL) {
      // We don't need the upper 32 bits of a 64-bit element for a shift amount.
      SDLoc DL(N);
      SDValue VL = N->getOperand(4);
      EVT VT = N->getValueType(0);
      ShAmt = DAG.getNode(YSXISD::VMV_V_X_VL, DL, VT, DAG.getUNDEF(VT),
  ...
  case ISD::SHL: {
    ...
    if (ShAmt.getOpcode() == YSXISD::SPLAT_VECTOR_SPLIT_I64_VL) {
      ...
      ShAmt = DAG.getNode(YSXISD::VMV_V_X_VL, DL, VT, DAG.getUNDEF(VT),
                          ShAmt.getOperand(1),
                          DAG.getRegister(YSX::X0, Subtarget.getXLenVT()));
      return DAG.getNode(N->getOpcode(), DL, VT, N->getOperand(0), ShAmt);
    }
    break;
  }
  ...
  case YSXISD::VFMSUB_VL:
  case YSXISD::VFNMSUB_VL:
  case YSXISD::STRICT_VFMADD_VL:
  case YSXISD::STRICT_VFNMADD_VL:
  case YSXISD::STRICT_VFMSUB_VL:
  case YSXISD::STRICT_VFNMSUB_VL:
  ...
  case ISD::STORE: {
    ...
    if (DCI.isBeforeLegalize() && IsScalarizable &&
      ...
      if (YSXMatInt::getIntMatCost(NewC, Subtarget.getXLen(), Subtarget,
                                     true) <= 2 &&
    ...
    // any illegal types.
    if ((Val.getOpcode() == YSXISD::VMV_X_S ||
  ...
  case YSXISD::VFMV_V_F_VL: {
    ...
    if (isOneConstant(VL))
      return DAG.getNode(YSXISD::VFMV_S_F_VL, DL, VT, Passthru, Scalar, VL);
    break;
  }
  case YSXISD::VMV_V_X_VL: {
    const MVT VT = N->getSimpleValueType(0);
    ...
    if (isOneConstant(VL) &&
      ...
      return DAG.getNode(YSXISD::VMV_S_X_VL, DL, VT, Passthru, Scalar, VL);
  ...
  case YSXISD::VMV_S_X_VL: {
    ...
    if (Scalar.getOpcode() == YSXISD::VMV_X_S && Passthru.isUndef() &&
    ...
    const MVT M1VT = YSXTargetLowering::getM1VT(VT);
    if (M1VT.bitsLT(VT)) {
    ...
    if (ConstantSDNode *Const = dyn_cast<ConstantSDNode>(Scalar);
        ...
        VT.bitsLE(YSXTargetLowering::getM1VT(VT)) && Passthru.isUndef())
      return DAG.getNode(YSXISD::VMV_V_X_VL, DL, VT, Passthru, Scalar, VL);
  ...
  case YSXISD::VMV_X_S: {
    ...
    if (M1VT.bitsLT(VecVT)) {
      ...
      return DAG.getNode(YSXISD::VMV_X_S, DL, N->getSimpleValueType(0), Vec);
    }
    break;
  }
  case ISD::INTRINSIC_VOID:
  ...
  case ISD::INTRINSIC_WO_CHAIN: {
    ...
    case Intrinsic::riscv_vsseg8_mask: {
      ...
      if (Subtarget.hasOptimizedSegmentLoadStore(NF) || !Tuple.hasOneUse() ||
          Tuple.getOpcode() != YSXISD::TUPLE_INSERT ||
  ...
  case ISD::BITCAST: {
    ...
    if (VT.isRISCVVectorTuple() && N0->getOpcode() == ISD::SPLAT_VECTOR) {
      ...
      for (unsigned i = 0; i < NF; ++i)
        Result = DAG.getNode(YSXISD::TUPLE_INSERT, DL, VT, Result, Splat,
                             DAG.getTargetConstant(i, DL, MVT::i32));
      return Result;
    }
  ...
  case YSXISD::TUPLE_EXTRACT: {
    ...
    if (Passthru.isUndef())
      ...
      Passthru = DAG.getNode(YSXISD::TUPLE_EXTRACT, DL, VT, Passthru,
                             N->getOperand(1));
  ...
  case YSXISD::VMERGE_VL: {
    ...
    return DAG.getNode(YSXISD::VMV_V_V_VL, SDLoc(N), N->getSimpleValueType(0),
                       Passthru, True, VL);
  }
  case YSXISD::VMV_V_V_VL: {
  ...
  case YSXISD::VSLIDEDOWN_VL:
  case YSXISD::VSLIDEUP_VL:
    if (N->getOperand(1)->isUndef())
      return N->getOperand(0);
    break;
  case YSXISD::VSLIDE1UP_VL:
  case YSXISD::VFSLIDE1UP_VL: {
    using namespace SDPatternMatch;
    SDValue SrcVec;
    SDLoc DL(N);
    MVT VT = N->getSimpleValueType(0);
    ...
    if (!N->getOperand(0).isUndef() ||
        !sd_match(N->getOperand(2),
                  m_AnyOf(m_ExtractElt(m_Value(SrcVec), m_Zero()),
                          m_Node(YSXISD::VMV_X_S, m_Value(SrcVec)))))
      break;
...
void YSXTargetLowering::computeKnownBitsForTargetNode(const SDValue Op,
                                                        KnownBits &Known,
  ...
  case YSXISD::ORC_B: {
    ...
    Known = DAG.computeKnownBits(Op.getOperand(0), Depth + 1);
    bool IsGORC = Op.getOpcode() == YSXISD::ORC_B;
...
unsigned YSXTargetLowering::ComputeNumSignBitsForTargetNode(
  ...
  case YSXISD::RORW:
  case YSXISD::ABSW:
  case YSXISD::FCVT_W_RV64:
  case YSXISD::FCVT_WU_RV64:
  case YSXISD::STRICT_FCVT_W_RV64:
  case YSXISD::STRICT_FCVT_WU_RV64:
...
bool YSXTargetLowering::canCreateUndefOrPoisonForTargetNode(
  ...
  case YSXISD::SELECT_CC:
    ...
    assert(Op.getOperand(0).getValueType().isInteger() &&
           "YSXISD::SELECT_CC only compares integers");
    return false;
  }
...
static SDValue convertLocVTToValVT(SelectionDAG &DAG, SDValue Val,
  ...
  if (VA.needsCustom()) {
    if (VA.getLocVT().isInteger() &&
      ...
      return DAG.getNode(YSXISD::FMV_H_X, DL, VA.getValVT(), Val);
    if (VA.getLocVT() == MVT::i64 && VA.getValVT() == MVT::f32)
      return DAG.getNode(YSXISD::FMV_W_X_RV64, DL, MVT::f32, Val);
...
static SDValue convertValVTToLocVT(SelectionDAG &DAG, SDValue Val,
  ...
  if (VA.needsCustom()) {
    if (LocVT.isInteger() &&
        (VA.getValVT() == MVT::f16 || VA.getValVT() == MVT::bf16))
      return DAG.getNode(YSXISD::FMV_X_ANYEXTH, DL, LocVT, Val);
    if (LocVT == MVT::i64 && VA.getValVT() == MVT::f32)
      return DAG.getNode(YSXISD::FMV_X_ANYEXTW_RV64, DL, MVT::i64, Val);
...
static SDValue unpackF64OnRV32DSoftABI(SelectionDAG &DAG, SDValue Chain,
  ...
  Register LoVReg = RegInfo.createVirtualRegister(&YSX::GPRRegClass);
  ...
  if (HiVA.isMemLoc()) {
    ...
    Register HiVReg = RegInfo.createVirtualRegister(&YSX::GPRRegClass);
    RegInfo.addLiveIn(HiVA.getLocReg(), HiVReg);
    Hi = DAG.getCopyFromReg(Chain, DL, HiVReg, MVT::i32);
  }

  // For big-endian, swap the order of Lo and Hi when building the pair.
  const YSXSubtarget &Subtarget = DAG.getSubtarget<YSXSubtarget>();
  if (!Subtarget.isLittleEndian())
    std::swap(Lo, Hi);
...
SDValue YSXTargetLowering::LowerCall(CallLoweringInfo &CLI,
  ...
  for (unsigned i = 0, j = 0, e = ArgLocs.size(), OutIdx = 0; i != e;
    ...
    if (VA.getLocVT() == MVT::i32 && VA.getValVT() == MVT::f64) {
      ...
      SDValue SplitF64 = DAG.getNode(
          YSXISD::SplitF64, DL, DAG.getVTList(MVT::i32, MVT::i32), ArgValue);
  ...
  for (unsigned i = 0, e = RVLocs.size(); i != e; ++i) {
    ...
    if (VA.getLocVT() == MVT::i32 && VA.getValVT() == MVT::f64) {
      ...
      RetValue = DAG.getNode(YSXISD::BuildPairF64, DL, MVT::f64, Lo, Hi);
    } else
...
YSXTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
  ...
  for (unsigned i = 0, e = RVLocs.size(), OutIdx = 0; i < e; ++i, ++OutIdx) {
    ...
    if (VA.getLocVT() == MVT::i32 && VA.getValVT() == MVT::f64) {
      ...
      assert(VA.needsCustom());
      SDValue SplitF64 = DAG.getNode(YSXISD::SplitF64, DL,
...
bool YSXTargetLowering::splitValueIntoRegisterParts(
  ...
  if (ValueVT.isRISCVVectorTuple() && PartVT.isRISCVVectorTuple()) {
    ...
    Val = DAG.getNode(YSXISD::TUPLE_INSERT, DL, PartVT, DAG.getUNDEF(PartVT),
...
SDValue YSXTargetLowering::joinRegisterPartsIntoValue(
  ...
  if ((ValueVT == PairVT ||
    ...
    // YSXISD::SplitGPRPair
    Val = DAG.getNode(YSXISD::SplitGPRPair, DL, DAG.getVTList(XLenVT, XLenVT),
                      Val);
...
bool YSXTargetLowering::isLegalStridedLoadStore(EVT DataType,
...
bool YSXTargetLowering::isLegalFirstFaultLoad(EVT DataType,
                                                Align Alignment) const {
  if (!Subtarget.hasVInstructions())
    return false;
