//===-- YSXMCAsmInfo.cpp - RISC-V Asm properties ------------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the declarations of the YSXMCAsmInfo properties.
//
//===----------------------------------------------------------------------===//
...
void YSXMCAsmInfo::anchor() {}

YSXMCAsmInfo::YSXMCAsmInfo(const Triple &TT) {
...
const MCExpr *YSXMCAsmInfo::getExprForFDESymbol(const MCSymbol *Sym,
                                                  unsigned Encoding,
...
void YSXMCAsmInfo::printSpecifierExpr(raw_ostream &OS,
                                        const MCSpecifierExpr &Expr) const {
  auto S = Expr.getSpecifier();
  bool HasSpecifier = S != 0 && S != ELF::R_RISCV_CALL_PLT;
  if (HasSpecifier)
    OS << '%' << YSX::getSpecifierName(S) << '(';
  printExpr(OS, *Expr.getSubExpr());
  if (HasSpecifier)
    OS << ')';
}

YSXMCAsmInfoDarwin::YSXMCAsmInfoDarwin() {
  CodePointerSize = 4;
