...
### Plan Version: 23 (Updated: Round 11 implementation)
...
|------|-----------|--------|-----|-------|-------|
| task3: Finish pruning YSX to the actual `rv64ima` source surface | AC-2, AC-3 | active after Round 11 implementation | coding | Claude | Round 11 removed the `YSXInstrInfo.cpp` verifier/comment/pseudo macro/disabled-body and unused helper slice, but vector TSFlags and operands in BaseInfo/TableGen, copied lowering/selection, frame helpers, and broad generated pseudo surfaces still need deletion or replacement with YSX-only rv64ima code. |
| task6: Revalidate after source pruning and Clang/MC surface fixes | AC-1, AC-2, AC-4 | active after Round 11 validation | coding | Claude | Round 11 validation passed for the current slice: YSX-only build, focused YSX lit, combined RISCV+YSX static build, smoke compiles, negative probes, `git diff --check`, and RISCV backend zero-diff. Keep rerunning after the remaining AC-3 pruning work. |
