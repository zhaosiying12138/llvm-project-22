...
### Plan Version: 19 (Updated: Round 9)
...
|------|-----------|--------|-----|-------|-------|
| task3: Finish pruning YSX to the actual `rv64ima` source surface | AC-2, AC-3 | active after Round 9 | coding | Claude | Round 9 removed the direct `YSXVType` forwarding namespace, pruned copied vector/vendor/profile tune feature state, and closed the reviewed MC `-mattr` leak. Retained direct vector helper use in copied lowering/selection, vector/FP/RV32/compressed metadata, frame helpers, false-return compatibility helpers, and broad TableGen surfaces still need deletion or replacement with YSX-only rv64ima code. |
| task6: Revalidate after source pruning and Clang/MC surface fixes | AC-1, AC-2, AC-4 | active after Round 9 | coding | Claude | Round 9 validated YSX-only build, focused YSX lit, combined RISCV+YSX static build, combined smoke compiles, negative MC tune/profile feature probes, and RISCV backend zero-diff; keep rerunning after the next backend pruning slices. |
...
| AC-2, AC-3, AC-4 | Round-9 YSXVType forwarding and tune/profile feature-surface pruning | 9 | pending review | YSX-only `ninja LLVMYSXCodeGen llvm-mc clang` passes; focused YSX lit reports 130 tests; combined RISCV+YSX static `ninja LLVMYSXCodeGen LLVMRISCVCodeGen llvm-mc llc clang lld` passes; `llvm-mc -triple=ysx64 -mattr=+vxrm-pipeline-flush,+log-vrgather,+single-element-vec-fp64,+prefer-vsetvli-over-read-vlenb,+andes45` probes reject; combined YSX/RISCV clang smoke compiles produce ELF64 RISC-V soft-float relocatables; `git diff -- llvm/lib/Target/RISCV | wc -l` remains `0`. |
