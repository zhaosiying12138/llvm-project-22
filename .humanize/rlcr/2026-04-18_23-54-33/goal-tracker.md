...
### Plan Version: 17 (Updated: Round 8 implementation)
...
|------|-----------|--------|-----|-------|-------|
| task3: Finish pruning YSX to the actual `rv64ima` source surface | AC-2, AC-3 | active after Round-8 implementation | coding | Claude | Round 8 removed the broad YSX arch parser and stale non-IMA pass declarations. Retained `YSXVType` forwarding, vector/FP/vendor/RV32/compressed lowering, selection, metadata, frame helpers, and false-return compatibility helpers still need deletion or replacement with YSX-only rv64ima code. |
| task6: Revalidate after source pruning and Clang/MC surface fixes | AC-1, AC-2, AC-4 | active after Round-8 implementation | coding | Claude | Round 8 validated YSX-only build, focused YSX lit, combined RISCV+YSX static build, combined smoke compiles, exact `.option arch` parsing, and RISCV backend zero-diff; keep rerunning after the next backend pruning slices. |
...
| AC-2, AC-3, AC-4 | Round-8 exact YSX arch parser and stale pass declaration pruning | 8 | pending review | `YSXISAInfo::parseArchString` now accepts only exact `rv64ima`; `.option arch, rv64ima` passes while `rv32ima` and `rv64ima_zbb` reject; stale non-IMA pass declarations and the unused vector-mask DAG mutation declaration were removed; YSX-only build, focused 130-test YSX lit, combined RISCV+YSX static build, combined smoke compiles, and RISCV backend zero-diff pass locally. |
