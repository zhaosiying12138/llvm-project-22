...
### Plan Version: 25 (Updated: Round 12 implementation)
...
|------|-----------|--------|-----|-------|-------|
| task3: Finish pruning YSX to the actual `rv64ima` source surface | AC-2, AC-3 | active after Round 12 implementation | coding | Claude | Round 12 fixed the reviewed unsupported-vector IR aborts and removed the DAGToDAG/CodeGenPrepare/memory-intrinsic crash-surface slice. Remaining AC-3 work is still substantial: `YSXSelectionDAGInfo.h` custom ISD names, broad vector lowering helpers, BaseInfo/TableGen vector metadata, RegisterInfo/Frame scalable-vector paths, Disassembler helpers, and generated pseudo surfaces still need deletion or replacement with YSX-only rv64ima code. |
| task6: Revalidate after source pruning and Clang/MC surface fixes | AC-1, AC-2, AC-4 | active after Round 12 implementation | coding | Claude | Round 12 validation passed for the current slice: YSX-only build, focused 132-test lit suite, combined RISCV+YSX build, smoke compiles, negative vector/FP/scalable-vector/RISC-V-intrinsic probes, `git diff --check`, and RISCV zero-diff check. Final validation remains active until the remaining AC-3 pruning work completes. |
...
| `YSXDisassembler.cpp` still has unused compressed/vector/FP decoder helpers | 8 | The current round removed parser and pass-surface entry points first; the warnings are dead-source cleanup candidates but do not block the exact parser/pass-declaration slice. | Revisit in the next AC-3 source-pruning slice, preferably alongside TableGen decoder/register-class cleanup. |
...
| AC-2, AC-3, AC-4 | Round-12 unsupported-vector IR guard and DAG/Lowering crash-surface pruning | 12 | 12 local | YSX-only build, 132-test focused lit suite, combined RISCV+YSX build, YSX/RISCV smoke compiles, unsupported `+v`/`rv64imaf` probes, scalable-vector IR and direct `llvm.riscv.vsetvli` negative `llc` probes, `clang -###`, `git diff --check`, and RISCV zero-diff check pass. Full AC-3 remains active for custom ISD, TableGen/BaseInfo, RegisterInfo/Frame, Disassembler, and generated pseudo surfaces. |
