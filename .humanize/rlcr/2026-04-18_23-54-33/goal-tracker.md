...
### Plan Version: 21 (Updated: Round 10)
...
|------|-----------|--------|-----|-------|-------|
| task3: Finish pruning YSX to the actual `rv64ima` source surface | AC-2, AC-3 | active after Round 10 | coding | Claude | Round 10 removed a metadata/instr-info slice, but retained direct vector helper use in copied lowering/selection, vector/FP/RV32/compressed TableGen metadata, frame helpers, false-return compatibility helpers, and broad generated pseudo surfaces still need deletion or replacement with YSX-only rv64ima code. |
| task6: Revalidate after source pruning and Clang/MC surface fixes | AC-1, AC-2, AC-4 | active after Round 10 | coding | Claude | Round 10 validated YSX-only build, focused YSX lit, combined RISCV+YSX static build, combined smoke compiles, and RISCV backend zero-diff; keep rerunning after the next backend pruning slices. |
...
| AC-3, AC-4 | Round-10 vector metadata and instruction-info pruning slice | 10 | pending review | YSX-only `ninja LLVMYSXCodeGen llvm-mc clang` passes; focused YSX lit reports 130 tests; combined RISCV+YSX static `ninja LLVMYSXCodeGen LLVMRISCVCodeGen llvm-mc llc clang lld` passes; YSX and RISCV smoke compiles produce ELF64 RISC-V soft-float relocatables; specified metadata/instr-info files no longer reference `RISCVVType`; `git diff -- llvm/lib/Target/RISCV | wc -l` remains `0`. |
